...
let test_shared_rng () =
  ...
  let wrapped =
    Env.map_observation
      ...
      env
  in
  let new_key = Rune.Rng.key 999 in
  Env.set_rng wrapped new_key;
  equal ~msg:"inner sees new rng" int (Rune.Rng.to_int new_key)
...
let make_box_action_env ~rng () =
  ...
  in
  let env =
    Env.create ~id:"BoxAct-v0" ~rng ~observation_space:obs_space
      ~action_space:act_space ~reset ~step ()
  in
  (env, last_action)

let test_clip_action () =
  let env, last_action = make_box_action_env ~rng () in
...
let make_box_obs_env ~rng () =
  ...
  let step _env action =
    ...
    Env.step_result
      ...
      ~reward:1.0 ()
  in
  Env.create ~id:"BoxObs-v0" ~rng ~observation_space:obs_space
    ~action_space:act_space ~reset ~step ()

let test_clip_observation () =
  let env = make_box_obs_env ~rng () in
...
let () =
  run "Fehu.Env (wrappers)"
    [
      group "state sharing"
        [
          test "set_rng on wrapper updates inner" test_shared_rng;
