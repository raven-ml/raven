...
let animate_episode log delay =
  ...
  Printf.printf "Won: %b | Total Reward: %.2f | Steps: %d\n\n" log.won
    log.total_reward log.total_steps;

  List.iter
    (fun frame ->
      Printf.printf "Step %d - Action: %s - Reward: %.2f" frame.step
        frame.action frame.reward;
  ...
  if log.won then print_endline "🎉 Level Complete!"
  else print_endline "Episode ended (timeout or failure)"
...
let summary_stats logs =
  ...
  let total_reward =
    List.fold_left (fun acc l -> acc +. l.total_reward) 0.0 logs
  in
  let avg_steps =
  ...
  Printf.printf "Wins: %d (%.1f%%)\n" wins
    (float_of_int wins /. float_of_int total *. 100.0);
