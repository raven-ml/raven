...
let () =
  let npz_arr1 : int32_t = Ndarray.arange Int32 0 5 1 in
  let npz_arr2 : int32_t = Ndarray.full Int32 [| 2; 2 |] 42l in
  let items_to_save : (string * packed_ndarray) list =
  ...
  Hashtbl.iter
    (fun name packed_arr ->
      ...
      | P arr -> (
          let dtype = Ndarray.dtype arr in
          let shape = Ndarray.shape arr in
          Format.printf "  Found member: '%s' (kind: %a, shape: %a)\n" name
            Ndarray.pp_dtype dtype Ndarray.pp_shape shape;
  ...
  let packed_member : packed_ndarray =
