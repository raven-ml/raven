...
type packed_ndarray = Utils.packed_ndarray =
  | P : ('a, 'b) Nd.t -> packed_ndarray

type ndarray_dims = [ `Gray of int * int | `Color of int * int * int ]

let get_ndarray_dims arr : ndarray_dims =
  match Nd.shape arr with
  | [| h; w |] -> `Gray (h, w)
  | [| h; w; c |] -> `Color (h, w, c)
  | s ->
      fail_msg "Invalid ndarray dimensions: expected 2 or 3, got %d (%s)"
        (Array.length s)
...
let load_image ?(grayscale = false) path =
  try
    ...
    | Ok img ->
        ...
        let nd = Nd.of_bigarray (genarray_of_array1 buffer) in
...
let save_image nd_img path =
  try
    let h, w, c =
      match get_ndarray_dims nd_img with
      | `Gray (h, w) -> (h, w, 1)
      | `Color (h, w, c) -> (h, w, c)
    in

    (* Ensure the input array is uint8 *)
    let data_gen = Nd.to_bigarray nd_img in
    let data =
...
let save_npy ndarray path =
  try
    let genarray = Nd.to_bigarray ndarray in
  ...
  try
    ...
    List.iter
      (fun name ->
        ...
        | Npy.P genarray ->
            let genarray = Genarray.change_layout genarray Bigarray.c_layout in
            Hashtbl.add archive name (P (Nd.of_bigarray genarray)))
      entries;
    Npy.Npz.close_in zi;
    archive
  with
...
let to_float16 = convert "to_float16" Nd.float16
let to_float32 = convert "to_float32" Nd.float32
let to_float64 = convert "to_float64" Nd.float64
let to_int8 = convert "to_int8" Nd.int8
let to_int16 = convert "to_int16" Nd.int16
let to_int32 = convert "to_int32" Nd.int32
let to_int64 = convert "to_int64" Nd.int64
let to_uint8 = convert "to_uint8" Nd.uint8
let to_uint16 = convert "to_uint16" Nd.uint16
let to_complex32 = convert "to_complex32" Nd.complex32
let to_complex64 = convert "to_complex64" Nd.complex64
