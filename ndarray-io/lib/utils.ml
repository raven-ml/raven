...
type packed_ndarray = P : ('a, 'b) Nd.t -> packed_ndarray
...
let convert : type a b.
    string -> (a, b) Ndarray.dtype -> packed_ndarray -> (a, b) Ndarray.t =
 fun name target_kind packed ->
  match packed with
  | P ndarray -> (
      let source_kind = Ndarray.dtype ndarray in
      match eq_dtype source_kind target_kind with
      | Some Npy.Eq.W -> ndarray
      | None ->
