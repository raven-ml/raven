...
(** Training logger.
    ...
    {[
      ...
      (* in training loop *)
      Log.scalars logger ~step ~epoch [ ("train/loss", loss, true) ];
      Log.close logger
    ]} *)
...
val scalars :
  ...
  unit
(** [scalars t ~step triples] appends one scalar per [(tag, value, minimize)].

    [minimize]: [true] = lower is better, [false] = higher is better.
