...
(* Shared fixtures and checks for the rune-next test suite. *)
...
let check_grad ?(h = 1e-5) ?(tol = 1e-3) ~msg (f : Nx.float64_t -> Nx.float64_t)
  ...
  let analytic = to_arr (Rune_next.grad' loss x) in
  let numeric =
    central_diff ~h
...
let check_grad2 ?(h = 1e-5) ?(tol = 1e-3) ~msg
  ...
  let g = Rune_next.grad (module Pair) loss { fst = a; snd = b } in
