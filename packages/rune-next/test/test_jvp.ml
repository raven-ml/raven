...
let test_jvp_record_analytic () =
  ...
  let _, dy = Rune_next.jvp (module Params) f p t in
  (* 2 * (1 - 2 + 3) = 4 *)
...
let test_jvp_constant_function () =
  ...
  let _, dy = Rune_next.jvp' f (v3 ()) (tangent_like (v3 ())) in
...
let test_jvp_aux () =
  ...
  let y, dy, aux = Rune_next.jvp_aux (module Params) f p t in
...
let test_jvp_matches_grad () =
  ...
  let _, dy = Rune_next.jvp' f x v in
...
let test_hessian_vector_product () =
  ...
  let _, hv = Rune_next.jvp' (Rune_next.grad' f) x v in
...
let test_grad_of_jvp () =
  ...
  let outer x = snd (Rune_next.jvp' f x v) in
  let g = Rune_next.grad' outer (vec64 [| 1.0; -2.0; 3.0 |]) in
...
let test_nested_jvp () =
  ...
  let f x = Nx.sum (Nx.mul x (Nx.mul x x)) in
  let v = vec64 [| 1.0; 0.5; -1.0 |] in
  let inner x = snd (Rune_next.jvp' f x v) in
  let _, ddy = Rune_next.jvp' inner (vec64 [| 1.0; -2.0; 3.0 |]) v in
