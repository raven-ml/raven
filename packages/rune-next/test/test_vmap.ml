...
let check_vmap ~msg f x =
  check_arr ~msg (to_arr (loop_map f x)) (Rune_next.vmap' f x)
...
let test_in_axis () =
  ...
  check_arr ~msg:"in_axis 1"
    ...
    (Rune_next.vmap' ~in_axis:1 (fun r -> Nx.sum (Nx.mul r r)) x)

let test_out_axis () =
  let y = Rune_next.vmap' ~out_axis:1 (fun r -> Nx.mul r r) (xs ()) in
...
let test_in_axes_non_leading () =
  ...
  let b = w32 () in
  let y =
    Rune_next.vmap ~in_axes:[ Some 1; None ]
      (module Pair)
      (fun p -> Nx.matmul p.fst p.snd)
      { fst = a; snd = b }
  in
...
let test_in_axes_maps_no_leaf () =
  raises_invalid_arg (fun () ->
      ignore
        (Rune_next.vmap ~in_axes:[ None; None ]
           (module Pair)
           (fun p -> Nx.add p.fst p.snd)
           { fst = xs (); snd = xs () }))
...
let test_nested_vmap () =
  ...
  let y = Rune_next.vmap' (Rune_next.vmap' f) x in
  let expected =
...
let test_per_sample_gradients () =
  ...
  let g = Rune_next.vmap' (fun x -> Rune_next.grad' f x) (xs ()) in
...
let test_grad_through_vmap () =
  ...
  let f x = Nx.sum (Rune_next.vmap' (fun r -> Nx.sum (Nx.mul r r)) x) in
  let g = Rune_next.grad' f (xs ()) in
...
let test_per_sample_gradients_of_gather () =
  ...
  let expected =
    Nx.stack ~axis:0
      (List.init 4 (fun i -> Rune_next.grad' f (Nx.slice [ Nx.I i ] (xs ()))))
  in
...
let test_jvp_through_vmap () =
  ...
  let f x = Rune_next.vmap' (fun r -> Nx.sum (Nx.mul r r)) x in
  let v = tangent_like (xs ()) in
  let _, dy = Rune_next.jvp' f (xs ()) v in
