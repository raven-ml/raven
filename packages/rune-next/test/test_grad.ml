...
let test_grad_record_analytic () =
  let g = Rune_next.grad (module Params) quadratic (params ()) in
...
let test_grad_preserves_structure () =
  let p = params () in
  let g = Rune_next.grad (module Params) quadratic p in
...
let test_value_and_grad_value () =
  let v, _ = Rune_next.value_and_grad (module Params) quadratic (params ()) in
  (* 1 + 4 + 9 + 3 * 0.5 = 15.5 *)
  check_arr ~msg:"value" [| 15.5 |] v

let test_value_and_grad_aux () =
  let f p = (quadratic p, "aux") in
  let v, g, aux = Rune_next.value_and_grad_aux (module Params) f (params ()) in
  check_arr ~msg:"value" [| 15.5 |] v;
  check_arr ~msg:"dw" [| 2.0; -4.0; 6.0 |] g.w;
...
let test_vjp_cotangent_scales () =
  let _, g =
    Rune_next.vjp (module Params) quadratic (params ()) (Nx.scalar f32 2.0)
  in
...
let test_mixed_dtype_single_pass () =
  ...
  in
  let g = Rune_next.grad (module Params) f (params ()) in
  check_arr ~msg:"dw" [| 2.0; -4.0; 6.0 |] g.w;
...
let test_gradient_descent_converges () =
  ...
  in
  let step p =
    let g = Rune_next.grad (module Params) f p in
...
let test_grad_single_tensor () =
  ...
  let g = Rune_next.grad' f (vec32 [| 1.0; -2.0; 3.0 |]) in
