...
let test_rule_replaces_autodiff () =
  let g = Rune_next.grad' (fun x -> Nx.sum (fake_grad_sin x)) (v3 ()) in
  check_arr ~msg:"fake rule" [| 100.0; 100.0; 100.0 |] g

let test_correct_rule_matches_autodiff () =
  let x = v3 () in
  let expected = Rune_next.grad' (fun x -> Nx.sum (Nx.sin x)) x in
  let actual = Rune_next.grad' (fun x -> Nx.sum (my_sin x)) x in
...
let test_multi_leaf_rule () =
  ...
  let g =
    Rune_next.grad (module Pair) (fun p -> Nx.sum (f p)) { fst = a; snd = b }
  in
...
let test_custom_vjp_rejects_forward_mode () =
  raises_invalid_arg (fun () ->
      ignore (Rune_next.jvp' my_sin (v3 ()) (tangent_like (v3 ()))))
...
let test_custom_rule_under_vmap_of_grad () =
  ...
  let g =
    Rune_next.vmap' (fun x -> Rune_next.grad' (fun x -> Nx.sum (my_sin x)) x) xs
  in
...
let () = run "rune-next custom" tests
