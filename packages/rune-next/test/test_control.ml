...
let test_grad_through_scan () =
  ...
  check_arr ~msg:"d scan"
    (to_arr (Rune_next.grad' g (v4 ())))
    (Rune_next.grad' f (v4 ()))
...
let test_vmap_of_scan () =
  ...
  in
  let y = Rune_next.vmap' cumsum_scan x in
...
let test_grad_through_cond () =
  ...
  check_arr ~msg:"then grad" [| 1.0; 4.0 |]
    (Rune_next.grad' f (vec64 [| 0.5; 2.0 |]));
  ...
  let f x =
    Nx.sum
      (Rune_next.while_loop
         ...
         x)
  in
  check_arr ~msg:"d while" [| 8.0; 8.0 |]
    (Rune_next.grad' f (vec64 [| 1.0; 0.5 |]))
...
let () = run "rune-next control" tests
