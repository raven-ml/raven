...
let test_pullback_reusable () =
  ...
  let _, pullback = Rune_next.vjp_fun (module Pair) f { fst = a; snd = b } in
  let ct1 = vec64 [| 1.0; 0.0; 0.0 |] and ct2 = vec64 [| 0.0; 2.0; 0.0 |] in
  let g1 = pullback ct1 in
  let g2 = pullback ct2 in
  let _, e1 = Rune_next.vjp (module Pair) f { fst = a; snd = b } ct1 in
  let _, e2 = Rune_next.vjp (module Pair) f { fst = a; snd = b } ct2 in
...
let test_jacfwd_matches_jacrev () =
  let jf = Rune_next.jacfwd' f32_fn (v3 ()) in
  let jr = Rune_next.jacrev' f32_fn (v3 ()) in
...
let test_jacobian_analytic () =
  ...
  let x = v3 () in
  let j = Rune_next.jacrev' f32_fn x in
...
let test_hessian_analytic () =
  ...
  let h = Rune_next.hessian' cube (v3 ()) in
...
let test_hvp_matches_hessian () =
  ...
  let hv = Rune_next.hvp' cube x v in
  let expected = Nx.matmul (Rune_next.hessian' cube x) v in
...
let test_hvp_structured () =
  ...
  in
  let hv = Rune_next.hvp (module Pair) f params v in
...
let test_check_grads_accepts_correct () =
  ...
  in
  match Rune_next.check_grads (module Pair) f params with
  | Ok () -> ()
  | Error msg -> fail msg
...
let test_check_grads_catches_wrong_rule () =
  ...
  in
  match
    Rune_next.check_grads (module Single) (fun x -> Nx.sum (broken x)) (v3 ())
  with
...
let () = run "rune-next jacobian" tests
