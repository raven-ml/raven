...
let err_no_rule op =
  invalid_arg
    (Printf.sprintf
       "Rune_next: the tangent of %s is not implemented; detach its input if \
      ...
      | Custom.E_custom_vjp (Custom.Vjp_call { tree; params; fwd; _ }) ->
          Some
            (fun k ->
              ...
              if !any then
                invalid_arg
                  "Rune_next: a custom_vjp function is not \
                   forward-differentiable; define a custom_jvp rule instead"
