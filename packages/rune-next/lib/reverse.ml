...
let err_no_rule op =
  invalid_arg
    (Printf.sprintf
       "Rune_next: the gradient of %s is not implemented; detach its input if \
      ...
      | Custom.E_custom_jvp (Custom.Jvp_call { tree; params; f; _ }) ->
          Some
            (fun k ->
              ...
              if !any then
                invalid_arg
                  "Rune_next: a custom_jvp function is not \
                   reverse-differentiable; define a custom_vjp rule instead"
