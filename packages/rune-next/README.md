...
Rune-next differentiates ordinary OCaml functions over ordinary OCaml values:
...
[kaun-next](../kaun-next/) for neural-network layers built this way, and
...
See the [API reference](lib/rune_next.mli) for the full contracts.
...
```sh
dune exec packages/rune-next/examples/01-gradient-descent/main.exe
```

## Limitations

Rune-next aims to fail loudly rather than return wrong gradients.
Current gaps:
