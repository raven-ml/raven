...
(executable
 (name main)
 (libraries rune-next nx))
```

## Your First Gradient

Rune-next differentiates ordinary functions over plain Nx tensors. For a function of a single tensor, use `grad'`:
...
Real models have more than one parameter. In rune-next the parameters are a record you define, made traversable by implementing `Nx.Ptree.S` — three one-line functions that visit the record's tensor leaves:
...
let () =
  ...
  let g = Rune_next.grad (module Params) loss params in
...
let () =
  ...
  let lr = 0.1 in
  let step p =
    let l, g = Rune_next.value_and_grad (module Params) loss p in
    let p =
...
This is the whole pattern — the full program is [`examples/01-gradient-descent`](https://github.com/raven-ml/raven/tree/main/packages/rune-next/examples/01-gradient-descent). For neural networks, [kaun-next](/docs/kaun-next/) provides layers whose parameter records compose exactly this way.
...
let () =
  ...
  let loss, g, pred = Rune_next.value_and_grad_aux (module Vec) f x in
...
let () =
  ...
  let f' = Rune_next.grad' f in      (* 4x³ *)
  let f'' = Rune_next.grad' f' in    (* 12x² *)
  let f''' = Rune_next.grad' f'' in  (* 24x *)
  let x = Nx.scalar Nx.float32 2.0 in
...
let () =
  ...
  let f v = Nx.mean (Nx.mul v (Rune_next.detach v)) in
  Printf.printf "with detach:  %s\n"
    (Nx.data_to_string (Rune_next.grad' f x));

  (* no_grad: nothing inside is recorded. *)
  let g v =
    let baseline = Rune_next.no_grad (fun () -> Nx.mean v) in
    Nx.mean (Nx.mul v (Nx.sub v baseline))
  in
  ignore (Rune_next.grad' g x)
```
...
- [Kaun-next Getting Started](/docs/kaun-next/getting-started/) — neural networks on top of rune-next
