...
let output channel events =
  ...
  let origins = Hashtbl.create (List.length devices) in
  List.iter (fun event ->
      ...
      let first = match Hashtbl.find_opt origins event.device with
        | None -> event.start_us | Some first -> min first event.start_us in
      Hashtbl.replace origins event.device first) events;
  ...
  List.iter (fun event ->
      ...
      Printf.fprintf channel ",\"pid\":%d,\"tid\":%d,\"ts\":%.17g,\"dur\":%.17g}"
        ...
        (event.start_us -. Hashtbl.find origins event.device)
