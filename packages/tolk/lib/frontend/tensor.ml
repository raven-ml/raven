...
let rec register t =
  let cap = Stdlib.Weak.length !live in
  if !live_n < cap then begin
    Stdlib.Weak.set !live !live_n (Some t);
    incr live_n
  end
  else begin
    ...
    let fresh = Stdlib.Weak.create cap' in
    List.iteri (fun i x -> Stdlib.Weak.set fresh i (Some x)) !kept;
    live := fresh;
    live_n := !n;
    register t
  end
