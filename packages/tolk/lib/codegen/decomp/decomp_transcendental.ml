...
let is_transcendental dt = List.mem (Dtype.scalar dt) transcendental_scalars
...
let const_float_dt dt x = const_float_v (Dtype.val_of dt) x
...
let shl x y =
  let dx = Uop.dtype x in
  let v = Dtype.val_of dx in
...
let int_for_float = function
  | Dtype.Float64 -> Dtype.Val.int64
  | Dtype.Float32 -> Dtype.Val.int32
  | Dtype.Float16 -> Dtype.Val.int16
  | _ -> Dtype.Val.int32

let uint_for_float = function
  | Dtype.Float64 -> Dtype.Val.uint64
  | Dtype.Float32 -> Dtype.Val.uint32
  | Dtype.Float16 -> Dtype.Val.uint16
  | _ -> Dtype.Val.uint32

let float_for_int = function
  | Dtype.Int64 -> Dtype.Val.float64
  | Dtype.Int32 -> Dtype.Val.float32
  | Dtype.Int16 -> Dtype.Val.float16
  | _ -> Dtype.Val.float32
...
let rintk d =
  ...
  let vcount = Dtype.vcount fdt in
  let out = Dtype.Val (Dtype.Val.vec vcount (int_for_float (Dtype.scalar fdt))) in
...
let pow2if q float_dtype =
  ...
  let qscalar = Dtype.scalar qdt in
  let vcount = Dtype.vcount qdt in
  let out_scalar = match qscalar with
    | Dtype.Int16 -> Dtype.Val.scalar (Dtype.Val.scalarize (Dtype.val_of float_dtype))
    | _ -> Dtype.Val.scalar (float_for_int qscalar)
  in
  let out = Dtype.Val (Dtype.Val.vec vcount (Dtype.Val.of_scalar out_scalar)) in
  let qv = Dtype.val_of qdt in
  let q_biased =
...
let ilogb2k d =
  let fdt = Uop.dtype d in
  let vcount = Dtype.vcount fdt in
  let int_dt = Dtype.Val (Dtype.Val.vec vcount (int_for_float (Dtype.scalar fdt))) in
  let iv = Dtype.val_of int_dt in
...
let ldexp2k d e =
  let fdt = Uop.dtype d in
  let one = Uop.const (Const.int (Dtype.val_of (Uop.dtype e)) 1) in
...
let frexp v =
  ...
  let m1_raw, m2_raw = match scalar with
  ...
  in
  let uint_dt = Dtype.Val (Dtype.Val.vec vcount (uint_for_float scalar)) in
  let uv = Dtype.val_of uint_dt in
...
let lazy_map_numbers x ~inf ~ninf ~nan ~ratio =
  let fdt = Uop.dtype x in
  let v = Dtype.val_of fdt in
...
let payne_hanek_reduction d =
  ...
  let intermediate_dtype =
    if Dtype.scalar fdt = Dtype.Float16
    then Dtype.Val (Dtype.Val.vec vcount Dtype.Val.float32)
    else fdt
  in
  let uint64_dt = Dtype.Val (Dtype.Val.vec vcount Dtype.Val.uint64) in
  let int32_dt = Dtype.Val (Dtype.Val.vec vcount Dtype.Val.int32) in
  let uint32_dt = Dtype.Val (Dtype.Val.vec vcount Dtype.Val.uint32) in
  let u64v = Dtype.val_of uint64_dt in
  let u32v = Dtype.val_of uint32_dt in
  let iv = Dtype.Val.int32 in
...
let cody_waite_reduction d =
  ...
  let qdh =
    if scalar = Dtype.Float64 then
      let int64_dt = Dtype.Val (Dtype.Val.vec vcount Dtype.Val.int64) in
      Uop.alu_binary ~op:Ops.Mul
  ...
  let r = match scalar with
    ...
    | Dtype.Float16 ->
        let f32_dt = Dtype.Val (Dtype.Val.vec vcount Dtype.Val.float32) in
...
let trig_poly d coeff32 coeff64 =
  ...
  let coeffs =
    if Dtype.scalar (Uop.dtype d) = Dtype.Float64 then coeff64 else coeff32
  in
...
let ifand q n =
  let v = Dtype.val_of (Uop.dtype q) in
...
let sin_poly_small d q =
  let r = sin_poly d in
  let v = Dtype.val_of (Uop.dtype r) in
...
let sin_poly_large d q =
  let v = Dtype.val_of (Uop.dtype d) in
...
let xlog2 d =
  ...
  let flt_min_val = match scalar with
...
let xpow base exponent =
  ...
  let v = Dtype.val_of dt in
  let exp_v = Dtype.val_of exp_dt in
  let zero = const_float_v v 0.0 in
  let exp_zero = const_float_v exp_v 0.0 in
  let one = const_float_v v 1.0 in
  let nan_c = const_float_v v Float.nan in
  let two_i = Uop.const (Const.int Dtype.Val.int32 2) in
  let open Uop.O in
...
let via_f32 f d dtype =
  if is_transcendental (Dtype.Val dtype) then Some (f d)
  else if Dtype.Val.is_float dtype then
    let f32_dt = Dtype.Val (Dtype.Val.vec (Dtype.Val.count dtype) Dtype.Val.float32) in
    let d32 = Uop.cast ~src:d ~dtype:f32_dt in
    Some (Uop.cast ~src:(f d32) ~dtype:(Dtype.Val dtype))
  else None
