...
let compute_setup q ?compute_class ?local_mem_window ?shared_mem_window ?local_mem
      ?local_mem_tpc_bytes () =
    Option.iter
      (fun c -> nvm q 1 Defs.nvc6c0_set_object [| c |])
      compute_class;
    Option.iter
      (fun w ->
        let w = va64 w in
        nvm q 1 Defs.nvc6c0_set_shader_local_memory_window_a
          [| hi32 w; lo32 w |])
      local_mem_window;
    Option.iter
      (fun w ->
        let w = va64 w in
        nvm q 1 Defs.nvc6c0_set_shader_shared_memory_window_a
          [| hi32 w; lo32 w |])
      shared_mem_window;
    Option.iter
      (fun a ->
        let a = va64 a in
        nvm q 1 Defs.nvc6c0_set_shader_local_memory_a [| hi32 a; lo32 a |])
      local_mem;
    Option.iter
      (fun b ->
        let b = Int64.of_int b in
        nvm q 1 Defs.nvc6c0_set_shader_local_memory_non_throttled_a
          [| hi32 b; lo32 b; 0xff |])
      local_mem_tpc_bytes

let memory_barrier q =
    nvm q 1 Defs.nvc6c0_invalidate_shader_caches_no_wfi
      [|
        ...
        List.iter (fun node -> match U.as_call node, U.arg node with
          ...
          | _, U.Arg.Typed ("barrier", _) ->
              ...
              if compute then begin
                let cq = Q.create () in
                memory_barrier cq;
                q (Array.to_list (Q.dwords cq) |> List.map u32)
              end
...
let ensure_has_local_memory (dev : 'meta device) ~num_gpcs
  ...
  if dev.slm_per_thread < required then begin
    ...
    submit_commands ~device ~queue:"COMPUTE:0" (Q.dwords cq);
...
let open_device ?(is_valid = fun () -> true) ~name (iface : 'mem Nv_iface.t) =
  ...
  let initialize device =
    let cq = Q.create () in
    compute_setup cq ~compute_class:usermode.Nv_iface.compute_class
      ~local_mem_window:hw.local_mem_window
      ~shared_mem_window:hw.shared_mem_window ();
    submit_commands ~device ~queue:"COMPUTE:0" (Q.dwords cq);
    let cp = Q.create () in
    nvm cp 4 Defs.nvc6c0_set_object [|usermode.Nv_iface.dma_class|];
    submit_commands ~device ~queue:"COPY:0" (Q.dwords cp) in
