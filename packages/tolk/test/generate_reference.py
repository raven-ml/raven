...
With --check, any changed committed upstream expectation also fails.
...
def check_expectations(committed, generated, expected, reference_only):
    """Compare generated raw bytes; reference-only entries still must exist."""
    report = {"matched": [], "changed": [], "missing": [],
              "reference_only": [], "missing_baselines": []}
    differences = []
    for filename in sorted(expected):
        source = generated / filename
        baseline = committed / filename
        if not source.is_file():
            report["missing"].append(filename)
            continue
        if not baseline.is_file():
            key = "reference_only" if filename in reference_only else "missing_baselines"
            report[key].append(filename)
            continue
        before, after = baseline.read_bytes(), source.read_bytes()
        if before == after:
        ...
        report["changed"].append({
            ...
            "generated_sha256": hashlib.sha256(after).hexdigest(),
        })
        differences.extend(difflib.diff_bytes(
            difflib.unified_diff, before.splitlines(keepends=True),
            after.splitlines(keepends=True),
            fromfile=("committed/" + filename).encode("utf-8"),
            tofile=("generated/" + filename).encode("utf-8"),
        ))
    # Diagnostics only: equality is always decided on the original bytes.
    with (generated / "comparison.diff").open("wb") as output:
...
def generate(reference, revision, output, suite, check_cpu=False, check=False):
    ...
    with tempfile.TemporaryDirectory(prefix="tolk-reference-") as tmp:
        ...
        for driver in drivers:
            ...
            # .expected.diff exceptions are not upstream output inventory.
            ...
            if check:
                comparison = check_expectations(driver.parent, destination, expected, reference_only)
                driver_report["comparison"] = comparison
                changed = [entry["file"] for entry in comparison["changed"]]
                if changed or comparison["missing_baselines"]:
                    failures.append(
                        f"{name}: changed baselines={changed}, "
                        f"missing baselines={comparison['missing_baselines']}; "
                        f"see {destination / 'comparison.diff'}"
                    )
            manifest["drivers"].append(driver_report)
    ...
    if failures:
        raise RuntimeError("reference corpus validation failed:\n" + "\n".join(failures))
...
def main():
    ...
    parser.add_argument("--check", action="store_true",
                        help="compare generated raw output with committed .expected bytes")
    ...
    generate(args.reference.resolve(), args.revision, args.output.resolve(), args.suite,
             check_cpu=args.check_cpu_sources, check=args.check)
