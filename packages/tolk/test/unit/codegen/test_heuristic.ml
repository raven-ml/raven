...
let image_tests =
  group "image"
    [
      test "IMAGE upcasts unit-stride image axis by four" (fun () ->
      ...
      test "IMAGE unrolls unit-stride reduce image axis by four" (fun () ->
  ...
  group "masked upcast"
    [
      ...
      test "upcasts small WHERE-guarded dim" (fun () ->
          ...
          let opts = run_heuristic ast ren in
          is_true (has is_upcast opts));
      test "IMAGE default occupancy floor skips small global grid" (fun () ->
          ...
          equal (list (pair int int)) []
            (List.filter_map upcast_axis_amount opts));
      test "IMAGE occupancy accounts for cumulative global upcasts" (fun () ->
