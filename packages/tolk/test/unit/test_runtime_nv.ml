  ...
  let allocator = match allocator with
    ...
    | None ->
        ...
        Device.Allocator.Pack {host_allocator with addr = Some (fun address ->
            Option.value (Hashtbl.find_opt device_addresses address) ~default:address)} in
  ...
  let bufferize u = match U.as_param u with
    ...
    (match image_address, U.node_tag u with
     | Some address, Some "program" ->
         Hashtbl.replace device_addresses (Device.Buffer.addr buffer) address
     | _, Some tag when String.starts_with ~prefix:"cmdbuf_" tag ->
         ...
         Hashtbl.replace device_addresses (Device.Buffer.addr buffer)
