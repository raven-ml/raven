...
let cache_info_semantic_key_parity () =
  ...
  let call_with_aux =
    Uop.call ~body:sink ~args:[] ~info:(call_info ~aux:{fallback = []; devices = ["NV"]; linked_owners = []; host = "CPU"; table = -1; inputs = []; outputs = []; timings = []; independent_accesses = []; host_deps = []; accesses = [[]]} ())
  in
  ...
  let linked = Uop.call ~body:sink ~args:[]
      ~info:{info with aux = Some {aux with linked_owners = ["NV", 7]}} in
  raises (Invalid_argument "Uop.export: graph carries linked native addresses")
    (fun () -> ignore (Uop.export linked));
