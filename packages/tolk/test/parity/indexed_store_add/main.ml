...
let kernel () =
  ...
  let k =
    U.range ~size:(U.const_int updates) ~axis:1 ~kind:Axis_type.Reduce ()
  in
  ...
  in
  let cell = U.index ~ptr:dst ~idxs:[ target ] () in
  let st = U.store ~dst:cell ~value:(U.load ~src:cell () + value) () in
  let e = U.end_ ~value:(U.end_ ~value:st ~ranges:[ k ]) ~ranges:[ j ] in
  U.sink
    ~kernel_info:
      {
