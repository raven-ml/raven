(* Parity case: C = A @ B, float16 inputs, float32 accumulate, M=N=K=16.
   ...
   engages the 16x16x16 half tensor core (WMMA kernel) on both
   architectures. *)
...
let backends =
  [
    ...
    ("amd_gfx1201", Tolk.Cstyle.amd Tolk.Gpu_target.RDNA4);
  ]

let build () =
  let m, n, k = (16, 16, 16) in
  let a = Helpers.mk_param ~idx:0 ~dtype:Dtype.float16 [ m; k ] in
  let b = Helpers.mk_param ~idx:1 ~dtype:Dtype.float16 [ k; n ] in
