...
let kernel () =
  let p0 = U.param ~slot:0 ~dtype:f16_ptr ~shape:(U.const_int 256) () in
  let p1 = U.param ~slot:1 ~dtype:Helpers.global_fptr ~shape:(U.const_int 256) () in
  let p2 = U.param ~slot:2 ~dtype:Helpers.global_fptr ~shape:(U.const_int 256) () in
