...
(* One configuration of the load benchmark: load a Llama checkpoint from the
   cache, import it at a dtype, and with [--device] run one compiled forward
   pass over eight tokens, which uploads every weight. It prints the wall time
   to the end of each phase; [run.sh] measures the process's peak memory around
   it, one process per configuration. *)
...
let () =
  ...
  in
  let params = Llama.of_hf cfg dt ckpt in
  ...
  if !device <> "" then begin
    ...
    in
    let logits = Rune.jit' ~device:!device forward ids in
