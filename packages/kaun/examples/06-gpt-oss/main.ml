...
let generate (type b) ?device cfg (params : (float, b) Nx.t Gpt_oss.params)
    (dt : (float, b) Nx.dtype) ~log ~count ~on_token prompt =
...
let () =
  ...
  if !prompt = "" then
    let out =
      generate ?device cfg params dt ~log:stdout ~count:(count 16)
    ...
    let out =
      generate ?device cfg params dt ~log:stderr ~count:(count 256) ~on_token
