...
(** Dropout regularization (Srivastava et al., 2014).
    ...
    Training-mode masks come from an explicit {!Rune.Rng} key when [?key] is
    ...
    fresh one per step with {!Rune.Rng.fold_in}. *)
...
(** [apply ~rate ~training ?key x] is [x] with dropout applied.
    ...
    mask is [Rune.Rng.bernoulli key ~p:(1. -. rate)] at [x]'s shape: the same
    key, rate and shape give the same mask, so derive a fresh key per call
    ({!Rune.Rng.split}, {!Rune.Rng.fold_in}) for a fresh mask. Without [?key],
    ...
    a key leaf of the step's input structure, or one {!Rune.Rng.fold_in}-derived
    ...
    For independent lanes, {!Rune.Rng.split} one key into per-lane keys, stack
