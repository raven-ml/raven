# Kaun vs. PyTorch — A Practical Comparison

This guide explains how kaun relates to PyTorch, focusing on:
...
The main shift is from mutable objects to immutable records: a PyTorch model is an object that owns its parameters and mutates them in place; a kaun model is a record of tensors that functions consume and produce. There is no module base class, no parameter registry, and no trainer.

---

## 1. Big-Picture Differences

| Aspect | PyTorch | kaun (OCaml) |
| --- | --- | --- |
...
**kaun** — the step is a pure function from state to state:
...
**kaun** — mode is an argument, statistics are a value:
...
**kaun** — for in-memory tensors, a `Seq.t` of minibatches:
...
Iterating the sequence again reshuffles — one sequence value serves every epoch. There is no worker pool or async prefetching; datasets are plain tensors (the `kaun.datasets` library loads MNIST, Fashion-MNIST, CIFAR-10 that way), and stdlib `Seq` combinators replace `DataLoader` options.
