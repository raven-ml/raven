...
Kaun-next has no layer abstraction. A layer is a plain record of tensors with an `apply` function; a model is a record of layers with hand-written one-line traversals. This guide covers the built-in layers, the model-as-record pattern, and stateful layers.
...
Every `apply` is differentiable through rune-next, in both reverse and forward mode.
...
Note the `~training` flag threaded to `Dropout.apply`: mode is an argument, not a mutable model state. This is [`examples/03-mnist-cnn`](https://github.com/raven-ml/raven/tree/main/packages/kaun-next/examples/03-mnist-cnn)'s model.
...
let () =
  ...
  let step (params, stats, ostate) =
    ...
    let loss, grads, stats' =
      Rune_next.value_and_grad_aux (module Net) objective params
    in
    let params, ostate =
