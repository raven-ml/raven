...
Kaun-next has no trainer. A training step is a function you write: it composes `Rune_next.value_and_grad` with one optimizer update, and the training loop is ordinary `Seq` iteration over minibatches. This guide covers each ingredient — the step, optimizers, losses, data, and metrics — and how they compose.

## The Layering

Kaun-next's library depends only on nx and rune-next. Optimizers come from [vega](/docs/vega/), an independent package that also sits directly on nx: you add `vega` to your own project's dependencies and compose it in your code. The three libraries meet in the one record type you define — all of them operate on any `Nx.Ptree.S` structure, so there is no adapter layer and nothing to configure.

## Anatomy of a Training Step

Every training step in kaun-next has the same three lines: a loss closing over the batch, a `value_and_grad`, an optimizer update.
...
A loss maps predictions and targets to a scalar tensor — the shape `Rune_next.grad` requires of an objective. Classification losses take raw logits and evaluate in log space, so they are finite and accurate at any logit magnitude:
...
The `kaun-next.datasets` library provides MNIST, Fashion-MNIST, and CIFAR-10 loaders returning plain tensors ready for `Data.batches2`.
...
- `Batch_norm.apply p stats ~training` — training normalizes with batch statistics and returns updated running statistics; eval uses the running statistics. Thread the statistics through the loop with `Rune_next.value_and_grad_aux` — see [Layers and Models](02-layers-and-models/) for the full pattern.

A model forward that takes `~training` serves both phases; evaluation is the same function with `~training:false`.

## Going Further

Because the step is an ordinary function of ordinary values, rune-next's other transformations apply directly: per-sample gradients are `Rune_next.vmap2` of `Rune_next.grad` over the batch (see the [rune-next transformations guide](/docs/rune-next/transformations/)), and `Rune_next.value_and_grad_aux` threads any auxiliary output — predictions for logging, updated statistics — out of the objective.

For complete programs, see [`examples/02-mnist`](https://github.com/raven-ml/raven/tree/main/packages/kaun-next/examples/02-mnist) (MLP, AdamW, accuracy evaluation) and [`examples/03-mnist-cnn`](https://github.com/raven-ml/raven/tree/main/packages/kaun-next/examples/03-mnist-cnn) (CNN, dropout, checkpointing).
