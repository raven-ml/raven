...
Each layer module ships its own `names` (`Linear.names` is `["w"; "b"]`, or `["w"]` without a bias), so a model's `names` is the same one-liner shape as its traversals. For structures only known at runtime, `Checkpoint.Ptree` names dynamic `Rune_next.Ptree.t` trees by their path from the root.
...
let () =
  ...
  let path = Filename.temp_file "kaun-next-doc" ".safetensors" in
...
## Foreign Checkpoints: kaun-next.hf

The `kaun-next.hf` library fetches files from [HuggingFace Hub](https://huggingface.co) repositories into a local cache and loads safetensors checkpoints — single-file or sharded — as `Checkpoint.t` values. Downloading shells out to `curl` (it must be on `PATH`); fetched files are cached, so only the first access touches the network.

<!-- $MDX skip -->
```ocaml
let ckpt = Kaun_next_hf.load_checkpoint "gpt2" in
List.iter print_endline (Kaun_next.Checkpoint.names ckpt)
...
[`examples/04-gpt2`](https://github.com/raven-ml/raven/tree/main/packages/kaun-next/examples/04-gpt2) runs the whole pipeline: it defines GPT-2 as a record of kaun-next layers (~150 lines), loads the real weights, and generates text. The adaptation is instructive because the HF checkpoint differs from the natural kaun-next model in two ways:
...
The `~like` template is a zero-initialized model built from the downloaded `config.json` (`Kaun_next_hf.load_config` fetches and parses it). There is no per-architecture loader in the library: the adaptation is ~40 lines of user code, and the same three combinators cover other exports.

## Next Steps

- [PyTorch Comparison](05-pytorch-comparison/) — `state_dict`, `torch.save`, and `from_pretrained` in kaun-next terms
