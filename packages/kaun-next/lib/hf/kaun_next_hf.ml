...
let of_entries ~op entries =
  ...
  List.iter
    (fun (name, _) ->
      if name = "" then invalid_argf "Kaun_next_hf.%s: empty entry name" op;
      if Hashtbl.mem seen name then
        invalid_argf "Kaun_next_hf.%s: duplicate name %S" op name;
      Hashtbl.add seen name ())
    entries;
  Checkpoint.concat
    (List.map
       (fun (name, Rune_next.Ptree.P x) -> Checkpoint.of_tensor name x)
       entries)
...
let transpose name t =
  ...
  | None -> invalid_argf "Kaun_next_hf.transpose: no entry named %S" name
  | Some (Rune_next.Ptree.P x) ->
      let nd = Array.length (Nx.shape x) in
      if nd < 2 then
        invalid_argf
          "Kaun_next_hf.transpose: entry %S has %d axes, needs at least 2" name
          nd;
      let x = Nx.swapaxes (nd - 2) (nd - 1) x in
      of_entries ~op:"transpose"
        (List.map
           (fun (n, e) -> if n = name then (n, Rune_next.Ptree.P x) else (n, e))
           (entries t))

let split ?(axis = -1) name ~into t =
  match Checkpoint.find name t with
  | None -> invalid_argf "Kaun_next_hf.split: no entry named %S" name
  | Some (Rune_next.Ptree.P x) ->
      let parts = List.length into in
      if parts = 0 then invalid_arg "Kaun_next_hf.split: empty name list";
      let shape = Nx.shape x in
      let nd = Array.length shape in
      let axis = if axis < 0 then axis + nd else axis in
      if axis < 0 || axis >= nd then
        invalid_argf "Kaun_next_hf.split: axis out of bounds for entry %S" name;
      if shape.(axis) mod parts <> 0 then
        invalid_argf
          "Kaun_next_hf.split: axis %d of entry %S has size %d, not a multiple \
           of %d"
          axis name shape.(axis) parts;
      let sections =
        List.map2
          (fun n x -> (n, Rune_next.Ptree.P x))
          into (Nx.split ~axis parts x)
      in
