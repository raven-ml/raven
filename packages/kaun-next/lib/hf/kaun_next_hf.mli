...
(** HuggingFace Hub integration.
    ...
    sharded — as {!Kaun_next.Checkpoint.t} values. Since Hub checkpoints name
    and lay out tensors by the exporting framework's conventions, {!rename},
    {!transpose} and {!split} adapt them to a model's own scheme; typed
    parameters then come out through {!Kaun_next.Checkpoint.to_params}:
...
(** {1:adapting Adapting foreign checkpoints}
    ...
    {!Kaun_next.Checkpoint.to_params} ignores entries its template does not
    name. *)

val rename :
  (string -> string) -> Kaun_next.Checkpoint.t -> Kaun_next.Checkpoint.t
...
val transpose : string -> Kaun_next.Checkpoint.t -> Kaun_next.Checkpoint.t
