...
(* Single-tensor Ptree.S instance for Rune_next.check_grads. *)
...
(* Gradients: analytic derivatives via Rune_next.grad' on float64. *)

let grad_of f x = Rune_next.grad' (fun x -> Nx.sum (f x)) x
  ...
  List.map
    (fun (name, objective) ->
      test (name ^ " gradient matches finite differences") (fun () ->
          match Rune_next.check_grads (module Single) objective (x ()) with
          | Ok () -> ()
          | Error msg -> fail msg))
    [
      ("relu", sum Fn.relu);
