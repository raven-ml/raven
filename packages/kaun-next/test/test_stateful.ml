...
let test_bn_grads_flow_to_params () =
  ...
  let f p =
    ...
    Nx.sum (Nx.mul y y)
  in
  (match Rune_next.check_grads ~tol:0.05 (module Batch_norm) f params with
  | Ok () -> ()
  | Error msg -> failf "gradient check failed: %s" msg);
  let grads = Rune_next.grad (module Batch_norm) f params in
...
let () = run "kaun-next stateful" tests
