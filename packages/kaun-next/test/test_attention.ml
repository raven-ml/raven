...
let test_core_gradients () =
  ...
  let loss { Qkv64.q; k; v } =
    ...
    Nx.sum (Nx.mul y y)
  in
  grads_ok (Rune_next.check_grads (module Qkv64) loss p)
...
let test_gradients () =
  ...
  let loss ?causal p =
    ...
    Nx.sum (Nx.mul y y)
  in
  grads_ok (Rune_next.check_grads (module Attention64) (loss ?causal:None) p);
  grads_ok (Rune_next.check_grads (module Attention64) (loss ~causal:true) p)
