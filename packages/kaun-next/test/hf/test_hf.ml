...
let entry name ckpt =
  match Checkpoint.get name ckpt with Rune_next.Ptree.P x -> Nx.cast f32 x
...
let with_cache_dir f =
  let dir = Filename.temp_dir "kaun_next_hf" "" in
  Fun.protect
...
let test_rename_collision () =
  raises_invalid_arg "Kaun_next_hf.rename: duplicate name \"b\"" (fun () ->
      Hf.rename (fun _ -> "b") (two_entries ()))

let test_rename_empty () =
  raises_invalid_arg "Kaun_next_hf.rename: empty entry name" (fun () ->
...
let test_transpose_errors () =
  raises_invalid_arg "Kaun_next_hf.transpose: no entry named \"w\"" (fun () ->
      Hf.transpose "w" Checkpoint.empty);
  raises_invalid_arg
    "Kaun_next_hf.transpose: entry \"v\" has 1 axes, needs at least 2"
    (fun () -> Hf.transpose "v" (Checkpoint.of_tensor "v" (vec [| 1.0 |])))
...
let test_split_errors () =
  raises_invalid_arg "Kaun_next_hf.split: no entry named \"x\"" (fun () ->
      Hf.split "x" ~into:[ "a" ] Checkpoint.empty);
  let ckpt = Checkpoint.of_tensor "x" (vec [| 1.0; 2.0; 3.0 |]) in
  raises_invalid_arg "Kaun_next_hf.split: empty name list" (fun () ->
      Hf.split "x" ~into:[] ckpt);
  raises_invalid_arg
    "Kaun_next_hf.split: axis 0 of entry \"x\" has size 3, not a multiple of 2"
    (fun () -> Hf.split "x" ~into:[ "a"; "b" ] ckpt);
  raises_invalid_arg "Kaun_next_hf.split: axis out of bounds for entry \"x\""
    (fun () -> Hf.split ~axis:1 "x" ~into:[ "a" ] ckpt);
  let two = two_entries () in
  raises_invalid_arg "Kaun_next_hf.split: duplicate name \"b\"" (fun () ->
...
let test_remap_into_attention () =
  let module Attention = Kaun_next.Attention in
