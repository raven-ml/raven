...
(* JIT compilation via effect handler.
   ...
   Intercepts Nx tensor operations to build a computation graph, then delegates
...
(* Physical identity ([==]) tracks tensor values through the effect handler —
   structural equality would collide on placeholder tensors. *)
...
let shape_node dims =
  let int_ n =
    T.const (Tolk_ir.Const.int Tolk_ir.Dtype.Val.index n) Tolk_ir.Dtype.index
  in
  match dims with
  | [] ->
...
let read_scalar_const (type a b) (t : (a, b) Nx_effect.t) (dt : Tolk_ir.Dtype.t)
  ...
  if Tolk_ir.Dtype.is_float dt then
    let v =
      if nbytes = 4 then Int32.float_of_bits (Bytes.get_int32_le buf 0)
      else Int64.float_of_bits (Bytes.get_int64_le buf 0)
    in
    ...
    let v =
      if nbytes <= 4 then Int32.to_int (Bytes.get_int32_le buf 0)
      else Int64.to_int (Bytes.get_int64_le buf 0)
    in
...
let trace (type a b c d) ?(device : Tolk.Device.t option)
  ...
  (* The Tiny_jit is created lazily on the second call (capture phase), because
     warmup runs eagerly and doesn't need a device. *)
  ...
  let ensure_tjit () =
    ...
    | None ->
        ...
        let fxn (input_bufs : B.t array) _var_vals : unit -> (c, d) Nx.t =
          if Tolk.Jit.is_capturing () then begin
            (* Capture: build tensor graph under effect handler, schedule, and
               register the linear. *)
            let x =
              make_placeholder
                ...
                !input_shape
            in
            let graph, ctx, result = capture_graph ~device_name f x in
            output_shape := infer_shape result;
            output_nx_dtype := Some (Obj.repr (Nx_effect.dtype result));
            buffers_ref := make_buffers_cb ctx dev input_bufs.(0);
            let linear =
              match
                Tolk.Schedule.lower_sink_to_linear
                  ~get_kernel_graph:Tolk.Rangeify.get_kernel_graph graph
              with
              | Some l -> l
            ...
            let out_dt : (c, d) Nx.dtype =
              Obj.obj (Option.get !output_nx_dtype)
            in
  ...
  fun (x : (a, b) Nx.t) ->
    if not !warmup_done then begin
      ...
      let thunk =
        Tolk.Jit.call tjit [| buf |] [] ~buffers:(fun node -> !buffers_ref node)
      in
      thunk ()
    end
