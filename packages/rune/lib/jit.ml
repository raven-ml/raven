    ...
    U.custom_function ~name:"loop"
      ~srcs:
        ([
           body_linear;
           cint n;
           cint (if reversed then 1 else 0);
...
(* The forward scan: trace the body once, compile it as a sub-program, and emit
   the loop call. The carry is a structure, so every tensor leaf has its own
   double-buffered carry pair, final-carry buffer, and stack slot; leaves pair
   with their buffers by traversal position (one [P.map]/[P.iter] instance
   visits leaves in a fixed order). The carry stack is written every iteration —
   the backward loop reads it — and carrying it always keeps the forward loop's
   shape independent of whether a backward pass exists.

   Forward loop argument layout: 0 = xs, then per leaf the carry init (src),
   then per leaf carry0, then per leaf carry1, then ys, then per leaf the final
...
and stage_scan : type r.
 ...
 fun st req k ->
  ...
  (* Per-leaf carry slots, paired by identity: the slot placeholder is bound to
     ...
     callback's side effects run in evaluation order (right-to-left for
     constructor arguments), which need not match [C.iter]'s traversal order,
     and the per-leaf lists below must all follow the latter. *)
  let slot_ins = Tbl.create 4 in
  let slot_c =
    C.map
      (fun (type a b) (leaf : (a, b) Nx_effect.t) ->
        ...
        Tbl.replace slot_ins (Obj.repr slot) (tolk_dtype cdt, c_shape, c_in);
        slot)
      c
  in
  ...
  C.iter
    (fun (type a b) (leaf : (a, b) Nx_effect.t) ->
      let tdt, c_shape, c_in = Tbl.find slot_ins (Obj.repr leaf) in
      let c_out = make_node st tdt (numel c_shape) in
  ...
  let trace_body () =
    Effect.Deep.match_with
      (fun () -> step.run (Scan.Packed_c (cmod, slot_c)) (Scan.Packed_t slot_x))
      () (handler st)
  in
  ...
  if not !stable then Effect.Deep.discontinue k Scan.Not_staged
    ...
    let infos =
      List.map2 (fun info node -> (info, node)) slot_infos c_next_nodes
    in
    let n_leaves = List.length infos in
    let pos_src i = 1 + i in
    let pos_carry0 i = 1 + n_leaves + i in
    let pos_carry1 i = 1 + (2 * n_leaves) + i in
    let pos_ys = 1 + (3 * n_leaves) in
    let pos_final i = 2 + (3 * n_leaves) + i in
    let pos_stack i = 2 + (4 * n_leaves) + i in
    ...
    (* The loop's outer buffers and the per-leaf carry stacks, registered by the
       scan's identity so the backward loop can read them. *)
    ...
    let args_nodes =
      [ xs_node ] @ src_args
      @ List.map (fun (c0, _, _, _) -> c0) leaf_bufs
      @ List.map (fun (_, c1, _, _) -> c1) leaf_bufs
      @ [ ys_buf ]
    ...
    in
    (* Register the carry stacks as outputs of the forward loop, wrapped in
       their AFTERs: the backward loop reads them, and only a graph-visible
       dependency keeps the forward loop reachable (and so scheduled) when the
       scan's declared outputs are dead — e.g. under [grad], which discards the
       loss value. *)
    Tbl.replace st.scan_stacks (Obj.repr step)
      (List.map
         (fun (_, _, _, stack) ->
           U.after ~src:stack ~deps:[ U.store ~dst:stack ~value:loop_call () ])
         leaf_bufs);
...
and stage_scan_bwd : type r.
 ...
 fun st bwd k ->
  ...
  in
  let module C = (val cmod) in
  (* The two packs bind distinct existential instances; both are this module's
     [t]. *)
  ...
  let numel_x = numel x_shape in
  let numel_y = numel y_shape in
  (* The backward body's slots: the step's carry and element, the incoming carry
     cotangent (a structure), and the output cotangent. The per-leaf nodes are
     recovered by identity through [C.iter] passes: a [C.map] callback's side
     effects run in evaluation order (right-to-left for constructor arguments),
     which need not match [C.iter]'s traversal order. *)
  let slot_ins = Tbl.create 4 in
  let slot_c =
    C.map
      (fun (type a b) (leaf : (a, b) Nx_effect.t) ->
  ...
  C.iter
    (fun (type a b) (leaf : (a, b) Nx_effect.t) ->
      slot_infos := Tbl.find slot_ins (Obj.repr leaf) :: !slot_infos)
    slot_c;
  let slot_infos = List.rev !slot_infos in
  let dc_ins = Tbl.create 4 in
  let slot_dc =
    C.map
      (fun (type a b) (leaf : (a, b) Nx_effect.t) ->
        let cdt = Nx_effect.dtype leaf in
        ...
        Tbl.replace st.traced (Obj.repr slot) ();
        Tbl.replace dc_ins (Obj.repr slot) dc_in;
        slot)
      c0
  in
  let dc_infos = ref [] in
  ...
  let pos_dc0 i = 2 + n_leaves + i in
  let pos_dc1 i = 2 + (2 * n_leaves) + i in
  ...
  in
  let loop_call =
    build_loop_call ~body_linear ~reversed:true ~n ~in_slots ~out_slots ~copies
      ~stacks:[] ~args:args_nodes
  in
  let after_dcs =
    List.map2
  ...
  in
  (* Identity pairing through the slot leaves, as for the forward loop's final
     carry: positional pairing under [C.map] can cross same-shaped leaves. *)
...
type 'q compiled = {
  ...
  cp_outputs : (Obj.t * packed * U.t * leaf_place) list;
      (* output leaf -> its placeholder (dtype and shape), buffer node, and
         placement *)
  cp_aliases : (int * int list) list;
      (* output buffer node tag -> traversal position of an input leaf whose
...
(* Elision: writing an output over a donated input's storage.
   ...
   cache) returns fresh outputs every call, and [~donate] releases the inputs
   afterwards, so a step holds two generations of state on the device. When
   an output may safely take a donated input's buffer, it is bound to that
   buffer instead of a fresh one and the input's resident entry moves to the
   output handle: one generation, no copy, and results identical.
...
let trace_compile (type p q) ~device:dev ~zero_copy ~const_cache ?multi ?beam
  ...
  let cp_aliases =
    if multi <> None then []
      ...
      in
      List.concat_map
        (fun ((_, Packed (odt, ph), node, place), (_, _, u, _, _)) ->
          ...
          if place <> P_single || Hashtbl.mem seen otag then []
            ...
            Array.iteri
              (fun i inp ->
                match fits inp with
                | Some true -> derived := i :: !derived
                | Some false -> unread := i :: !unread
                | None -> ())
              cp_inputs;
            match List.rev !derived @ List.rev !unread with
            | [] -> []
            | candidates -> [ (otag, candidates) ]
          end)
...
let replay (type p q) ~donate (module P : Nx.Ptree.S with type t = p)
  ...
  let out_bufs : (int, Tolk.Device.Buffer.t list) Hashtbl.t =
    Hashtbl.create 8
  in
  (* Elision claims: an output takes a donated input's storage when that input
     seeded from a resident entry seeding no other leaf of this call, and no
     earlier output claimed the entry. *)
  let claims : (int, resident_entry) Hashtbl.t = Hashtbl.create 4 in
  if donate && not c.cp_zero_copy then begin
    let uses e =
      Array.fold_left
    ...
    List.iter
      (fun (otag, candidates) ->
        if not (Hashtbl.mem claims otag) then
          List.iter
            (fun i ->
              ...
              | Some e
                when (not (Hashtbl.mem claims otag))
                  ...
                  claimed := e :: !claimed;
                  reused_bytes := !reused_bytes + e.r_nbytes;
  ...
  (* Donation. The devices have synchronized, so the storage of each input
     that seeded from a resident entry is either owned by an output handle now
     (a claimed entry: its storage moved) or returned to the allocator so the
     next call's fresh outputs reuse it. The handle becomes Donated — forcing
     it now raises. *)
...
let jit (type p c d) ?device ?donate ?beam ?beam_parallel
  ...
  jit2 ?device ?donate ?beam ?beam_parallel (module P) (module Q) f

let jit' (type a b c d) ?device ?donate ?beam ?beam_parallel
  ...
  jit ?device ?donate ?beam ?beam_parallel (module L) f
