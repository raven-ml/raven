...
(** Raised when a function cannot be compiled: it read the value of a traced
    ...
    compiler does not support (FFT and the eigensolvers, complex, int4 and
    uint4 tensors, assigning into a view). QR, triangular solves, and Cholesky
    do compile: they unroll at trace time into the fixed number of steps their
    shapes imply. *)
