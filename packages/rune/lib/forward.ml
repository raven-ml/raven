...
(* Forward-mode differentiation as an effect handler over Nx operations.
   ...
   Forward_k is the batched sibling: the same rule table with every tangent a
   [k]-lane batch on a leading axis. A new Nx operation needs a rule in both
   files, in the same category, and test_jvp_k.ml's reference comparisons must
   cover it. *)
