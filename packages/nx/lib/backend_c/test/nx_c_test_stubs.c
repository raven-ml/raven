...
/* Test-only introspection stub for the binding's kind->tag pin. Given a bare
   ...
   maps its kind to, so test_nx_c can assert that value equals Nx_dtype.t's
   constructor index for every one of the 19 dtypes. Independent of the t
   record layout — a pure kind->tag check. Lives here, not in the library or an engine
