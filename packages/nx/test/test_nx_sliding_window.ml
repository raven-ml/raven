...
let check_against_reference msg ~axis ~window ~step shape data =
  ...
  in
  let windows =
    Nx.sliding_window ~axis ~window ~step (Nx.create Nx.float64 shape data)
  in
  check_t msg expected_shape expected windows

let test_basic () =
  let x = Nx.create Nx.int32 [| 5 |] [| 1l; 2l; 3l; 4l; 5l |] in
  let windows = Nx.sliding_window ~window:3 x in
...
let test_step () =
  ...
  check_t "step 2 window 2" [| 3; 2 |]
    [| 1l; 2l; 3l; 4l; 5l; 6l |]
    (Nx.sliding_window ~window:2 ~step:2 x);
...
let test_batched () =
  ...
  in
  let windows =
    Nx.sliding_window ~window:3 ~step:2
...
let test_negative_axis () =
  ...
  let negative = Nx.sliding_window ~axis:(-2) ~window:2 x in
  let positive = Nx.sliding_window ~axis:1 ~window:2 x in
...
let test_strided_input () =
  ...
  check_t "transposed input" expected_shape expected
    (Nx.sliding_window ~axis:1 ~window:2 ~step:2 xt)
...
let test_rfft_over_view () =
  ...
  let frames = Nx.sliding_window ~window:32 ~step:8 x in
...
let test_errors () =
  ...
  check_invalid_arg "window zero"
    "sliding_window: window must be >= 1, got 0" (fun () ->
      Nx.sliding_window ~window:0 x);
  check_invalid_arg "step zero" "sliding_window: step must be >= 1, got 0"
    (fun () -> Nx.sliding_window ~window:2 ~step:0 x);
  check_invalid_arg "window too large"
    "sliding_window: cannot slide window 6 along axis 0 in shape [5] (6>5)"
    (fun () -> Nx.sliding_window ~window:6 x);
  check_invalid_arg "axis out of bounds"
    "sliding_window: axis 1 out of bounds for 1D tensor" (fun () ->
      Nx.sliding_window ~axis:1 ~window:2 x);
  check_invalid_arg "scalar input"
    "sliding_window: axis -1 out of bounds for 0D tensor" (fun () ->
      Nx.sliding_window ~window:1 (Nx.scalar Nx.float32 1.0))
