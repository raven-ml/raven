...
(* Overlapping windows address a single element from several positions, so a
   write through them has no defined result; the view knows it and the frontend
   refuses the write. A view that steps at least its own width holds each
   element once and stays writable. *)
let test_overlapping_view_rejects_writes () =
  ...
  check_invalid_arg "blit into overlapping windows"
    "blit: destination is a broadcast or overlapping view, which addresses one \
     element from several positions; write into a copy instead" (fun () ->
