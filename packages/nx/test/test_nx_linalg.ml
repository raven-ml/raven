...
let test_triangular_lower () =
  let x = Nx.solve_triangular tri_l tri_b in
  check_nx ~epsilon:1e-5 "triangular lower" tri_b (Nx.matmul tri_l x)

let test_triangular_upper () =
  let x = Nx.solve_triangular ~upper:true tri_u tri_b in
...
let test_triangular_vector_rhs () =
  ...
  let x = Nx.solve_triangular tri_l b in
...
let test_triangular_singular () =
  ...
  raises ~msg:"triangular singular"
    (Nx.Linalg_error { op = "solve_triangular"; kind = `Singular })
    (fun () -> ignore (Nx.solve_triangular a b))

let test_triangular_bad_rhs () =
  check_invalid_arg "triangular rhs not matrix/vector"
    "solve_triangular: b must be a [.., n] vector or a [.., n, nrhs] matrix"
    (fun () ->
      ignore
        (Nx.solve_triangular tri_l
           (Nx.create Nx.float32 [| 1; 2; 2 |] [| 1.; 2.; 3.; 4. |])));
  check_invalid_arg "triangular rhs wrong length"
    "solve_triangular: vector right-hand side length does not match a"
    (fun () ->
      ignore
        (Nx.solve_triangular tri_l (Nx.create Nx.float32 [| 2 |] [| 1.; 2. |])));
  let a2 =
