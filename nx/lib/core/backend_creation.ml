...
module Make (B : Backend_intf.S) = struct
  (* Helper to create nx from host buffer *)
  let create_nx_from_host_buffer context descriptor host_buffer =
  ...
  let create context dtype shape ocaml_array =
    ...
    if n = 0 && Array.length shape > 0 then B.empty context dtype shape
      ...
      in
      create_nx_from_host_buffer context descriptor host_buffer
