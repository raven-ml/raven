...
(* Check if a symbolic shape can be evaluated to concrete *)
let try_eval_shape shape =
  match Symbolic_shape.eval shape with
  | Some concrete -> concrete
  | None ->
      Error.failed ~op:"view" ~what:"cannot evaluate symbolic shape"
        ~hint:"all symbolic dimensions must be bound before this operation" ()
  ...
  if n = 0 then Symbolic_shape.static 1
    ...
    let rec prod_dims i acc =
      if i >= n then acc
        ...
        prod_dims (i + 1)
          (match acc with
          | Symbolic_shape.Static a -> (
              match v.shape.(i) with
              | Symbolic_shape.Static b -> Symbolic_shape.Static (a * b)
              | Symbolic_shape.Dynamic _ as d -> d)
          | Symbolic_shape.Dynamic _ -> acc)
    in
...
let can_be_strided view =
  ...
  || Array.for_all2
       (fun (b, e) s -> b = 0 && e = s)
       (Option.get view.mask)
       (try_eval_shape view.shape)
...
let reshape view new_shape =
  ...
  if Symbolic_shape.equal view.shape new_shape then view
    ...
    | Some old_arr, Some new_arr -> (
        ...
        then create ~offset:0 new_shape (* Cannot reshape views with masks *)
          ...
          let new_strides = compute_strides new_arr in
          create ~offset:view.offset ~strides:new_strides new_shape
          (* Reshaping to/from scalar *)
        else if Symbolic_shape.rank new_shape = 0 then
          create ~offset:view.offset new_shape
        else
          (* Try to find a valid reshape strategy *)
          let old_strides = view.strides in
                ...
                Array.map
                  (fun dim ->
                    if dim = 1 then 0
                      ...
                      let stride = old_strides.(!old_idx) in
                      incr old_idx;
                      stride))
                  new_arr
              in
          ...
          let try_merge_split () =
            ...
            for i = 0 to Array.length old_arr - 1 do
              if old_arr.(i) > 1 then
                old_dims := (old_arr.(i), old_strides.(i)) :: !old_dims
            done;
                    ...
                    let rec collect_merge size stride dims needed =
                      if size = needed then Some (dims, stride)
                        ...
                        | (next_size, next_stride) :: rest ->
                            (* Check if next dimension is contiguous with
                               current *)
                            if next_stride = stride * size then
                              collect_merge (size * next_size) stride rest
                                needed
          ...
          | None -> (
              ...
              | None ->
                  Error.cannot ~op:"reshape" ~what:"reshape strided view"
                    ...
                    ~reason:"incompatible memory layout"
                    ~hint:
...
let unsafe_resize view arg new_mask_opt =
  ...
  let _ = try_eval_shape view.shape in
...
let pad view arg =
  ...
  if Array.for_all (fun (b, e) -> b = 0 && e = 0) arg then view
    ...
    let shape_arr = try_eval_shape view.shape in
    let zvarg =
      Array.mapi
        (fun i s ->
...
let merge v1 v2 =
  ...
  then Some v2 (* Case 3: v1 is contiguous and can be reshaped to v2 *)
    ...
    | _ -> None (* Case 4: Handle operations on broadcast views *)
    ...
    | Some _s2_arr ->
        ...
        Some v2
    | _ -> None
    (* Case 5: Handle expand operations - if v2 is an expand of v1 *)
    ...
    | _ -> None (* Case 6: Handle shrink views - if v2 has a mask *)
...
let simplify view =
  ...
  | Some shape_arr ->
      ...
      if mask <> view.mask then
        create view.shape ~strides:view.strides ~offset:view.offset ?mask
