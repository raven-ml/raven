...
static inline void copy_value_at(int kind, const void *src_data, long src_off,
  ...
  switch (kind) {
    ...
    case CAML_BA_FLOAT64:
      ...
      return;
    case CAML_BA_COMPLEX32:
      ((complex32 *)dst_data)[dst_off] =
          ((const complex32 *)src_data)[src_off];
      return;
    case CAML_BA_COMPLEX64:
      ((complex64 *)dst_data)[dst_off] =
          ((const complex64 *)src_data)[src_off];
      return;
    case NX_BA_BFLOAT16:
...
static inline long compute_out_offset_arg(const ndarray_t *x, const ndarray_t *out,
                                          int axis, long outer_idx,
                                          long inner_idx, bool keepdims) {
  long off = out->offset;
  long tmp_outer = outer_idx;
  for (int d = axis - 1; d >= 0; --d) {
...
static void arg_reduce_impl(const ndarray_t *x, ndarray_t *out, int kind,
  ...
  if (fast_path) {
    ...
    _Pragma("omp parallel for if(groups >= NX_ARG_PAR_THRESHOLD)")
  ...
  for (long outer_idx = 0; outer_idx < outer; ++outer_idx) {
    for (long inner_idx = 0; inner_idx < inner; ++inner_idx) {
      ...
      long out_off = compute_out_offset_arg(x, out, axis, outer_idx, inner_idx,
                                            keepdims);
...
static int sort_impl(const ndarray_t *x, ndarray_t *out, int kind, int axis,
  ...
  _Pragma("omp parallel for if(groups >= NX_SORT_PAR_THRESHOLD)")
  for (long g = 0; g < groups; ++g) {
    int tid = 0;
#if defined(_OPENMP)
...
static const char *validate_axis(const ndarray_t *x, int axis,
                                 const char *op) {
...
CAMLprim value caml_nx_argmax(value v_x, value v_out, value v_axis,
  ...
  int out_kind = nx_ba_get_kind(Caml_ba_array_val(Field(v_out, FFI_TENSOR_DATA)));
