...
let data : type a b. (a, b) t -> (a, b, Bigarray.c_layout) Bigarray.Array1.t =
 fun t ->
  (* For compatibility with the frontend's unsafe_get, we need to return
     a bigarray that preserves the view's offset. This means returning
     the entire buffer, not just the viewed portion. *)
...
let op_matmul a b =
  Ops_matmul.op_matmul a.context a b
