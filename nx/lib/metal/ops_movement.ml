...
let pad ctx t out padding fill_value =
  ...
  Internal.with_command_buffer ctx (fun cmd_buffer ->
      ...
      let in_strides = 
        ...
        | Some s -> s
        | None -> Error.failed ~op:"make_contiguous" ~what:"cannot get strides for non-contiguous view" () in
      ...
      let in_offset_val =
        let offset_val = 
          ...
          | Symbolic_shape.Static n -> n
          | Symbolic_shape.Dynamic _ -> Error.failed ~op:"make_contiguous" ~what:"cannot evaluate symbolic offset" () in
        ...
        (fun t ->
          ...
          Internal.with_command_buffer ctx (fun cmd_buffer ->
              ...
              for i = 0 to ndim - 1 do
                ...
                Ctypes.(
                  in_shape_arr +@ i <-@ Unsigned.UInt32.of_int in_shape.(i))
              done;
