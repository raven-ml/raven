...
let set_view_params encoder view ~shape_index ~strides_index ~offset_index
  ...
  let shape = 
    ...
    | Some arr -> arr
    | None -> Error.failed ~op:"set_view_params" ~what:"cannot evaluate symbolic shape" ()
  in
  let strides = 
    match Lazy_view.strides view with
    | Some s -> s
    | None -> Error.failed ~op:"set_view_params" ~what:"cannot get strides for non-contiguous view" ()
  in
  let offset = 
    match Lazy_view.offset view with
    | Symbolic_shape.Static n -> n
    | Symbolic_shape.Dynamic _ -> Error.failed ~op:"set_view_params" ~what:"cannot evaluate symbolic offset" () in
...
let dispatch_unary_op (ctx : Internal.context) (op_name : string)
  ...
  let numel = 
    ...
    | Symbolic_shape.Static n -> n
    | Symbolic_shape.Dynamic _ -> Error.failed ~op:"dispatch_unary" ~what:"cannot evaluate symbolic numel" () in
