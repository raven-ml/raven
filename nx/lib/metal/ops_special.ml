...
let linear_index view indices =
  ...
  if Array.length indices <> ndim then
    Error.invalid ~op:"linear_index" ~what:"indices"
      ~reason:(Printf.sprintf "rank mismatch: %d vs %d" (Array.length indices) ndim) ();
  let strides = get_strides view in
  let offset = get_offset view in
  let physical_offset = ref offset in
  Array.iteri
    (fun i idx ->
      physical_offset := !physical_offset + (idx * strides.(i)))
    indices;
  !physical_offset
...
let assign ctx dst src =
  ...
  if
    not
      (is_c_contiguous dst.Internal.view
      && get_offset dst.Internal.view = 0)
  then
