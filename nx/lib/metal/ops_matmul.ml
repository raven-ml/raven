...
let op_matmul ctx a b =
  ...
  if k_a <> k_b then
    invalid_arg (Printf.sprintf "dot: cannot contract %s (last axis: %d) to %s (axis %d: %d) (size %d≠%d)"
  ...
  Internal.with_command_buffer ctx (fun cmd_buffer ->
      ...
      (* Set dimensions *)
      let ndim_out_val = Ctypes.(allocate uint32_t (Unsigned.UInt32.of_int ndim_out)) in
      let ndim_a_val = Ctypes.(allocate uint32_t (Unsigned.UInt32.of_int ndim_a)) in
      let ndim_b_val = Ctypes.(allocate uint32_t (Unsigned.UInt32.of_int ndim_b)) in
      ...
      let group_size =
        { Metal.Size.width = min 8 n; height = min 8 m; depth = 1 }
      in
      
      ComputeCommandEncoder.dispatch_threadgroups encoder
        ~threadgroups_per_grid:
          { Metal.Size.width = (n + group_size.width - 1) / group_size.width;
            height = (m + group_size.height - 1) / group_size.height;
            depth = !batch_size }
