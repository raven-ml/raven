  ...
  let t_padded =
    ...
    then t
      ...
      let old_shape = 
        ...
        | Some arr -> arr
        | None -> Error.failed ~op:"ensure_3d_for_conv" ~what:"cannot evaluate symbolic shape" () in
      let new_shape =
        Array.mapi
          (fun i dim ->
      ...
      Internal.with_command_buffer ctx (fun cmd_buffer ->
          ...
          let in_strides = 
            ...
            | Some s -> s
            | None -> Error.failed ~op:"unfold" ~what:"cannot get strides for non-contiguous view" () in
          ...
          let in_offset = 
            ...
            | Symbolic_shape.Dynamic _ -> Error.failed ~op:"unfold" ~what:"cannot evaluate symbolic offset" () in
          let in_offset_val =
