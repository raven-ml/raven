...
let op_unfold ctx t ~kernel_size ~stride ~dilation ~padding =
  ...
  let output_spatial_dims = Array.init n_spatial (fun i ->
  ...
  Internal.with_command_buffer ctx (fun cmd_buffer ->
      ...
      let params = Ctypes.(allocate_n uint32_t ~count:(6 + 6*n_spatial + 2)) in
      ...
      (* Kernel sizes *)
      for i = 0 to n_spatial - 1 do
        Ctypes.(params +@ (3 + n_spatial + i) <-@ Unsigned.UInt32.of_int kernel_size.(i))
      done;
      
      (* Strides *)
      for i = 0 to n_spatial - 1 do
        Ctypes.(params +@ (3 + 2*n_spatial + i) <-@ Unsigned.UInt32.of_int stride.(i))
      done;
      
      (* Dilations *)
      for i = 0 to n_spatial - 1 do
        Ctypes.(params +@ (3 + 3*n_spatial + i) <-@ Unsigned.UInt32.of_int dilation.(i))
      done;
      
      (* Padding *)
      for i = 0 to n_spatial - 1 do
        Ctypes.(params +@ (3 + 4*n_spatial + i) <-@ Unsigned.UInt32.of_int (fst padding.(i)))
      done;
      
      (* Output spatial dimensions *)
      for i = 0 to n_spatial - 1 do
        Ctypes.(params +@ (3 + 5*n_spatial + i) <-@ Unsigned.UInt32.of_int output_spatial_dims.(i))
      done;
      
      Ctypes.(params +@ (3 + 6*n_spatial) <-@ Unsigned.UInt32.of_int num_blocks);
      Ctypes.(params +@ (3 + 6*n_spatial + 1) <-@ Unsigned.UInt32.of_int kernel_elements);
      
      ComputeCommandEncoder.set_bytes encoder
        ~bytes:Ctypes.(to_voidp params)
        ~length:((6 + 6*n_spatial + 2) * 4) ~index:2;
      
      (* Dispatch kernel *)
      let total_threads = batch_size * num_blocks * channels * kernel_elements in
      let threads_per_group, num_groups =
...
let op_fold ctx t ~output_size ~kernel_size ~stride ~dilation ~padding =
  ...
  if Array.length t_shape <> 3 then
    invalid_arg "op_fold: input must be 3D (batch, channels * kernel_elements, blocks)";
  ...
  if Array.for_all2 (fun p q -> p = q) padding (Array.make n_spatial (0, 0)) then
    out_padded
  else
    let bounds = Array.concat [
      [| (0, batch_size); (0, channels) |];
      Array.mapi (fun i _ -> (fst padding.(i), padded_size.(i) - snd padding.(i))) output_size
    ] in
