...
kernel void pad_float(device float* out [[buffer(0)]],
                     ...
                     constant int& in_offset [[buffer(8)]],
                     uint gid [[thread_position_in_grid]]) {
    ...
    if (in_bounds) {
        ...
        if (ndim > 0) in_idx += int(in_pos.x) * in_strides[0];
        if (ndim > 1) in_idx += int(in_pos.y) * in_strides[1];
        if (ndim > 2) in_idx += int(in_pos.z) * in_strides[2];
