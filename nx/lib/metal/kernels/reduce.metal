...
inline uint compute_strided_index(uint reduction_id, uint element_in_reduction,
                                 ...
                                 constant int* strides, int offset,
    ...
    // Convert multi-dimensional index to strided index
    int linear_idx = offset;
    for (uint i = 0; i < ndim; i++) {
        linear_idx += int(out_idx[i]) * strides[i];
    }
...
kernel void reduce_sum_float(device float* out [[buffer(0)]],
                            ...
                            constant int* strides [[buffer(9)]],
                            constant int& offset [[buffer(10)]],
    ...
    for (uint i = tid; i < reduction_size; i += REDUCE_THREADS) {
        uint idx = compute_strided_index(reduction_id, i, shape, axes, strides, offset, ndim, naxes);
        if (idx < in_size) {
            sum += in[idx];
        }
    }
