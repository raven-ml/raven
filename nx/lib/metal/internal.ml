 ...
 fun t ba ->
  ...
  | Bigarray.Complex64 ->
      ...
      let view_size = 
        ...
        | Symbolic_shape.Static n -> n
        | Symbolic_shape.Dynamic _ -> Error.failed ~op:"copy_from_metal_buffer" ~what:"cannot copy with symbolic size" ()
      in
      let offset = 
        match Lazy_view.offset view with
        | Symbolic_shape.Static n -> n
        | Symbolic_shape.Dynamic _ -> Error.failed ~op:"copy_from_metal_buffer" ~what:"cannot copy with symbolic offset" ()
      in
      ...
      if Lazy_view.is_contiguous view then
        ...
        let shape = 
          ...
          | Some arr -> arr
          | None -> Error.failed ~op:"copy_from_metal_buffer" ~what:"cannot copy with symbolic shape" ()
        in
        let strides = 
          match Lazy_view.strides view with
          | Some s -> s
          | None -> Error.failed ~op:"copy_from_metal_buffer" ~what:"cannot copy non-contiguous symbolic view" ()
        in
