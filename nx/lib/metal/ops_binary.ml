...
let dispatch_binary ctx op_name a b =
  ...
  Internal.with_command_buffer ctx (fun cmd_buffer ->
      ...
      (* Pass view offsets *)
      let a_offset_val = Ctypes.(allocate int32_t (Int32.of_int (View.offset a.Internal.view))) in
      ComputeCommandEncoder.set_bytes encoder
        ~bytes:Ctypes.(to_voidp a_offset_val)
        ~length:4 ~index:7;
        
      let b_offset_val = Ctypes.(allocate int32_t (Int32.of_int (View.offset b.Internal.view))) in
      ComputeCommandEncoder.set_bytes encoder
