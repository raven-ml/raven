...
let make_tensor x shape =
  ...
  create x.context x.dtype (make_buffer x.dtype numel) (Lazy_view.create (Symbolic_shape.of_ints shape))

let op_buffer ctx dtype size =
  create ctx dtype (make_buffer dtype size) (Lazy_view.create (Symbolic_shape.of_ints [| size |]))
...
let get_shape view =
  ...
  | Some arr -> arr
  | None -> Error.failed ~op:"get_shape" ~what:"cannot evaluate symbolic shape" ()

(* Helper to get strides from view *)
let get_strides view =
  match Lazy_view.strides view with
  | Some s -> s
  | None -> Error.failed ~op:"get_strides" ~what:"cannot get strides for non-contiguous view" ()

(* Helper to get offset from view *)
let get_offset view =
  match Lazy_view.offset view with
  | Symbolic_shape.Static n -> n
  | Symbolic_shape.Dynamic _ -> Error.failed ~op:"get_offset" ~what:"cannot evaluate symbolic offset" ()

(* Helper to get numel from view *)
let get_numel view =
  match Lazy_view.numel view with
  | Symbolic_shape.Static n -> n
  | Symbolic_shape.Dynamic _ -> Error.failed ~op:"get_numel" ~what:"cannot evaluate symbolic numel" ()
...
let op_assign dst src =
  ...
  assign (Lazy_view.ndim src.view) src_shape src.buffer
    (get_strides src.view) (get_offset src.view) dst.buffer
    (get_strides dst.view) (get_offset dst.view);
...
let op_where cond x y =
  ...
  where (Lazy_view.ndim x.view) x_shape cond.buffer
    (get_strides cond.view) (get_offset cond.view) x.buffer
    (get_strides x.view) (get_offset x.view) y.buffer (get_strides y.view)
    (get_offset y.view) result.buffer (get_strides result.view)
    (get_offset result.view);
...
let op_gather data indices axis =
  ...
  gather data_ndim data_shape data.buffer (get_strides data.view)
    (get_offset data.view) indices.buffer
    (get_strides indices.view)
...
let op_scatter ?(mode = `Set) ?(unique_indices = false) data_template indices
  ...
  scatter template_ndim template_shape indices_ndim indices_shape
    ...
    indices.buffer
    (get_strides indices.view)
    (get_offset indices.view) updates.buffer
    (get_strides updates.view)
    (get_offset updates.view) axis result.buffer (get_strides result.view)
    (get_offset result.view) computation_mode;
...
let op_matmul a b =
  ...
  matmul a.buffer a_shape (get_strides a.view) (get_offset a.view)
    b.buffer b_shape (get_strides b.view) (get_offset b.view)
    result.buffer output_shape (get_strides result.view)
    (get_offset result.view);
...
let op_unfold x ~kernel_size ~stride ~dilation ~padding =
  ...
  unfold (Lazy_view.ndim x.view) x_shape x.buffer (get_strides x.view)
    (get_offset x.view) (Lazy_view.ndim result.view) output_shape
    result.buffer (get_strides result.view) (get_offset result.view)
...
let op_fold x ~output_size ~kernel_size ~stride ~dilation ~padding =
  ...
  fold (Lazy_view.ndim x.view) x_shape x.buffer (get_strides x.view)
    (get_offset x.view) (Lazy_view.ndim result.view) full_output_shape
    result.buffer (get_strides result.view) (get_offset result.view)
    output_spatial_shape kernel_size stride padding_lower dilation;
  let _ = x.view in
  (* FIX: Keep input tensor alive during C call. *)
