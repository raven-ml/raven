 ...
 fun t_src ->
  ...
  (if total_elements > 0 then
     ...
     | Some strides ->
         ...
         let src_buffer = buffer t_src in
         let buffer_size = Array1.dim src_buffer in

         iterate_view_indices src_shape indices (fun idx ->
             let src_idx = src_offset + Shape.ravel_index idx strides in
             if src_idx < 0 || src_idx >= buffer_size then
               Error.invalid ~op:"copy" ~what:"index out of bounds"
                 ~reason:
...
let fill : type a b. a -> (a, b) t -> unit =
 fun value t_fill ->
  ...
  if total_elements = 0 then () (* No elements to fill *)
    ...
    if n_dims = 0 then
      (* Scalar case *)
      Array1.set fill_buffer (offset t_fill) value
    else
