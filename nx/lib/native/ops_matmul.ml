...
let kernel_matmul_float32 context (a : (float, float32_elt) t) 
  ...
  let get_batch_offset batch_idx shape ndim stride_batch =
    ...
    for i = batch_dims - 1 downto 0 do
      ...
      if actual_dim > 1 then
        idx := !idx + batch_coord.(i) * !stride;
  ...
  if !batch_size > 1 then
    ...
    Parallel.parallel_for context.pool 0 (!batch_size - 1) (fun start_batch end_batch ->
      for batch = start_batch to end_batch do
        let a_batch_offset = get_batch_offset batch shape_a ndim_a stride_a_batch in
        let b_batch_offset = get_batch_offset batch shape_b ndim_b stride_b_batch in
        ...
        for i = 0 to m - 1 do
          for j = 0 to n - 1 do
            ...
            for l = 0 to k - 1 do
              ...
              sum := !sum +. Array1.unsafe_get a_buf a_idx *. Array1.unsafe_get b_buf b_idx
            done;
