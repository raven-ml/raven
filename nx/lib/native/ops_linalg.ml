...
let[@inline] get_2d t i j =
  let strides = 
    ...
    | Some s -> s
    | None -> Error.failed ~op:"get_2d" ~what:"cannot get strides for non-contiguous view" ()
  in
  ...
  let offset = 
    ...
    | Symbolic_shape.Dynamic _ -> Error.failed ~op:"get_2d" ~what:"cannot evaluate symbolic offset" () in
...
let[@inline] set_2d t i j value =
  let strides = 
    match Lazy_view.strides t.view with
    | Some s -> s
    | None -> Error.failed ~op:"set_2d" ~what:"cannot get strides for non-contiguous view" ()
  in
  let stride0 = strides.(0) in
  let stride1 = strides.(1) in
  let offset = 
    match Lazy_view.offset t.view with
    | Symbolic_shape.Static n -> n
    | Symbolic_shape.Dynamic _ -> Error.failed ~op:"ops_linalg" ~what:"cannot evaluate symbolic offset" () in
...
let[@inline] get_3d t b i j =
  let strides = 
    match Lazy_view.strides t.view with
    | Some s -> s
    | None -> Error.failed ~op:"ops_linalg" ~what:"cannot get strides for non-contiguous view" ()
  in
  let stride0 = strides.(0) in
  let stride1 = strides.(1) in
...
let get_matrix_dims t =
  let shape = 
    ...
    | Some arr -> arr
    | None -> Error.failed ~op:"ops_linalg" ~what:"cannot evaluate symbolic shape" ()
  in
...
let triangular_solve (type a b) ~upper ~transpose ~unit_diag ctx (a : (a, b) t)
  ...
  let b_expanded =
    if b_is_1d then
      ...
      { b with view = Lazy_view.reshape (Symbolic_shape.of_ints new_shape) b.view }
    else b
  in
  ...
  if b_is_1d then
    { output with view = Lazy_view.reshape (Symbolic_shape.of_ints [| b_shape.(0) |]) output.view }
  else output
