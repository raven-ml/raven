...
let calculate_unfold_output_shape input_shape ~kernel_size ~stride ~dilation ~padding =
...
let kernel_unfold_float32 (input : (float, float32_elt) t) 
  ...
  let num_spatial_dims = Array.length kernel_size in
  let batch_dims = Array.length input_shape - num_spatial_dims - 1 in
  let batch_size = Array.fold_left ( * ) 1 (Array.sub input_shape 0 batch_dims) in
  let channels = input_shape.(batch_dims) in
        ...
        for c = 0 to channels - 1 do
          ...
          if !valid then
            ...
            let input_idx = 
              ...
              input_flat_spatial
            in
            Array1.unsafe_set out_buf out_idx (Array1.unsafe_get input_buf (offset input + input_idx))
          else
...
let kernel_fold_float32 (input : (float, float32_elt) t) 
    (out : (float, float32_elt) t)
    ~output_size ~kernel_size ~stride ~dilation ~padding
    start_block end_block =
  let input_buf, out_buf = (buffer input, buffer out) in
  ...
  let unfolded_spatial = Array.init num_spatial_dims (fun i ->
    let effective_kernel = (kernel_size.(i) - 1) * dilation.(i) + 1 in
    (padded_spatial.(i) - effective_kernel) / stride.(i) + 1
  ) in
      ...
      for kernel_idx = 0 to kernel_elements - 1 do
        ...
        for c = 0 to channels - 1 do
          ...
          if !valid then
            ...
            let output_flat_spatial = ravel_spatial output_coords output_spatial in
            let output_idx = 
            ...
            let input_val = Array1.unsafe_get input_buf (offset input + input_idx) in
...
let kernel_unfold (type a b) (context : context) (input : (a, b) t) (out : (a, b) t)
    ~kernel_size ~stride ~dilation ~padding =
  (* Calculate number of output blocks for parallelization *)
  let out_shape = shape out in
  let num_blocks = out_shape.(Array.length out_shape - 1) in (* last dimension is num_blocks *)
  
  if num_blocks > 1 then
    (* Parallelize across output blocks *)
    (* Note: parallel_for's end_idx is exclusive in the worker function *)
    Parallel.parallel_for context.pool 0 (num_blocks - 1) (fun start_block end_block ->
      match Array1.kind (buffer input) with
      | Float32 -> kernel_unfold_float32 input out ~kernel_size ~stride ~dilation ~padding start_block end_block
      | _ -> failwith "Unfold not yet implemented for this dtype")
  else
    (* Single block - run directly *)
    match Array1.kind (buffer input) with
    | Float32 -> kernel_unfold_float32 input out ~kernel_size ~stride ~dilation ~padding 0 num_blocks
    | _ -> failwith "Unfold not yet implemented for this dtype"

(* Generic unfold (im2col) operation dispatcher *)
let unfold context input ~kernel_size ~stride ~dilation ~padding =
  let output_shape = calculate_unfold_output_shape (shape input) ~kernel_size ~stride ~dilation ~padding in
...
let kernel_fold (type a b) (context : context) (input : (a, b) t) (out : (a, b) t)
    ~output_size ~kernel_size ~stride ~dilation ~padding =
  (* For fold, we need to be careful about parallelization.
     When stride=1 and kernel_size>1, multiple input blocks write to overlapping output locations.
     For now, let's try parallel blocks but be aware of potential race conditions. *)
  let input_shape = shape input in
  let num_blocks = input_shape.(Array.length input_shape - 1) in
  
  if num_blocks > 1 && false then  (* Disabled for now due to potential race conditions *)
    Parallel.parallel_for context.pool 0 (num_blocks - 1) (fun start_block end_block ->
      match Array1.kind (buffer input) with
      | Float32 -> kernel_fold_float32 input out ~output_size ~kernel_size ~stride ~dilation ~padding start_block end_block
      | _ -> failwith "Fold not yet implemented for this dtype")
  else
    (* Run sequentially *)
    match Array1.kind (buffer input) with
    | Float32 -> kernel_fold_float32 input out ~output_size ~kernel_size ~stride ~dilation ~padding 0 num_blocks
    | _ -> failwith "Fold not yet implemented for this dtype"
...
let fold context input ~output_size ~kernel_size ~stride ~dilation ~padding =
  ...
  let input_shape = shape input in
  let batch_dims = Array.length input_shape - 2 in (* -2 for channel*kernel and num_blocks dims *)
  let batch_shape = Array.sub input_shape 0 batch_dims in
  ...
  kernel_fold context input out ~output_size ~kernel_size ~stride ~dilation ~padding;
  out