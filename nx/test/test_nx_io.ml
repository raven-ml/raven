...
let test_hdf5_save_load_single () =
  if not Nx_io.hdf5_available then skip ()
  else
    let test_data = Nx.arange Nx.float32 0 100 1 |> Nx.reshape [| 10; 10 |] in
    let path = temp_file "test_h5_" ".h5" in

    (* Save the data *)
    Nx_io.save_h5 ~dataset:"test_data" path test_data;

    (* Load it back *)
    let loaded = Nx_io.load_h5 ~dataset:"test_data" path in
    let loaded_f32 = Nx_io.as_float32 loaded in

    (* Check shape and values *)
    check (array int) "loaded shape" [| 10; 10 |] (Nx.shape loaded_f32);
    check_array_approx "loaded values" test_data loaded_f32;

    (* Clean up *)
    Sys.remove path

let test_hdf5_save_load_multiple () =
  if not Nx_io.hdf5_available then skip ()
  else
    let weights = Nx.randn Nx.float32 [| 5; 3 |] in
    let bias = Nx.zeros Nx.float32 [| 3 |] in
...
let test_hdf5_different_dtypes () =
  if not Nx_io.hdf5_available then skip ()
  else
    let path = temp_file "test_h5_dtypes_" ".h5" in

    (* Test float32 *)
    let f32_data = Nx.ones Nx.float32 [| 2; 3 |] in
    Nx_io.save_h5 ~dataset:"float32" path f32_data;
    let f32_loaded = Nx_io.load_h5 ~dataset:"float32" path in
    let f32_conv = Nx_io.as_float32 f32_loaded in
    check (array int) "float32 shape" [| 2; 3 |] (Nx.shape f32_conv);

    (* Test int32 *)
    let i32_data = Nx.ones Nx.int32 [| 3; 4 |] in
    Nx_io.save_h5 ~dataset:"int32" path i32_data;
    let i32_loaded = Nx_io.load_h5 ~dataset:"int32" path in
    let i32_conv = Nx_io.as_int32 i32_loaded in
    check (array int) "int32 shape" [| 3; 4 |] (Nx.shape i32_conv);

    (* Test float64 *)
    let f64_data = Nx.ones Nx.float64 [| 1; 5 |] in
    Nx_io.save_h5 ~dataset:"float64" path f64_data;
    let f64_loaded = Nx_io.load_h5 ~dataset:"float64" path in
    let f64_conv = Nx_io.as_float64 f64_loaded in
    check (array int) "float64 shape" [| 1; 5 |] (Nx.shape f64_conv);
    ...
    [
      ...
      ( "hdf5",
        [
          ...
          test_case "Save/load single dataset" `Quick test_hdf5_save_load_single;
          test_case "Save/load multiple datasets" `Quick
            test_hdf5_save_load_multiple;
          test_case "Different dtypes" `Quick test_hdf5_different_dtypes;
        ] );
      ( "safetensors",
        [
