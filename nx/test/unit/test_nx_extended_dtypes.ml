...
module Make (Backend : Nx_core.Backend_intf.S) = struct
  ...
  let test_create_bool ctx () =
    let t = Nx.create ctx Nx_core.Dtype.bool [| 4 |] [| false; true; false; true |] in
  ...
  let test_create_float8_e4m3 ctx () =
    let t = Nx.create ctx Nx_core.Dtype.float8_e4m3 [| 3 |] [| 1.0; 2.0; 3.0 |] in
    check_t ~eps:0.1 "create float8_e4m3" [| 3 |] [| 1.0; 2.0; 3.0 |] t

  let test_scalar_float8_e4m3 ctx () =
    (* Test with a value that can be exactly represented in Float8_e4m3.
       With 3-bit mantissa, we can represent 1.000 through 1.111 in binary.
       For example: 11.0 = 1.011 × 2^3 is exactly representable. *)
  ...
  let test_create_float8_e5m2 ctx () =
    let t = Nx.create ctx Nx_core.Dtype.float8_e5m2 [| 3 |] [| 1.0; 2.0; 3.0 |] in
  ...
  let test_create_complex16 ctx () =
    let t = Nx.create ctx Nx_core.Dtype.complex16 [| 2 |] 
      [| Complex.{re=1.0; im=2.0}; Complex.{re=3.0; im=4.0} |] in
    check_t ~eps:0.01 "create complex16" [| 2 |] 
      [| Complex.{re=1.0; im=2.0}; Complex.{re=3.0; im=4.0} |] t

  let test_scalar_complex16 ctx () =
    let t = Nx.scalar ctx Nx_core.Dtype.complex16 Complex.{re=5.0; im=6.0} in
    check_t ~eps:0.01 "scalar complex16" [||] [| Complex.{re=5.0; im=6.0} |] t

  let test_zeros_complex16 ctx () =
    let t = Nx.zeros ctx Nx_core.Dtype.complex16 [| 2; 2 |] in
    check_t ~eps:0.01 "zeros complex16" [| 2; 2 |] 
      [| Complex.zero; Complex.zero; Complex.zero; Complex.zero |] t

  let test_ones_complex16 ctx () =
    let t = Nx.ones ctx Nx_core.Dtype.complex16 [| 2; 2 |] in
    check_t ~eps:0.01 "ones complex16" [| 2; 2 |] 
      [| Complex.one; Complex.one; Complex.one; Complex.one |] t
  ...
  let test_dtype_properties () =
    (* Test is_float *)
    check bool "bfloat16 is_float" true (Nx_core.Dtype.is_float Nx_core.Dtype.bfloat16);
    check bool "float8_e4m3 is_float" true (Nx_core.Dtype.is_float Nx_core.Dtype.float8_e4m3);
    check bool "float8_e5m2 is_float" true (Nx_core.Dtype.is_float Nx_core.Dtype.float8_e5m2);
    check bool "bool is_float" false (Nx_core.Dtype.is_float Nx_core.Dtype.bool);
    
    (* Test is_complex *)
    check bool "complex16 is_complex" true (Nx_core.Dtype.is_complex Nx_core.Dtype.complex16);
    check bool "bfloat16 is_complex" false (Nx_core.Dtype.is_complex Nx_core.Dtype.bfloat16);
    ...
    (* Test itemsize *)
    check int "bfloat16 itemsize" 2 (Nx_core.Dtype.itemsize Nx_core.Dtype.bfloat16);
    ...
    check int "float8_e4m3 itemsize" 1 (Nx_core.Dtype.itemsize Nx_core.Dtype.float8_e4m3);
    check int "float8_e5m2 itemsize" 1 (Nx_core.Dtype.itemsize Nx_core.Dtype.float8_e5m2);
    check int "complex16 itemsize" 4 (Nx_core.Dtype.itemsize Nx_core.Dtype.complex16);
    ...
    (* Test to_string *)
    check string "bfloat16 to_string" "bfloat16" (Nx_core.Dtype.to_string Nx_core.Dtype.bfloat16);
    check string "bool to_string" "bool" (Nx_core.Dtype.to_string Nx_core.Dtype.bool);
    check string "int4 to_string" "int4" (Nx_core.Dtype.to_string Nx_core.Dtype.int4);
    check string "uint4 to_string" "uint4" (Nx_core.Dtype.to_string Nx_core.Dtype.uint4);
    check string "float8_e4m3 to_string" "float8_e4m3" (Nx_core.Dtype.to_string Nx_core.Dtype.float8_e4m3);
    check string "float8_e5m2 to_string" "float8_e5m2" (Nx_core.Dtype.to_string Nx_core.Dtype.float8_e5m2);
    check string "complex16 to_string" "complex16" (Nx_core.Dtype.to_string Nx_core.Dtype.complex16);
    check string "qint8 to_string" "qint8" (Nx_core.Dtype.to_string Nx_core.Dtype.qint8);
    check string "quint8 to_string" "quint8" (Nx_core.Dtype.to_string Nx_core.Dtype.quint8)
  ...
  let test_dtype_min_max_values () =
    ...
    check int "qint8 min_value" (-128) (Nx_core.Dtype.min_value Nx_core.Dtype.qint8);
    ...
    check int "quint8 max_value" 255 (Nx_core.Dtype.max_value Nx_core.Dtype.quint8)
  ...
  let suite backend_name ctx =
    ...
    [
      ...
      maybe_test (not is_metal) "create int4" `Quick (test_create_int4 ctx);
      maybe_test (not is_metal) "scalar int4" `Quick (test_scalar_int4 ctx);
      maybe_test (not is_metal) "zeros int4" `Quick (test_zeros_int4 ctx);
      maybe_test (not is_metal) "ones int4" `Quick (test_ones_int4 ctx);
      maybe_test (not is_metal) "arange int4" `Quick (test_arange_int4 ctx);
      
      (* UInt4 tests - NOT supported by Metal *)
      maybe_test (not is_metal) "create uint4" `Quick (test_create_uint4 ctx);
      maybe_test (not is_metal) "scalar uint4" `Quick (test_scalar_uint4 ctx);
      maybe_test (not is_metal) "zeros uint4" `Quick (test_zeros_uint4 ctx);
      maybe_test (not is_metal) "ones uint4" `Quick (test_ones_uint4 ctx);
      maybe_test (not is_metal) "arange uint4" `Quick (test_arange_uint4 ctx);
      
      (* Float8_e4m3 tests - NOT supported by Metal *)
      maybe_test (not is_metal) "create float8_e4m3" `Quick (test_create_float8_e4m3 ctx);
      maybe_test (not is_metal) "scalar float8_e4m3" `Quick (test_scalar_float8_e4m3 ctx);
      maybe_test (not is_metal) "zeros float8_e4m3" `Quick (test_zeros_float8_e4m3 ctx);
      maybe_test (not is_metal) "ones float8_e4m3" `Quick (test_ones_float8_e4m3 ctx);
      
      (* Float8_e5m2 tests - NOT supported by Metal *)
      maybe_test (not is_metal) "create float8_e5m2" `Quick (test_create_float8_e5m2 ctx);
      maybe_test (not is_metal) "scalar float8_e5m2" `Quick (test_scalar_float8_e5m2 ctx);
      maybe_test (not is_metal) "zeros float8_e5m2" `Quick (test_zeros_float8_e5m2 ctx);
      maybe_test (not is_metal) "ones float8_e5m2" `Quick (test_ones_float8_e5m2 ctx);
      
      (* Complex16 tests - NOT supported by Metal *)
      maybe_test (not is_metal) "create complex16" `Quick (test_create_complex16 ctx);
      maybe_test (not is_metal) "scalar complex16" `Quick (test_scalar_complex16 ctx);
      maybe_test (not is_metal) "zeros complex16" `Quick (test_zeros_complex16 ctx);
      maybe_test (not is_metal) "ones complex16" `Quick (test_ones_complex16 ctx);
      
      (* QInt8 tests - NOT supported by Metal *)
      maybe_test (not is_metal) "create qint8" `Quick (test_create_qint8 ctx);
      maybe_test (not is_metal) "scalar qint8" `Quick (test_scalar_qint8 ctx);
      maybe_test (not is_metal) "zeros qint8" `Quick (test_zeros_qint8 ctx);
      maybe_test (not is_metal) "ones qint8" `Quick (test_ones_qint8 ctx);
      
      (* QUInt8 tests - NOT supported by Metal *)
      maybe_test (not is_metal) "create quint8" `Quick (test_create_quint8 ctx);
      maybe_test (not is_metal) "scalar quint8" `Quick (test_scalar_quint8 ctx);
      maybe_test (not is_metal) "zeros quint8" `Quick (test_zeros_quint8 ctx);
      maybe_test (not is_metal) "ones quint8" `Quick (test_ones_quint8 ctx);
      
      (* Dtype property tests - always included *)
      Some (test_case "dtype properties" `Quick test_dtype_properties);
      Some (test_case "dtype min/max values" `Quick test_dtype_min_max_values);
    ] );
    ]
end