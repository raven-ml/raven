...
module Make (Backend : Nx_core.Backend_intf.S) = struct
  ...
  let test_create_int16 ctx () =
    let t =
      Nx.create ctx Nx_core.Dtype.int16 [| 4 |]
        [| 1; 2; 3; 4 |]
    in
    check_t "create int16" [| 4 |]
      [| 1; 2; 3; 4 |]
      t
  ...
  let test_astype_float32_to_int16 ctx () =
    let t = Nx.create ctx Nx_core.Dtype.float32 [| 4 |] [| 1.0; 2.5; 3.9; 255.0 |] in
    let u = Nx.astype Nx_core.Dtype.int16 t in
    check_t "astype to int16" [| 4 |]
      [| 1; 2; 3; 255 |]
      u

  let test_astype_int64_to_float32 ctx () =
    let t =
      Nx.create ctx Nx_core.Dtype.int64 [| 3 |]
        [| 1000L; 2000L; 3000L |]
    in
    let u = Nx.astype Nx_core.Dtype.float32 t in
  ...
  let type_conversion ctx =
    [
      ...
      ( "astype float32 to int16",
        `Quick,
        test_astype_float32_to_int16 ctx );
      ( "astype int64 to float32",
        `Quick,
        test_astype_int64_to_float32 ctx );
    ]
