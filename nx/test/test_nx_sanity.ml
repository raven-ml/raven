...
let creation_tests =
  [
    ...
    test_case "scalar" `Quick (fun () ->
        Nx.scalar float32 42.0 |> check_t "scalar" [||] [| 42.0 |]);
...
let property_access_tests =
  [
    ...
    test_case "dtype" `Quick (fun () ->
        let t = Nx.create float32 shape_2x3 test_array in
        check bool "dtype is float32" true (Nx.dtype t = float32));
...
let data_manipulation_tests =
  [
    test_case "blit" `Quick (fun () ->
        let src = Nx.ones float32 shape_2x3 in
        let dst = Nx.zeros float32 shape_2x3 in
        Nx.blit src dst;
        check_t "blit" shape_2x3 [| 1.; 1.; 1.; 1.; 1.; 1. |] dst);
    test_case "fill" `Quick (fun () ->
        let t = Nx.zeros float32 shape_2x3 in
...
let element_wise_binary_tests =
  [
    test_case "add" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 3.0 in
        let b = Nx.full float32 shape_2x3 2.0 in
    ...
    test_case "sub" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 5.0 in
        let b = Nx.full float32 shape_2x3 2.0 in
        Nx.sub a b |> check_t "sub" shape_2x3 [| 3.; 3.; 3.; 3.; 3.; 3. |]);
    test_case "sub_s" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 10.0 in
    ...
    test_case "mul_s" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 4.0 in
        Nx.mul_s a 3.0
    ...
    test_case "pow" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 2.0 in
        let b = Nx.full float32 shape_2x3 3.0 in
    ...
    test_case "mod" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 7.0 in
        let b = Nx.full float32 shape_2x3 3.0 in
    ...
    test_case "maximum" `Quick (fun () ->
        let a = Nx.full float32 shape_2x3 3.0 in
        let b = Nx.full float32 shape_2x3 5.0 in
        Nx.maximum a b
...
let comparison_tests =
  [
    test_case "equal" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 1.; 2.; 1. |] in
        let b = Nx.create float32 [| 3 |] [| 1.; 3.; 1. |] in
    ...
    test_case "greater" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 5.; 3.; 1. |] in
        let b = Nx.create float32 [| 3 |] [| 3.; 3.; 2. |] in
    ...
    test_case "less" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 1.; 3.; 5. |] in
        let b = Nx.create float32 [| 3 |] [| 3.; 3.; 2. |] in
...
let element_wise_unary_tests =
  [
    ...
    test_case "clip" `Quick (fun () ->
        let a = Nx.create float32 [| 5 |] [| -2.; 0.; 5.; 10.; 12. |] in
    ...
    test_case "lerp" `Quick (fun () ->
        let start_t = Nx.zeros float32 [| 3 |] in
        let end_t = Nx.full float32 [| 3 |] 10.0 in
        let weight = Nx.create float32 [| 3 |] [| 0.0; 0.5; 1.0 |] in
...
let special_value_tests =
  [
    test_case "isnan" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 1.0; nan; 0.0 |] in
        Nx.isnan a |> check_t "isnan" [| 3 |] [| 0; 1; 0 |]);
    test_case "isinf" `Quick (fun () ->
        let a =
          Nx.create float32 [| 4 |] [| 1.0; infinity; neg_infinity; 0.0 |]
        in
        Nx.isinf a |> check_t "isinf" [| 4 |] [| 0; 1; 1; 0 |]);
    test_case "isfinite" `Quick (fun () ->
        let a = Nx.create float32 [| 4 |] [| 1.0; infinity; nan; 0.0 |] in
...
let ternary_tests =
  [
    test_case "where" `Quick (fun () ->
        ...
        let x = Nx.create float32 [| 5 |] [| 1.; 2.; 3.; 4.; 5. |] in
        let y = Nx.create float32 [| 5 |] [| 10.; 20.; 30.; 40.; 50. |] in
...
let reduction_tests =
  [
    ...
    test_case "prod" `Quick (fun () ->
        let a = Nx.create float32 [| 4 |] [| 1.; 2.; 3.; 4. |] in
        Nx.prod a |> check_t "prod" [||] [| 24.0 |]);
    test_case "max" `Quick (fun () ->
        let a = Nx.create float32 [| 5 |] [| 1.; 5.; 3.; 2.; 4. |] in
        Nx.max a |> check_t "max" [||] [| 5.0 |]);
    test_case "min" `Quick (fun () ->
        let a = Nx.create float32 [| 5 |] [| 5.; 1.; 3.; 2.; 4. |] in
    ...
    test_case "std" `Quick (fun () ->
        let a = Nx.create float32 [| 4 |] [| 1.; 3.; 5.; 7. |] in
    ...
    test_case "array_equal" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 1.; 2.; 3. |] in
        let b = Nx.create float32 [| 3 |] [| 1.; 2.; 3. |] in
        let eq1 = Nx.array_equal a b in
        check int "array_equal same" 1 (Nx.get_item [] eq1);
        let d = Nx.create float32 [| 3 |] [| 1.; 2.; 4. |] in
        let eq2 = Nx.array_equal a d in
        check int "array_equal different" 0 (Nx.get_item [] eq2));
  ]

let shape_manipulation_tests =
  [
    test_case "reshape" `Quick (fun () ->
        let a = Nx.create float32 shape_2x3 test_array in
    ...
    test_case "unflatten" `Quick (fun () ->
        let a = Nx.create float32 [| 6 |] test_array in
    ...
    test_case "squeeze" `Quick (fun () ->
        let a = Nx.ones float32 [| 1; 3; 1 |] in
    ...
    test_case "broadcast_arrays" `Quick (fun () ->
        let a = Nx.ones float32 [| 1; 3 |] in
        let b = Nx.full float32 [| 2; 1 |] 2.0 in
...
let array_combination_tests =
  [
    test_case "concatenate" `Quick (fun () ->
        let a = Nx.create float32 [| 2; 3 |] [| 1.; 2.; 3.; 4.; 5.; 6. |] in
        let b = Nx.create float32 [| 1; 3 |] [| 7.; 8.; 9. |] in
...
let type_conversion_tests =
  [
    test_case "cast" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 1.7; 2.3; 3.9 |] in
        let b = Nx.cast Nx.int32 a in
        check_t "cast values" [| 3 |] [| 1l; 2l; 3l |] b);
    test_case "astype" `Quick (fun () ->
        let a = Nx.create float32 [| 2 |] [| 3.14; 2.71 |] in
        let b = Nx.astype float64 a in
        check bool "astype dtype" true (Nx.dtype b = float64));
    ...
    test_case "to_array" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 7.; 8.; 9. |] in
...
let indexing_slicing_tests =
  [
    ...
    test_case "set" `Quick (fun () ->
        let a = Nx.zeros float32 shape_2x3 in
        let value = Nx.create float32 [| 3 |] [| 7.; 8.; 9. |] in
        Nx.set [ 1 ] a value;
        check_t "set" shape_2x3 [| 0.; 0.; 0.; 7.; 8.; 9. |] a);
    test_case "get_item" `Quick (fun () ->
        let a = Nx.create float32 shape_2x3 test_array in
        check (float 1e-10) "get_item [0,0]" 1.0 (Nx.get_item [ 0; 0 ] a);
        check (float 1e-10) "get_item [1,2]" 6.0 (Nx.get_item [ 1; 2 ] a));
    test_case "set_item" `Quick (fun () ->
        let a = Nx.zeros float32 shape_2x3 in
        Nx.set_item [ 0; 1 ] 42.0 a;
        Nx.set_item [ 1; 2 ] 99.0 a;
        check (float 1e-10) "set_item [0,1]" 42.0 (Nx.get_item [ 0; 1 ] a);
        check (float 1e-10) "set_item [1,2]" 99.0 (Nx.get_item [ 1; 2 ] a));
    test_case "slice" `Quick (fun () ->
        let a = Nx.create float32 [| 5 |] [| 1.; 2.; 3.; 4.; 5. |] in
        Nx.slice [ R [ 1; 4 ] ] a |> check_t "slice" [| 3 |] [| 2.; 3.; 4. |]);
    test_case "set_slice" `Quick (fun () ->
        let a = Nx.zeros float32 [| 5 |] in
        let value = Nx.create float32 [| 2 |] [| 10.; 20. |] in
...
let linear_algebra_tests =
  [
    test_case "dot" `Quick (fun () ->
        let a = Nx.create float32 [| 3 |] [| 1.; 2.; 3. |] in
        let b = Nx.create float32 [| 3 |] [| 4.; 5.; 6. |] in
        Nx.dot a b |> check_t "dot" [||] [| 32.0 |]);
    test_case "matmul" `Quick (fun () ->
        let a = Nx.create float32 [| 2; 3 |] [| 1.; 2.; 3.; 4.; 5.; 6. |] in
        let b = Nx.create float32 [| 3; 2 |] [| 1.; 4.; 2.; 5.; 3.; 6. |] in
...
let neural_network_tests =
  [
    ...
    test_case "correlate1d" `Quick (fun () ->
        let x = Nx.create float32 [| 1; 1; 5 |] [| 1.; 2.; 3.; 4.; 5. |] in
        let w = Nx.create float32 [| 1; 1; 3 |] [| 1.; 0.; -1. |] in
        let y = Nx.correlate1d x w in
        check_t "correlate1d values" [| 1; 1; 3 |] [| -2.; -2.; -2. |] y);
    test_case "correlate2d" `Quick (fun () ->
        let x = Nx.ones float32 [| 1; 1; 5; 5 |] in
        let w = Nx.ones float32 [| 1; 1; 3; 3 |] in
    ...
    test_case "max_pool1d" `Quick (fun () ->
        let x = Nx.create float32 [| 1; 1; 6 |] [| 1.; 3.; 2.; 6.; 4.; 5. |] in
...
let random_tests =
  [
    test_case "rand" `Quick (fun () ->
        let t = Nx.rand float32 shape_2x3 in
...
let sorting_searching_tests =
  [
    test_case "sort" `Quick (fun () ->
        let a = Nx.create float32 [| 5 |] [| 3.; 1.; 4.; 1.; 5. |] in
    ...
    test_case "argmax" `Quick (fun () ->
        let a = Nx.create float32 [| 5 |] [| 3.; 1.; 5.; 2.; 4. |] in
...
let higher_order_tests =
  [
    ...
    test_case "iter" `Quick (fun () ->
        let a = Nx.create float32 [| 2; 2 |] [| 1.; 2.; 3.; 4. |] in
        let sum = ref (Nx.scalar float32 0.0) in
        Nx.iter (fun x -> sum := Nx.add !sum x) a;
        check (float 0.01) "iter sum" 10.0 (Nx.get_item [] !sum));
    test_case "fold" `Quick (fun () ->
        let a = Nx.create float32 [| 2; 3 |] [| 1.; 2.; 3.; 4.; 5.; 6. |] in
        let sum =
          Nx.fold (fun acc x -> Nx.add acc x) (Nx.scalar float32 0.0) a
        in
        check (float 0.01) "fold sum" 21.0 (Nx.get_item [] sum));
    test_case "fold product" `Quick (fun () ->
        let a = Nx.create float32 [| 2; 2 |] [| 1.; 2.; 3.; 4. |] in
        let prod =
          Nx.fold (fun acc x -> Nx.mul acc x) (Nx.scalar float32 1.0) a
        in
        check (float 0.01) "fold product" 24.0 (Nx.get_item [] prod));
    test_case "fold max" `Quick (fun () ->
        let a = Nx.create float32 [| 2; 3 |] [| 1.; 5.; 3.; 2.; 6.; 4. |] in
        let max_val =
          Nx.fold
            (fun acc x -> Nx.maximum acc x)
            (Nx.scalar float32 neg_infinity)
            a
        in
        check (float 0.01) "fold max" 6.0 (Nx.get_item [] max_val));
...
let () =
  run "Nx Sanity Tests"
    [
      ("Creation Functions", creation_tests);
      ("Range Generation", range_generation_tests);
      ("Property Access", property_access_tests);
      ("Data Manipulation", data_manipulation_tests);
      ("Element-wise Binary Operations", element_wise_binary_tests);
      ("Comparison Operations", comparison_tests);
      ("Element-wise Unary Operations", element_wise_unary_tests);
      ("Bitwise Operations", bitwise_tests);
      ("Logical Operations", logical_tests);
      ("Special Value Checks", special_value_tests);
      ("Ternary Operations", ternary_tests);
      ("Reduction Operations", reduction_tests);
      ("Shape Manipulation", shape_manipulation_tests);
      ("Array Combination", array_combination_tests);
      ("Type Conversion", type_conversion_tests);
      ("Indexing and Slicing", indexing_slicing_tests);
      ("Linear Algebra", linear_algebra_tests);
      ("Neural Network", neural_network_tests);
      ("Random Number Generation", random_tests);
      ("Sorting and Searching", sorting_searching_tests);
      ("Display and Formatting", display_formatting_tests);
      ("Higher-order Functions", higher_order_tests);
    ]
