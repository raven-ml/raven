...
module Platform = struct
  type t = MacOS | IOS | Catalyst | Linux | Unknown of string
  ...
  let link_libs ~ffi = function
    | MacOS | IOS | Catalyst -> [ "-lobjc"; "-framework"; "Foundation" ] @ ffi
    | Linux ->
        let ldirs =
          [
...
let libffi_flags c =
  ...
  | None -> ([], [ "-lffi" ])
  | Some pc -> (
      match Pkg_config.query pc ~package:"libffi" with
      | None -> ([], [ "-lffi" ])
      | Some pkg -> (pkg.cflags, "-lffi" :: pkg.libs))
...
let () =
  main ~name:"objc_discover" (fun c ->
      ...
      let ffi_cflags, ffi_libs = libffi_flags c in
      ...
      let cclibs = Platform.link_libs ~ffi:ffi_libs platform in
      Flags.write_sexp "cclibs.sexp" cclibs;

      write_ocaml_module ~name:"platform"
        ~type_def:"type t = MacOS | IOS | Catalyst | Linux | Unknown of string"
        ~current_value:
          (match platform with
