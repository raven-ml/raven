...
let image_path = "ndarray-cv/example/lena.png"

(* Helper to visualize Sobel output (int16) as uint8 *)
let visualize_sobel (sobel_img : int16_t) : uint8_t =
  (* Calculate absolute values *)
  let abs_sobel = Ndarray.map Int.abs sobel_img in

  (* Use astype and Ndarray reductions for min/max *)
  let abs_sobel_f = Ndarray.astype Ndarray.float32 abs_sobel in
  let min_val_t = Ndarray.min ~keepdims:false abs_sobel_f in
  let max_val_t = Ndarray.max ~keepdims:false abs_sobel_f in

  (* Check if tensors are actually scalars before getting item *)
  let min_val =
    if Ndarray.ndim min_val_t = 0 then Ndarray.get_item [||] min_val_t else 0.0
    (* Default or error *)
  in
  let max_val =
    if Ndarray.ndim max_val_t = 0 then Ndarray.get_item [||] max_val_t
    else 255.0
    (* Default or error *)
  in
  ...
  if range <= 1e-6 then (* Avoid division by zero if image is flat *)
    Ndarray.zeros Ndarray.uint8 (Ndarray.shape sobel_img)
  else
    (* Scale to 0.0 - 1.0 *)
    let range_scalar = Ndarray.scalar Ndarray.float32 range in
    let min_scalar = Ndarray.scalar Ndarray.float32 min_val in
    let scaled_f =
      Ndarray.div (Ndarray.sub abs_sobel_f min_scalar) range_scalar
    in
    (* Convert to uint8 [0, 255] *)
    Ndarray_cv.to_uint8 scaled_f
