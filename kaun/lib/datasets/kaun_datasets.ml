...
let vision_pipeline ?(batch_size = 32) ?(shuffle_buffer = 10000)
  ...
  let _ = num_workers in
  (* TODO: Use when parallel processing is needed *)
  dataset
...
let text_pipeline ?(batch_size = 32) ?(shuffle_buffer = 10000)
  ...
  let dataset =
    ...
    | Some boundaries ->
        ...
        in
        Kaun_dataset.bucket_by_length ~boundaries ~batch_sizes
          (fun (tokens, _) -> Array.length tokens)
          dataset
    | None -> Kaun_dataset.batch batch_size dataset
  in
