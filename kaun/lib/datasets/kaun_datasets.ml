...
let mnist ?(train=true) ?(flatten=false) ?(normalize=true) ?(data_format=`NCHW) () =
  ...
  let x = match data_format with
    | `NCHW ->
        ...
        Printf.printf "[Kaun_datasets.mnist] After transpose, is_c_contiguous: %b\n%!" 
  ...
  let y = Rune.squeeze y ~axes:[|1|] in  (* Remove the extra dimension [N, 1] -> [N] *)
