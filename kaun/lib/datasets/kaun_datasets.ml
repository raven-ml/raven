...
let mnist ?(train = true) ?(flatten = false) ?(normalize = true)
  ...
  let x =
    if normalize then 
      Rune.div x (Rune.scalar device Rune.float32 255.0) 
    else x
  in
...
let cifar10 ?(train = true) ?(normalize = true) ?(data_format = `NCHW)
  ...
  let x =
    if normalize then
      let mean_arr = Bigarray.Array1.of_array Bigarray.float32 Bigarray.c_layout [| 0.485; 0.456; 0.406 |] in
      let std_arr = Bigarray.Array1.of_array Bigarray.float32 Bigarray.c_layout [| 0.229; 0.224; 0.225 |] in
      let mean = Rune.of_bigarray device (Bigarray.genarray_of_array1 mean_arr) in
...
let fashion_mnist ?(train = true) ?(flatten = false) ?(normalize = true)
    ?(data_format = `NCHW) ?cache_dir:_ ~device () =
  (* Load from nx-datasets *)
  let (x_train, y_train), (x_test, y_test) = Nx_datasets.load_fashion_mnist () in
...
let imdb ?(train = true) ?tokenizer ?(max_length = 512) ?cache_dir:_ ~device () =
  ...
  let texts = Array.init num_samples (fun i ->
    if i mod 2 = 0 then
      "This movie was absolutely fantastic great amazing wonderful"
    else  
      "This movie was terrible awful bad horrible worst"
  ) in
  
  (* Create labels *)
  let labels = Array.init num_samples (fun i -> 
  ...
  let tokenized = 
    ...
    | None ->
        ...
        Kaun_dataset.tokenize Kaun_dataset.whitespace_tokenizer 
          ~max_length ~truncation:true text_dataset
  in
...
let wikitext ?(dataset_name = `Wikitext2) ?tokenizer ?(sequence_length = 1024) 
  ...
  let _ = dataset_name in  (* Mark as intentionally unused *)
  ...
  let tokenizer = Option.value tokenizer ~default:Kaun_dataset.whitespace_tokenizer in
  let tokens = tokenizer text in
  
  (* Create sliding windows for language modeling *)
  let num_windows = max 1 ((Array.length tokens - sequence_length - 1) / sequence_length) in
  
  let windows = Array.init num_windows (fun i ->
...
let iris ?(normalize = true) ?(train_split = 0.8) ?shuffle_seed ~device () =
  let _ = train_split in  (* Mark as intentionally unused - will use train_test_split *)
  ...
  let x =
    if normalize then
      ...
      Rune.div (Rune.sub x mean) (Rune.add std (Rune.scalar device Rune.float32 1e-8))
    else x
  in
...
let boston_housing ?(normalize = true) ?(train_split = 0.8) ~device () =
  ...
  (* Use California housing as a replacement since Boston housing is not in nx-datasets *)
...
let download_and_extract ~url ~cache_dir ?(extract = true) () =
  ...
  (* Extract if needed *)
  if extract && (Filename.check_suffix filename ".tar.gz" || 
                 Filename.check_suffix filename ".zip") then begin
    let extract_dir = Filename.chop_extension filepath in
    if not (Sys.file_exists extract_dir) then begin
      ...
      let cmd = 
        if Filename.check_suffix filename ".tar.gz" then
          ...
          Printf.sprintf "unzip -q %s -d %s" filepath cache_dir
      in
      match Sys.command cmd with
...
let train_test_split ?(test_size = 0.2) ?(shuffle = true) ?seed dataset =
  ...
  let test_dataset = dataset |> Kaun_dataset.skip train_length |> Kaun_dataset.take test_length in
  
  (train_dataset, test_dataset)

(** {1 Pre-configured Pipelines} *)

let vision_pipeline ?(batch_size = 32) ?(shuffle_buffer = 10000) 
    ?(prefetch_buffer = 2) ?(num_workers = 4) dataset =
  let _ = num_workers in  (* TODO: Use when parallel processing is needed *)
  dataset
  |> Kaun_dataset.shuffle ~buffer_size:shuffle_buffer
  |> Kaun_dataset.batch_map batch_size (fun batch ->
      (* Stack image and label arrays into batched tensors *)
  ...
  |> Kaun_dataset.prefetch ~buffer_size:prefetch_buffer

let text_pipeline ?(batch_size = 32) ?(shuffle_buffer = 10000)
    ?bucket_boundaries ?(prefetch_buffer = 2) ?(num_workers = 4) dataset =
  let _ = num_workers in  (* TODO: Use when parallel processing is needed *)
  let dataset = Kaun_dataset.shuffle ~buffer_size:shuffle_buffer dataset in
  
  (* Apply bucketing if boundaries provided *)
  let dataset = 
    match bucket_boundaries with
    | Some boundaries ->
        let batch_sizes = 
          (* Create decreasing batch sizes for longer sequences *)
          List.map (fun _ -> batch_size) boundaries @ [batch_size / 2]
        in
        Kaun_dataset.bucket_by_length 
          ~boundaries 
          ~batch_sizes
          (fun (tokens, _) -> Array.length tokens)
          dataset
    | None ->
        Kaun_dataset.batch batch_size dataset
  in
  
  (* Map to split the batch into token and label arrays *)
  let dataset = Kaun_dataset.map (fun batch ->
    let tokens, labels = Array.split batch in
    (tokens, labels)
  ) dataset in
  
  dataset |> Kaun_dataset.prefetch ~buffer_size:prefetch_buffer