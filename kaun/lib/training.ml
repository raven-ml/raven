module State = struct
  type ('layout, 'dev) t = {
    ...
    opt_state : ('layout, 'dev) Kaun_optim.opt_state;
    metrics : ('layout, 'dev) Metrics.Collection.t option;
    rngs : Rune.Rng.key;
    model : Module.t;
    optimizer : ('layout, 'dev) Kaun_optim.gradient_transformation;
  }

  let create ~model ~optimizer ?metrics ~rngs ~device ~dtype () =
    (* Always initialize with a dummy input *)
    let params = model.Module.init ~rngs ~device ~dtype in
    let opt_state = optimizer.Kaun_optim.init params in
    { step = 0; params; opt_state; metrics; rngs; model; optimizer }

  let apply_gradients state ~grads =
    let updates, opt_state =
      state.optimizer.Kaun_optim.update state.opt_state state.params grads
    in
    Kaun_optim.apply_updates_inplace state.params updates;
...
module Callbacks = struct
  ...
  let model_checkpoint ~filepath ?(monitor = "val_loss") ?(mode = `Min)
    ...
    let save_checkpoint ctx =
      ...
      Kaun_checkpoint.save_params ~path ~params:ctx.state.params ();
