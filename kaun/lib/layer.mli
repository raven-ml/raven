(** Neural network layer constructors.
    ...
    {!Module.t}, which encapsulates parameter initialization and forward
...
val dropout : rate:float -> unit -> Module.t
...
val batch_norm : num_features:int -> unit -> Module.t
...
val max_pool2d : kernel_size:int * int -> ?stride:int * int -> unit -> Module.t
...
val avg_pool2d : kernel_size:int * int -> ?stride:int * int -> unit -> Module.t
...
val sequential : Module.t list -> Module.t
...
val rms_norm :
  dim:int -> ?eps:float -> ?scale_init:Initializers.t -> unit -> Module.t
...
val layer_norm :
  dim:int -> ?eps:float -> ?elementwise_affine:bool -> unit -> Module.t
...
val multi_head_attention :
  ...
  ?dropout:'e ->
  ?use_qk_norm:'f ->
  ?attn_logits_soft_cap:'g ->
  ?query_pre_attn_scalar:'h ->
  unit ->
  Module.t
...
val rope_embedding :
  ...
  unit ->
  Module.t
(** [rope_embedding ~dim ...] creates a Rotary Position Embedding layer.

    Implements RoPE from "RoFormer: Enhanced Transformer with Rotary Position
    Embedding". Encodes positional information by rotating query and key
    vectors.

    @param dim Embedding dimension (must be even)
    @param max_seq_len Maximum sequence length. Default: [2048]
    @param base_frequency Base frequency for rotation. Default: [10000.0]
    @param scale_factor Scaling factor for frequencies. Default: [1.0] *)

val sinusoidal_pos_embedding : max_len:'a -> embed_dim:'b -> unit -> Module.t
(** [sinusoidal_pos_embedding ~max_len ~embed_dim ()] creates fixed sinusoidal
    position embeddings.

    Implements position encodings from "Attention Is All You Need" using sine
    and cosine functions. Provides fixed positional information without
    learnable parameters.

    @param max_len Maximum sequence length
    @param embed_dim Embedding dimension

    No learnable parameters. Position encodings are computed deterministically.
*)
