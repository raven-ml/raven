...
let truncated_normal ~mean ~stddev ~lower ~upper rng shape dev dtype =
...
let constant value : ('layout, 'dev) t =
...
let uniform ?(scale = 0.01) () rng shape dev dtype =
  let u01 = Rune.rand dev dtype ~seed:rng shape in
  Rune.mul u01 (Rune.scalar dev dtype scale)

let normal ?(stddev = 0.01) () rng shape dev dtype =
  let z = Rune.randn dev dtype ~seed:rng shape in
  Rune.mul z (Rune.scalar dev dtype stddev)

let truncated_normal_init ?(stddev = 0.01) ?(lower = -2.0) ?(upper = 2.0) () rng
    shape dev dtype =
  truncated_normal ~mean:0.0 ~stddev ~lower ~upper rng shape dev dtype

(* Variance scaling initializer - the general framework *)
let variance_scaling ~scale ~mode ~distribution ~in_axis ~out_axis () rng shape
    dev dtype =
  ...
  | `Normal ->
      ...
      Rune.mul z (Rune.scalar dev dtype stddev)
  | `Truncated_normal ->
      truncated_normal ~mean:0.0 ~stddev ~lower:(-2.0) ~upper:2.0 rng shape dev
        dtype
  | `Uniform ->
...
let orthogonal ?(scale = 1.0) ?(column_axis = -1) () rng shape dev dtype =
  let rank = Array.length shape in
  let column_axis =
  ...
  let a = Rune.randn dev dtype ~seed:rng flat_shape in
  ...
  let a_scaled = Rune.mul a (Rune.scalar dev dtype (scale /. norm_factor)) in

  (* Reshape back to original shape *)
  Rune.reshape shape a_scaled

let delta_orthogonal ?(scale = 1.0) ?(column_axis = -1) () rng shape dev dtype =
  ...
  let is_square = Array.for_all (fun d -> d = spatial_dims.(0)) spatial_dims in
  ...
  let _orth = orthogonal ~scale ~column_axis () rng orth_shape dev dtype in
