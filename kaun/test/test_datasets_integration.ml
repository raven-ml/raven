...
let test_mnist_streaming () =
  ...
  Kaun_dataset.iter (fun (x_batch, y_batch) ->
    ...
    Printf.printf "Batch %d: images shape [%s], labels shape [%s]\n"
      !batch_count
      (x_shape |> Array.to_list |> List.map string_of_int |> String.concat "; ")
      (y_shape |> Array.to_list |> List.map string_of_int |> String.concat "; ")
  ) dataset;
...
let test_train_test_split () =
  ...
  assert (train_count = 120);  (* 80% of 150 *)
  assert (test_count = 30);    (* 20% of 150 *)
