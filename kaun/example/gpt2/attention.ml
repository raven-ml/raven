    ...
    apply =
      (fun params ~training ?rngs x ->
        ...
        | Record fields ->
            ...
            let query =
              Rune.slice_ranges [ 0; 0; 0 ]
                [ batch_size; seq_len; embd_dim ]
                qkv
            in
            let key =
              Rune.slice_ranges [ 0; 0; embd_dim ]
                [ batch_size; seq_len; 2 * embd_dim ]
                qkv
            in
            let value =
              Rune.slice_ranges
                [ 0; 0; 2 * embd_dim ]
                [ batch_size; seq_len; 3 * embd_dim ]
                qkv
            in
            ...
            let query =
              Rune.reshape [| batch_size; seq_len; num_heads; head_dim |] query
            in
            let key =
              Rune.reshape [| batch_size; seq_len; num_heads; head_dim |] key
            in
            let value =
              Rune.reshape [| batch_size; seq_len; num_heads; head_dim |] value
            in
            ...
            let scale =
              if scale_attn_weights then 1.0 /. (float_of_int head_dim ** 0.5)
              else 1.0
            in
            let scores =
              Rune.mul scores
                (Rune.scalar (Rune.device scores) (Rune.dtype scores) scale)
            in

            (* Apply causal mask *)
            let mask_slice =
              Rune.slice_ranges [ 0; 0 ] [ seq_len; seq_len ] causal_mask
            in
            (* Expand mask for batch and heads dimensions *)
            let mask_slice =
              Rune.reshape [| 1; 1; seq_len; seq_len |] mask_slice
            in
            let mask_slice =
              Rune.broadcast_to
            ...
            let masked_scores =
              let large_neg =
                Rune.scalar (Rune.device scores) (Rune.dtype scores) (-1e4)
              in
              let large_neg_broadcast =
                Rune.broadcast_to (Rune.shape scores) large_neg
              in
              (* Convert mask to uint8 for where condition *)
              let mask_bool = Rune.cast Rune.uint8 mask_slice in
              (* where(mask, scores, large_neg) - keep scores where mask is 1,
                 else large_neg *)
            ...
            (* Transpose back: [batch, num_heads, seq_len, head_dim] -> [batch,
            ...
            let attn_output =
              Rune.reshape [| batch_size; seq_len; embd_dim |] attn_output
            in
