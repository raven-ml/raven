...
let () =
  ...
  let test_inference () =
    ...
    let text =
      Inference.infer_text "The quick brown fox" ~model_name:"gpt2"
        ~max_length:10 ()
    in
  ...
  let test_model_creation () =
    ...
    let _fine_tune params _dataset num_epochs _batch_size learning_rate =
      ...
      for epoch = 1 to num_epochs do
        ...
        let input_ids =
          let ids =
            Rune.Rng.uniform
              (Rune.Rng.key (42 + epoch))
              device dtype
              [| batch_size; seq_length |]
          in
          Rune.mul ids
            (Rune.scalar device dtype (float_of_int config.vocab_size))
        in
        let labels = input_ids in
        (* For language modeling, labels = inputs shifted *)
        ...
        let loss, grads =
          Kaun.value_and_grad
            (fun params ->
              ...
              in
              let labels_flat =
                Rune.reshape [| batch_size * seq_len |] labels
              in
              Loss.softmax_cross_entropy_with_indices logits_flat labels_flat)
            !current_params
        in
        ...
        let updates, new_state =
          optimizer.update !opt_state !current_params grads
        in
        opt_state := new_state;
    ...
    let input_ids =
      ...
      in
      (* Cast to int for indexing - but embedding layer expects float, so keep
         as float *)
      ids
    in
