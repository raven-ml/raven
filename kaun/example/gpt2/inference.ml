...
let generate_greedy model params config prompt_tokens max_length =
  ...
  for _ = 1 to max_length do
    ...
    let input_ids = 
      ...
      let ba = Bigarray.Array2.create Bigarray.float32 Bigarray.c_layout batch_size seq_len in
      Array.iteri (fun i token_id ->
        ba.{0, i} <- float_of_int token_id
      ) tokens_array;
    ...
    let last_logits = Rune.slice_ranges [0; seq_len - 1; 0] [1; seq_len; vocab_size] logits in
...
let generate_with_sampling model params config prompt_tokens max_length 
  ...
  for _ = 1 to max_length do
    ...
    (* Use Saga's sampler *)
    let next_token = Saga.Sampler.sample_token 
      ~temperature
      ~top_k
      ~top_p
      logits
...
let infer_text text ?(model_name="gpt2") ?(max_length=50) () =
  ...
  let generated = generate_with_sampling 
    model params config tokens max_length
    ~temperature:0.8
    ~top_k:40
    ()
  in
...
(* Note: This is a simplified example. For production use:
   1. Use proper GPT-2 tokenizer (BPE)
   2. Implement KV caching for efficiency
