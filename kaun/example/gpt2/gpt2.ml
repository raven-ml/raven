...

  {
    init =
      (fun ~rngs ~device ~dtype ->
        ...
        let num_rngs = 3 + num_layers in
        (* token_emb, pos_emb, ln_f, and blocks *)
        let rngs_array =
          Array.init num_rngs (fun i ->
              if i < Array.length rngs_split then rngs_split.(i)
              else
                (Rune.Rng.split rngs_split.(0)).(i mod Array.length rngs_split))
        in

        (* Token embeddings *)
        let token_emb = Layer.embedding ~vocab_size ~embed_dim:embd_dim () in
        let token_emb_params =
          init token_emb ~rngs:rngs_array.(0) ~device ~dtype
        in

        (* Position embeddings *)
        let pos_emb =
          Layer.embedding ~vocab_size:max_pos ~embed_dim:embd_dim ()
        in
        ...
        let blocks =
          Array.init num_layers (fun i ->
              ...
              let block_params =
                init block ~rngs:rngs_array.(3 + i) ~device ~dtype
              in
              block_params)
        in
    ...
    apply =
      (fun params ~training ?rngs input_ids ->
        match params with
        | Record fields ->
            let get_params name =
            ...
            (* Token embeddings *)
            let token_emb =
              Layer.embedding ~vocab_size ~embed_dim:embd_dim ()
            in
            let input_embeds =
              apply token_emb token_emb_params ~training:false input_ids
            in

            (* Position IDs - create range from 0 to seq_len *)
            let position_ids =
              let pos =
                Rune.arange (Rune.device input_ids) Rune.int32 0 seq_len 1
              in
            ...
            in
            let position_embeds =
              apply pos_emb pos_emb_params ~training:false position_ids
            in
...

  {
    init =
      (fun ~rngs ~device ~dtype ->
        ...
        let lm_head =
          Layer.linear ~in_features:config.Config.n_embd
            ~out_features:vocab_size ()
        in
        let lm_head_params = init lm_head ~rngs:rng2 ~device ~dtype in

        Ptree.record_of
          [ ("transformer", model_params); ("lm_head", lm_head_params) ]);
    apply =
      (fun params ~training ?rngs input_ids ->
        match params with
        | Record fields ->
            ...
            let model = gpt2_model ~config () in
            let hidden_states =
              apply model transformer_params ~training ?rngs input_ids
            in
            ...
            in
            let logits =
              apply lm_head lm_head_params ~training ?rngs hidden_states
            in
