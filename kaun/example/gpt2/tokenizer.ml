...
let get_tokenizer ?(cache_dir = "/tmp/gpt2_tokenizer") () =
  ...
  download_if_needed "https://www.dropbox.com/s/s93xkhgcac5nbmn/vocab.json?dl=1"
    vocab_file;
  download_if_needed "https://www.dropbox.com/s/7f5n1gf348sy1mt/merges.txt?dl=1"
    merges_file;
...
let encode tokenizer text = Saga.encode tokenizer text

(* Decode token IDs back to text *)
let decode tokenizer ids = Saga.decode tokenizer ids
  ...
  if padding then
    ...
    List.map
      (fun arr ->
        ...
        else if len > actual_max_length then Array.sub arr 0 actual_max_length
        else arr)
      encoded
  else encoded
