...
let train () =
  ...
  for epoch = 1 to 10 do
    ...
    Dataset.iter
      (fun (x_batch, y_batch) ->
        incr batch_count;
        if !batch_count = 1 then Printf.printf "Got first batch! Shape: X=[%s], Y=[%s]\n%!"
          (String.concat "; " (Array.to_list (Array.map string_of_int (Rune.shape x_batch))))
          (String.concat "; " (Array.to_list (Array.map string_of_int (Rune.shape y_batch))));
        let batch_start = Unix.gettimeofday () in
        ...
        if !batch_count <= 3 || !batch_count mod 100 = 0 then
          Printf.printf "  Batch %d: %.3fs (fwd+bwd: %.3fs, opt: %.3fs, metric: %.3fs)\n%!" 
