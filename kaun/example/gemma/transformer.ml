    ...
    apply =
      (fun params ~training:_ ?rngs:_ x ->
        ...
        | Record fields ->
            ...
            let tanh_arg =
              Rune.mul
                (Rune.scalar (Rune.device gate) (Rune.dtype gate) sqrt_2_over_pi)
                inner
            in
            let gate_activated =
              Rune.mul gate
...

  {
    init =
      (fun ~rngs ~device ~dtype ->
        ...
        in
        let pre_attn_norm_params =
          init pre_attn_norm ~rngs:rng1 ~device ~dtype
        in
        ...
        in
        let attention_params = init attention ~rngs:rng3 ~device ~dtype in
          ...
          if config.Config.use_post_attn_norm then
            let norm =
              Layer.rms_norm ~dim:config.Config.embed_dim
                ~eps:config.Config.rms_norm_eps ()
            in
            init norm ~rngs:rng5 ~device ~dtype
          else List []
        in
        ...
        let pre_ffn_norm =
          Layer.rms_norm ~dim:config.Config.embed_dim
            ~eps:config.Config.rms_norm_eps ()
        in
        let pre_ffn_norm_params = init pre_ffn_norm ~rngs:rng7 ~device ~dtype in
        ...
        in
        let ffn_params = init ffn ~rngs:rng8 ~device ~dtype in
          ...
          if config.Config.use_post_ffw_norm then
            let norm =
              Layer.rms_norm ~dim:config.Config.embed_dim
                ~eps:config.Config.rms_norm_eps ()
            in
            init norm ~rngs:rng2 ~device ~dtype
          else List []
        in
      ...
      (fun ~rngs ~device ~dtype ->
        ...
        let blocks_params =
          let rec init_blocks idx rngs acc =
            if idx >= config.Config.num_layers then List.rev acc
              ...
              let block_params = init block ~rngs:rng_block ~device ~dtype in
        ...
        let final_norm =
          Layer.rms_norm ~dim:config.Config.embed_dim
            ~eps:config.Config.rms_norm_eps ()
        in
        let final_norm_params = init final_norm ~rngs:rng_norm ~device ~dtype in
    ...
    apply =
      (fun params ~training ?rngs x ->
        ...
        | Record fields -> (
            ...
            let x =
              Rune.zeros (Rune.device embeddings) (Rune.dtype embeddings)
                [| batch_size; seq_len; config.Config.embed_dim |]
            in
