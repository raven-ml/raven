...
module Make (B : S) = struct
  ...
  let test_grad_neg () =
    (* neg: d/dx (-x) = -1 *)
    let grad_neg = T.grad T.neg (T.scalar ctx T.float32 1.0) in
  ...
  let test_grad_relu () =
    ...
    let expected = T.create ctx T.float32 [| 4 |] [| 0.; 0.; 1.; 1. |] in
  ...
  let test_grad_abs () =
    ...
    let expected = T.create ctx T.float32 [| 4 |] [| -1.; -1.; 1.; 1. |] in
  ...
  let test_grad_mean () =
    ...
    check_rune ~eps "mean gradient" (T.full ctx T.float32 [| 2; 2 |] 0.25) grad
  ...
  let test_grad_min () =
    ...
    let x = T.create ctx T.float32 [| 2; 2 |] [| 4.; 2.; 3.; 1. |] in
  ...
  let test_grad_prod () =
    ...
    let expected = T.create ctx T.float32 [| 3 |] [| 12.; 8.; 6. |] in
  ...
  let test_grad_tile () =
    ...
    let expected = T.full ctx T.float32 [| 2 |] 3.0 in
  ...
  let test_grad_leaky_relu () =
    ...
    let x = T.create ctx T.float32 [| 4 |] [| -2.; -1.; 1.; 2. |] in
    let grad =
      T.grad (fun x -> T.sum (T.leaky_relu ~negative_slope:0.01 x)) x
    in
    let expected = T.create ctx T.float32 [| 4 |] [| 0.01; 0.01; 1.; 1. |] in
    check_rune ~eps "leaky_relu gradient" expected grad

  let test_grad_elu () =
    (* ELU gradient *)
    let x = T.create ctx T.float32 [| 2 |] [| -1.; 1. |] in
    let grad = T.grad (fun x -> T.sum (T.elu ~alpha:1.0 x)) x in
    (* For x > 0: grad = 1, for x < 0: grad = alpha * exp(x) *)
    let expected = T.create ctx T.float32 [| 2 |] [| 0.3678794; 1. |] in
  ...
  let test_grad_dot () =
    ...
    let x = T.create ctx T.float32 [| 3 |] [| 1.; 2.; 3. |] in
    let y = T.create ctx T.float32 [| 3 |] [| 4.; 5.; 6. |] in
  ...
  let test_grad_norm () =
    (* L2 norm gradient *)
    let x = T.create ctx T.float32 [| 3 |] [| 3.; 0.; 4. |] in
    let grad = T.grad (fun x -> T.norm x) x in
    (* Gradient of ||x|| is x/||x|| *)
    let expected = T.create ctx T.float32 [| 3 |] [| 0.6; 0.; 0.8 |] in
  ...
  let test_grad_binary_cross_entropy () =
    ...
    let logits_bce =
      T.create ctx T.float32 [| 4; 1 |] [| -1.; 0.5; 0.5; -1. |]
    in
  ...
  let test_grad_chain_rule () =
    ...
    check_scalar ~eps "chain rule: grad(x⁴) at x=2" 32.0
      (scalar_value grad_chain)
  ...
  let test_grad_pow () =
    ...
    let x = T.scalar ctx T.float32 2.0 in
    let n = T.scalar ctx T.float32 3.0 in
  ...
  let test_grad_zero () =
    (* Gradient at zero *)
    let grad = T.grad (fun x -> T.mul x x) (T.scalar ctx T.float32 0.0) in
  ...
  let test_grad_large_values () =
    ...
    let x = T.scalar ctx T.float32 1e10 in
    let grad = T.grad (fun x -> T.div x (T.scalar ctx T.float32 1e20)) x in
  ...
  let test_grad_small_values () =
    ...
    let x = T.scalar ctx T.float32 1e-10 in
    let grad = T.grad (fun x -> T.mul x (T.scalar ctx T.float32 1e10)) x in
    check_scalar ~eps "small value gradient" 1e10 (scalar_value grad)

  let suite =
    [
      ( B.prefix ^ "binary operations",
        [
      ...
      ( B.prefix ^ "composition and higher-order",
        [
...
module Native_grad = Make (struct
  type dev = [ `ocaml ]

  let device = T.ocaml
  let prefix = "Native grad :: "
end)

module C_grad = Make (struct
  type dev = [ `c ]
...
module Metal_grad = Make (struct
  type dev = [ `metal ]

  let device = T.metal ()
  let prefix = "Metal grad :: "
end)

(* Test suite *)
let () =
  run "Rune Autodiff Tests"
    (List.concat [ Native_grad.suite; C_grad.suite; Metal_grad.suite ])
