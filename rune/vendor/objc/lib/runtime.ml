...
module Raw = struct
  ...
  let send ~self ~cmd ~typ =
    foreign "objc_msgSend" (id @-> _SEL @-> typ) self cmd

  (** Sends a message with a simple return value to an instance of a class. *)
  let send_gilesc ?(release_runtime_lock = false) ~self ~typ cmd_name =
    foreign ~release_runtime_lock "objc_msgSend"
      (id @-> _SEL @-> typ)
      self (selector cmd_name)

  (** Sends a message with a simple return value to the superclass of an
      instance of a class. *)
  let send_super ~self ~cmd ~typ =
    match Platform.current with
    | GNUStep ->
        let self_class = Object.get_class self in
        let super_cls_of_self =
          Objc_c.Functions.Class.get_superclass self_class
        in
        let actual_super_cls =
          if Objc_c.Functions.Class.is_meta_class self_class then
            Objc_c.Functions.Class.get_superclass
              (Object.get_class super_cls_of_self)
          else super_cls_of_self
        in
        let imp =
          Objc_c.Functions.Class.get_method_implementation actual_super_cls cmd
        in
  ...
  let send_returning_id = send ~typ:(returning id)

  (** Shortcut for type [id @-> void] *)
  let send_with_id_arg = send ~typ:(id @-> returning void)

  (** Sends a message with a data-structure return value to an instance of a
      class. *)
  let send_stret ~self ~cmd ~typ ~return_type =
    match Arch.current with
    | Amd64 -> (
        match sizeof return_type with
        | 2 | 4 | 8 | 16 -> send ~self ~cmd ~typ
        | _ -> foreign "objc_msgSend_stret" (id @-> _SEL @-> typ) self cmd)
    | Arm64 -> send ~self ~cmd ~typ
    | _ ->
        failwith
          "send_stret: not supported on this architecture. Use send instead."
...
let nsstring_class = C.Functions.Objc.get_class "NSString"
let nil = coerce (ptr void) C.Types.id null
let is_nil = is_null

(** Returns a new instance of the receiving class. *)
let alloc self = Raw.send_returning_id ~self ~cmd:(selector "alloc")

(** Allocates a new instance of the receiving class, sends it an init message,
    and returns the initialized object. *)
let _new_ self = Raw.send_returning_id ~self ~cmd:(selector "new")

(** Implemented by subclasses to initialize a new object (the receiver)
    immediately after memory for it has been allocated. *)
let init self = Raw.send_returning_id ~self ~cmd:(selector "init")

(** Returns the object returned by copyWithZone: *)
let _copy_ self = Raw.send_returning_id ~self ~cmd:(selector "copy")

(** Increments the receiver’s reference count. *)
let retain self = Raw.send_returning_id ~self ~cmd:(selector "retain")

(** Decrements the receiver’s reference count. *)
let release self =
  Raw.send ~self ~cmd:(selector "release") ~typ:(returning void)

(** Decrements the receiver’s retain count at the end of the current autorelease
    pool block. *)
let autorelease self =
  Raw.send ~self ~cmd:(selector "autorelease") ~typ:(returning void)
...
let alloc_object class_name = alloc (C.Functions.Objc.get_class class_name)
...
let to_string self =
  let conv instance =
    Raw.send ~self:instance ~cmd:(selector "UTF8String") ~typ:(returning string)
  in
  if is_nil self then ""
  else if
    Raw.send ~self
  ...
  let setter ?(notify_change = false) ~typ ~enc ivar_name =
    ...
    let imp self _cmd value =
      if notify_change then
        Raw.send_with_id_arg ~self ~cmd:(selector "willChangeValueForKey:") key;

      ivar_ptr ~self ~ivar_name |> from_voidp typ <-@ value;

      if notify_change then
        Raw.send_with_id_arg ~self ~cmd:(selector "didChangeValueForKey:") key
    in
...
module Class = struct
  ...
  (** Defines a new class and registers it with the Objective-C Raw. *)
  let define ?(superclass = C.Functions.Objc.get_class "NSObject")
    ...
    let allocate_class ?(extra_bytes = Unsigned.Size_t.of_int 0) ~superclass
        name =
      C.Functions.Objc.allocate_class superclass name extra_bytes
    in
...
module Block = struct
  ...
  let self = C.Functions.Objc.get_class "__NSGlobalBlock"
...
let default_uncaught_exception_handler ex =
  let name = Raw.send ~self:ex ~cmd:(selector "name") ~typ:(returning id)
  and reason = Raw.send ~self:ex ~cmd:(selector "reason") ~typ:(returning id)
  and to_string self =
    Raw.send ~self ~cmd:(selector "UTF8String") ~typ:(returning string)
  in
