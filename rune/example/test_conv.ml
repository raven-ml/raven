...
let () =
  ...
  (* Simple test case *)
  let x = randn ctx Float32 [| 2; 1; 5; 5 |] in  (* batch=2, channels=1, 5x5 *)
  let w = randn ctx Float32 [| 3; 1; 3; 3 |] in  (* 3 output channels, 3x3 kernel *)
  ...
  Printf.printf "Gradient shape: %s\n" (Array.to_list (shape (List.hd grads)) |> List.map string_of_int |> String.concat ", ")