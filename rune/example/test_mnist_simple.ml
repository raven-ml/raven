...
let () =
  ...
  Printf.printf "After conv1: %s\n" (Array.to_list (shape conv1_out) |> List.map string_of_int |> String.concat ", ");
  ...
  Printf.printf "After pool1: %s\n" (Array.to_list (shape pool1_out) |> List.map string_of_int |> String.concat ", ");
  ...
  Printf.printf "Conv1_w gradient shape: %s\n" (Array.to_list (shape (List.nth grads 0)) |> List.map string_of_int |> String.concat ", ");
  Printf.printf "Conv1_b gradient shape: %s\n" (Array.to_list (shape (List.nth grads 1)) |> List.map string_of_int |> String.concat ", ")