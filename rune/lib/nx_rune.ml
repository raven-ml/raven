...
let view : type a b. (a, b) t -> Lazy_view.t = function
  ...
  | Symbolic_tensor { shape; _ } -> Lazy_view.create (Symbolic_shape.of_ints shape)
...
let shape_op1 eff cpu_op metal_op c_op t_in shape_arg =
  ...
  with Effect.Unhandled _ -> (
    match t_in with
    | Ocaml_tensor t -> Ocaml_tensor (cpu_op t (Symbolic_shape.of_ints shape_arg))
    | C_tensor t -> C_tensor (c_op t (Symbolic_shape.of_ints shape_arg))
    | Metal_tensor t -> Metal_tensor (metal_op t (Symbolic_shape.of_ints shape_arg))
    | Symbolic_tensor _ ->
...
let op_reshape t_in new_shape =
  ...
  shape_op1
    ...
    Nx_native.op_reshape Rune_metal.op_reshape Nx_c.op_reshape t_in new_shape_array
