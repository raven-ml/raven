...
let add ?out a b = Debug.with_context "add" (fun () -> T.add ?out a b)
let sub ?out a b = Debug.with_context "sub" (fun () -> T.sub ?out a b)
let mul ?out a b = Debug.with_context "mul" (fun () -> T.mul ?out a b)
let div ?out a b = Debug.with_context "div" (fun () -> T.div ?out a b)
let pow ?out a b = Debug.with_context "pow" (fun () -> T.pow ?out a b)
let mod_ ?out a b = Debug.with_context "mod" (fun () -> T.mod_ ?out a b)
let maximum ?out a b = Debug.with_context "maximum" (fun () -> T.maximum ?out a b)
let minimum ?out a b = Debug.with_context "minimum" (fun () -> T.minimum ?out a b)

(* Unary operations *)

let neg ?out x = Debug.with_context "neg" (fun () -> T.neg ?out x)
let conjugate x = Debug.with_context "conjugate" (fun () -> T.conjugate x)
let abs ?out x = Debug.with_context "abs" (fun () -> T.abs ?out x)
let sign ?out x = Debug.with_context "sign" (fun () -> T.sign ?out x)
let square ?out x = Debug.with_context "square" (fun () -> T.square ?out x)
let sqrt ?out x = Debug.with_context "sqrt" (fun () -> T.sqrt ?out x)
let recip ?out x = Debug.with_context "recip" (fun () -> T.recip ?out x)
let exp ?out x = Debug.with_context "exp" (fun () -> T.exp ?out x)
let log ?out x = Debug.with_context "log" (fun () -> T.log ?out x)
let sin ?out x = Debug.with_context "sin" (fun () -> T.sin ?out x)
let cos ?out x = Debug.with_context "cos" (fun () -> T.cos ?out x)
let tan ?out x = Debug.with_context "tan" (fun () -> T.tan ?out x)
...
let sum ?out ?axes ?keepdims x =
  Debug.with_context "sum" (fun () -> T.sum ?out ?axes ?keepdims x)

let max ?out ?axes ?keepdims x =
  Debug.with_context "max" (fun () -> T.max ?out ?axes ?keepdims x)

let min ?out ?axes ?keepdims x =
  Debug.with_context "min" (fun () -> T.min ?out ?axes ?keepdims x)

let mean ?out ?axes ?keepdims x =
  Debug.with_context "mean" (fun () -> T.mean ?out ?axes ?keepdims x)

let prod ?out ?axes ?keepdims x =
  Debug.with_context "prod" (fun () -> T.prod ?out ?axes ?keepdims x)

(* Linear algebra *)

let matmul ?out a b = Debug.with_context "matmul" (fun () -> T.matmul ?out a b)
let dot ?out a b = Debug.with_context "dot" (fun () -> T.dot ?out a b)
...
let outer ?out a b = Debug.with_context "outer" (fun () -> T.outer ?out a b)
...
let trace ?out ?offset a = Debug.with_context "trace" (fun () -> T.trace ?out ?offset a)
...
let where ?out cond if_true if_false =
  Debug.with_context "where" (fun () -> T.where ?out cond if_true if_false)
...
let minimum_s ?out a s = Debug.with_context "minimum_s" (fun () -> T.minimum_s ?out a s)

let rminimum_s ?out s a =
  Debug.with_context "rminimum_s" (fun () -> T.rminimum_s ?out s a)
...
let mod_s ?out a s = Debug.with_context "mod_s" (fun () -> T.mod_s ?out a s)

(* Bitwise operations *)

let bitwise_xor ?out a b =
  Debug.with_context "bitwise_xor" (fun () -> T.bitwise_xor ?out a b)

let bitwise_or ?out a b =
  Debug.with_context "bitwise_or" (fun () -> T.bitwise_or ?out a b)

let bitwise_and ?out a b =
  Debug.with_context "bitwise_and" (fun () -> T.bitwise_and ?out a b)

(* Logical operations *)

let logical_and ?out a b =
  Debug.with_context "logical_and" (fun () -> T.logical_and ?out a b)

let logical_or ?out a b =
  Debug.with_context "logical_or" (fun () -> T.logical_or ?out a b)

let logical_xor ?out a b =
  Debug.with_context "logical_xor" (fun () -> T.logical_xor ?out a b)

let logical_not ?out x = Debug.with_context "logical_not" (fun () -> T.logical_not ?out x)

(* Comparison operations *)

let cmplt ?out a b = Debug.with_context "cmplt" (fun () -> T.cmplt ?out a b)
let less ?out a b = Debug.with_context "less" (fun () -> T.less ?out a b)
let cmpne ?out a b = Debug.with_context "cmpne" (fun () -> T.cmpne ?out a b)
let not_equal ?out a b = Debug.with_context "not_equal" (fun () -> T.not_equal ?out a b)
let cmpeq ?out a b = Debug.with_context "cmpeq" (fun () -> T.cmpeq ?out a b)
let equal ?out a b = Debug.with_context "equal" (fun () -> T.equal ?out a b)
let greater ?out a b = Debug.with_context "greater" (fun () -> T.greater ?out a b)

let greater_equal ?out a b =
  Debug.with_context "greater_equal" (fun () -> T.greater_equal ?out a b)

let less_equal ?out a b =
  Debug.with_context "less_equal" (fun () -> T.less_equal ?out a b)

(* Already defined: neg, abs, sign, square, sqrt, recip, exp, log, sin, cos,
   tan *)
let bitwise_not ?out x = Debug.with_context "bitwise_not" (fun () -> T.bitwise_not ?out x)
let invert ?out x = Debug.with_context "invert" (fun () -> T.invert ?out x)
let sigmoid ?out x = Debug.with_context "sigmoid" (fun () -> T.sigmoid ?out x)
let rsqrt ?out x = Debug.with_context "rsqrt" (fun () -> T.rsqrt ?out x)
let asin ?out x = Debug.with_context "asin" (fun () -> T.asin ?out x)
let acos ?out x = Debug.with_context "acos" (fun () -> T.acos ?out x)
let atan ?out x = Debug.with_context "atan" (fun () -> T.atan ?out x)
let sinh ?out x = Debug.with_context "sinh" (fun () -> T.sinh ?out x)
let cosh ?out x = Debug.with_context "cosh" (fun () -> T.cosh ?out x)
let tanh ?out x = Debug.with_context "tanh" (fun () -> T.tanh ?out x)
let asinh ?out x = Debug.with_context "asinh" (fun () -> T.asinh ?out x)
let acosh ?out x = Debug.with_context "acosh" (fun () -> T.acosh ?out x)
let atanh ?out x = Debug.with_context "atanh" (fun () -> T.atanh ?out x)
let ceil ?out x = Debug.with_context "ceil" (fun () -> T.ceil ?out x)
let floor ?out x = Debug.with_context "floor" (fun () -> T.floor ?out x)
let round ?out x = Debug.with_context "round" (fun () -> T.round ?out x)
let isinf ?out x = Debug.with_context "isinf" (fun () -> T.isinf ?out x)
let isnan ?out x = Debug.with_context "isnan" (fun () -> T.isnan ?out x)
let isfinite ?out x = Debug.with_context "isfinite" (fun () -> T.isfinite ?out x)
let relu ?out x = Debug.with_context "relu" (fun () -> T.relu ?out x)
let log_sigmoid ?out x = Debug.with_context "log_sigmoid" (fun () -> T.log_sigmoid ?out x)
let exp2 ?out x = Debug.with_context "exp2" (fun () -> T.exp2 ?out x)
let log2 ?out x = Debug.with_context "log2" (fun () -> T.log2 ?out x)

(* More advanced operations *)

let lerp ?out a b weight = Debug.with_context "lerp" (fun () -> T.lerp ?out a b weight)
let lshift ?out a n = Debug.with_context "lshift" (fun () -> T.lshift ?out a n)
let rshift ?out a n = Debug.with_context "rshift" (fun () -> T.rshift ?out a n)

let clamp ?out ?min ?max x =
  Debug.with_context "clamp" (fun () -> T.clamp ?out ?min ?max x)

let clip ?out ?min ?max x = Debug.with_context "clip" (fun () -> T.clip ?out ?min ?max x)

(* Already defined: sum, max, min, prod, mean *)
let var ?out ?axes ?keepdims ?ddof x =
  Debug.with_context "var" (fun () -> T.var ?out ?axes ?keepdims ?ddof x)

let std ?out ?axes ?keepdims ?ddof x =
  Debug.with_context "std" (fun () -> T.std ?out ?axes ?keepdims ?ddof x)
...
let softmax ?out ?axes ?scale x =
  Debug.with_context "softmax" (fun () -> T.softmax ?out ?axes ?scale x)
...
let add_s ?out a s = Debug.with_context "add_s" (fun () -> T.add_s ?out a s)
