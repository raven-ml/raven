...
let foreign_malloc = Foreign.foreign "malloc" Ctypes.(size_t @-> returning (ptr void))
(* let foreign_free = Foreign.foreign "free" Ctypes.(ptr void @-> returning void) *)
let foreign_memcpy = Foreign.foreign "memcpy" 
...
module Renderer = struct
  ...
  let pending_kernels : (string, Ir.Lowered.graph_t) Hashtbl.t = Hashtbl.create 10
...
module Compiler = struct
  ...
  let compile_instruction context builder vars_table = function
    ...
    | Ir.Lowered.L_ALU { dst; op; args; dtype } ->
        ...
        let get_var v = 
          ...
          with Not_found -> failwith (Printf.sprintf "Variable v%d not found" v)
        in
        let llvalue = match op, args with
          | Ir.Lowered.Binary Ir.Add, [a; b] ->
              ...
              | _ ->
                  Llvm.build_add a_val b_val "add" builder
              end
          | Ir.Lowered.Binary Ir.Mul, [a; b] ->
              ...
              | _ ->
                  Llvm.build_mul a_val b_val "mul" builder
              end
          | Ir.Lowered.Binary Ir.Sub, [a; b] ->
              ...
              | _ ->
                  Llvm.build_sub a_val b_val "sub" builder
              end
          | Ir.Lowered.Binary Ir.Div, [a; b] ->
              ...
              | _ ->
                  Llvm.build_sdiv a_val b_val "div" builder
              end
          | Ir.Lowered.Binary Ir.Cmplt, [a; b] ->
              ...
              | _ ->
                  Llvm.build_icmp Llvm.Icmp.Slt a_val b_val "cmplt" builder
              end
          | Ir.Lowered.Unary Ir.Neg, [a] ->
              ...
              | _ ->
                  Llvm.build_neg a_val "neg" builder
              end
          ...
          | Ir.Lowered.Ternary Ir.Where, [cond; true_val; false_val] ->
              ...
              let true_val = get_var true_val in
              let false_val = get_var false_val in
              (* Ensure condition is i1 for select instruction *)
              let cond_i1 = 
                if Llvm.type_of cond_val = Llvm.i1_type context then
                  cond_val
                else
                  (* Convert to i1 by comparing with 0 *)
                  Llvm.build_icmp Llvm.Icmp.Ne cond_val 
                    (Llvm.const_int (Llvm.type_of cond_val) 0) "cond_bool" builder
              in
              Llvm.build_select cond_i1 true_val false_val "where" builder
          | Ir.Lowered.Ternary Ir.Mulacc, [a; b; c] ->
              let a_val = get_var a in
              let b_val = get_var b in
              let c_val = get_var c in
              let mul_result = match dtype with
                | Ir.Dtype.Any_Dtype Ir.Dtype.Float32 ->
                    Llvm.build_fmul a_val b_val "mul_tmp" builder
                | _ ->
                    Llvm.build_mul a_val b_val "mul_tmp" builder
              in
              begin match dtype with
              | Ir.Dtype.Any_Dtype Ir.Dtype.Float32 ->
                  Llvm.build_fadd mul_result c_val "mulacc" builder
              | _ ->
                  Llvm.build_add mul_result c_val "mulacc" builder
              end
    ...
    | Ir.Lowered.L_Load { dst; buf; idx; dtype; valid = _ } ->
        let ptr_val = 
          ...
          with Not_found -> failwith (Printf.sprintf "Buffer v%d not found" buf)
        in
        let idx_val = 
          try Hashtbl.find vars_table idx 
          with Not_found -> failwith (Printf.sprintf "Index v%d not found" idx)
        in
        (* Use uint8 for bools since that's how they're stored *)
        let actual_lltype = match dtype with
          | Ir.Dtype.Any_Dtype Ir.Dtype.Bool -> Llvm.i8_type context
          | _ -> dtype_to_llvm_type context dtype
        in
        let gep = Llvm.build_gep actual_lltype ptr_val [| idx_val |] "gep" builder in
        let loaded = Llvm.build_load actual_lltype gep "load" builder in
        (* Convert uint8 to bool if needed *)
        let final_value = match dtype with
          | Ir.Dtype.Any_Dtype Ir.Dtype.Bool ->
              (* Truncate i8 to i1 *)
              Llvm.build_trunc loaded (Llvm.i1_type context) "to_bool" builder
          | _ -> loaded
        in
    ...
    | Ir.Lowered.L_Store { buf; idx; src; valid = _ } ->
        ...
        let src_val = 
          ...
          with Not_found -> failwith (Printf.sprintf "Source v%d not found" src)
        in
        let elem_type = Llvm.type_of src_val in
        let gep = Llvm.build_gep elem_type ptr_val [| idx_val |] "gep" builder in
    ...
    | Ir.Lowered.L_Buffer { dtype; size; out } ->
        ...
        let lltype = dtype_to_llvm_type context dtype in
        let alloca = Llvm.build_array_alloca lltype 
          (Llvm.const_int (Llvm.i32_type context) size) "buffer" builder in
  ...
  let compile_kernel device_info lowered_ir kernel_name =
    ...
    let all_params = lowered_ir.Ir.Lowered.kernel_input_vars @ 
                     lowered_ir.Ir.Lowered.kernel_output_vars in
    let num_params = List.length all_params in
    (* Add extra parameter for size *)
    let param_types = Array.init (num_params + 1) (fun i ->
    ...
    List.iter (function
      | Ir.Lowered.L_Special { dst; kind = Ir.Special_index_kind.Global_task_idx _ } ->
    ...
    let process_with_loop global_idx_var =
      ...
      let current_idx = Llvm.build_load i32_type counter_ptr "main_idx" builder in
      let cond = Llvm.build_icmp Llvm.Icmp.Slt current_idx size_param "main_cond" builder in
      ...
      List.iter (function
        | Ir.Lowered.L_Special { dst = _; kind = Ir.Special_index_kind.Global_task_idx _ } ->
      ...
      let next_idx = Llvm.build_add current_idx (Llvm.const_int i32_type 1) "next_main_idx" builder in
    ...
    let rec process_instructions instrs =
      ...
      | Ir.Lowered.L_Range { idx; bound } :: rest ->
          ...
          (* Initialize loop counter *)
          let counter_ptr = Llvm.build_alloca (Llvm.i32_type context) "counter" builder in
          let zero = Llvm.const_int (Llvm.i32_type context) 0 in
          ignore (Llvm.build_store zero counter_ptr builder);
          ignore (Llvm.build_br loop_bb builder);
          ...
          (* Loop header *)
          Llvm.position_at_end loop_bb builder;
          let current_idx = Llvm.build_load (Llvm.i32_type context) counter_ptr "idx" builder in
          Hashtbl.add vars_table idx current_idx;
          let cond = Llvm.build_icmp Llvm.Icmp.Slt current_idx bound_val "loop_cond" builder in
          ignore (Llvm.build_cond_br cond body_bb end_bb builder);
          ...
          (* Increment counter and branch back *)
          let next_idx = Llvm.build_add current_idx (Llvm.const_int (Llvm.i32_type context) 1) "next_idx" builder in
          ignore (Llvm.build_store next_idx counter_ptr builder);
          ignore (Llvm.build_br loop_bb builder);
    ...
    (* If we have a global task index, wrap in a loop, otherwise process normally *)
    (match !has_global_task_idx with
    | Some global_idx_var ->
        process_with_loop global_idx_var
    | None ->
        process_instructions lowered_ir.instructions);
    ...
    (* Debug: print instructions and the module *)
    if false then begin
      Printf.eprintf "Instructions for %s:\n" kernel_name;
      List.iter (fun instr ->
        Printf.eprintf "  %s\n" (match instr with
          | Ir.Lowered.L_Range { idx; bound } -> Printf.sprintf "L_Range idx=v%d bound=v%d" idx bound
          | Ir.Lowered.L_EndRange -> "L_EndRange"
          | Ir.Lowered.L_Const { out; _ } -> Printf.sprintf "L_Const out=v%d" out
          | Ir.Lowered.L_Load { dst; buf; idx; _ } -> Printf.sprintf "L_Load dst=v%d buf=v%d idx=v%d" dst buf idx
          | Ir.Lowered.L_Store { buf; idx; src; _ } -> Printf.sprintf "L_Store buf=v%d idx=v%d src=v%d" buf idx src
          | Ir.Lowered.L_ALU { dst; args; _ } -> Printf.sprintf "L_ALU dst=v%d args=%s" dst (String.concat "," (List.map (fun v -> Printf.sprintf "v%d" v) args))
          | Ir.Lowered.L_Special { dst; _ } -> Printf.sprintf "L_Special dst=v%d" dst
          | _ -> "Other"
        )
      ) lowered_ir.instructions;
      Printf.eprintf "LLVM Module for %s:\n%s\n%!" kernel_name (Llvm.string_of_llmodule module_);
    end;
    
    (* Return the module and function name - module will be added to engine *)
    (module_, kernel_name)

  let compile ~device_info ~source_code ~options:_ =
    (* source_code is the kernel name from Renderer.render *)
    let kernel_name = source_code in
    
    (* Get the lowered IR that Renderer stored *)
    match Hashtbl.find_opt Renderer.pending_kernels kernel_name with
    | None ->
        Error (Printf.sprintf "No lowered IR found for kernel '%s'" kernel_name)
    | Some lowered_ir ->
        (* Compile the lowered IR *)
        let (module_, fn_name) = compile_kernel device_info lowered_ir kernel_name in
        Hashtbl.remove Renderer.pending_kernels kernel_name;
        (* Add the compiled module to the execution engine - it takes ownership *)
        Llvm_executionengine.add_module module_ device_info.engine;
        let native_artifact = { function_names = [fn_name] } in
        let entry_points = [kernel_name] in
...
module Runtime = struct
  let allocate_buffer ~device_info:_ ~size_in_bytes ~dtype =
    let ptr = 
      if size_in_bytes > 0 then
        ...
        Nativeint.zero
    in
    let native_buffer = { ptr; size_bytes = size_in_bytes; dtype = Ir.Dtype.Any_Dtype dtype } in
    Ok { Backend_intf.native_buffer; size_in_bytes; dtype }

  let copy_to_device ~dest_buffer ~host_data ~host_data_offset_bytes ~copy_size_bytes =
    if copy_size_bytes > 0 then begin
      let dest_ptr = Ctypes.(ptr_of_raw_address dest_buffer.Backend_intf.native_buffer.ptr) in
      let src_ptr = Ctypes.ptr_of_raw_address 
        (Nativeint.add host_data (Nativeint.of_int host_data_offset_bytes)) in
      ignore (foreign_memcpy dest_ptr src_ptr (Unsigned.Size_t.of_int copy_size_bytes))
    end;
    Ok ()

  let copy_from_device ~src_buffer ~host_dest_ptr ~device_data_offset_bytes ~copy_size_bytes =
    if copy_size_bytes > 0 then begin
      let src_ptr = Ctypes.ptr_of_raw_address 
        (Nativeint.add src_buffer.Backend_intf.native_buffer.ptr (Nativeint.of_int device_data_offset_bytes)) in
  ...
  let get_kernel ~artifact ~kernel_name =
    (* Check if kernel_name is in the list of functions *)
    if List.mem kernel_name artifact.Backend_intf.native_artifact.function_names then
      (* Just store the name, we'll look it up when launching *)
      let native_kernel = { name = kernel_name } in
      Ok { Backend_intf.native_kernel; name = kernel_name }
    else
      Error (Printf.sprintf "Kernel '%s' not found" kernel_name)
  ...
  let launch_kernel ?local_dims:_ ~device_info ~global_dims ~args kernel =
    ...
    (* Printf.eprintf "Launching kernel %s with size %d and %d args\n%!" 
      kernel.Backend_intf.native_kernel.name size (List.length args); *)
    
    (* Call function based on number of arguments (plus size parameter) *)
    let open Ctypes in
    (match ptr_list with
    | [p1] ->
        let fn_ptr = Llvm_executionengine.get_function_address 
          kernel.Backend_intf.native_kernel.name 
          (Foreign.funptr (ptr void @-> int @-> returning void)) engine in
        fn_ptr p1 size
    | [p1; p2] ->
        let fn_ptr = Llvm_executionengine.get_function_address 
          kernel.Backend_intf.native_kernel.name 
          (Foreign.funptr (ptr void @-> ptr void @-> int @-> returning void)) engine in
        fn_ptr p1 p2 size
    | [p1; p2; p3] ->
        let fn_ptr = Llvm_executionengine.get_function_address 
          kernel.Backend_intf.native_kernel.name 
          (Foreign.funptr (ptr void @-> ptr void @-> ptr void @-> int @-> returning void)) engine in
        fn_ptr p1 p2 p3 size
    | [p1; p2; p3; p4] ->
        let fn_ptr = Llvm_executionengine.get_function_address 
          kernel.Backend_intf.native_kernel.name 
          (Foreign.funptr (ptr void @-> ptr void @-> ptr void @-> ptr void @-> int @-> returning void)) engine in
        fn_ptr p1 p2 p3 p4 size
    | [p1; p2; p3; p4; p5] ->
        let fn_ptr = Llvm_executionengine.get_function_address 
          kernel.Backend_intf.native_kernel.name 
          (Foreign.funptr (ptr void @-> ptr void @-> ptr void @-> ptr void @-> ptr void @-> int @-> returning void)) engine in
        fn_ptr p1 p2 p3 p4 p5 size
    | [p1; p2; p3; p4; p5; p6] ->
        let fn_ptr = Llvm_executionengine.get_function_address 
          kernel.Backend_intf.native_kernel.name 
          (Foreign.funptr (ptr void @-> ptr void @-> ptr void @-> ptr void @-> ptr void @-> ptr void @-> int @-> returning void)) engine in
        fn_ptr p1 p2 p3 p4 p5 p6 size
    | _ ->
        failwith (Printf.sprintf "Unsupported number of arguments: %d" (List.length args))
    );
    Ok ()
