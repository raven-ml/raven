    ...
    let render_instruction ~name_of ~smem_prefix indent
      ...
      | L_ALU { dst; op; args; dtype } -> (
          ...
          | Ir.Lowered.Binary bop, [ a; b ] -> (
              ...
              | Shr ->
                  Printf.sprintf "%s%s %s = %s %s %s;" ind
                    (metal_of_dtype dtype) (v dst) (v a) (binop_str bop) (v b)
              | Max | Min ->
                  Printf.sprintf "%s%s %s = %s(%s, %s);" ind
          ...
          | Ir.Lowered.Ternary Where, [ cond; x; y ] ->
              Printf.sprintf "%s%s %s = %s ? %s : %s;" ind
                (metal_of_dtype dtype) (v dst) (v cond) (v x) (v y)
      ...
      | L_Define_Global { ptr; dtype; size } ->
          Printf.sprintf "%sdevice %s* %s[%d];" ind (metal_of_dtype dtype)
            (v ptr) size
      ...
      | L_Wmma { dst; a; b; c; m; n; k; dtype } ->
          (* Tensor core operations - Metal doesn't have native WMMA, use
             regular matmul *)
