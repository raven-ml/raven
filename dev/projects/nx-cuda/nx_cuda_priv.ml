...
(** Convert GPU array to host Nx_c.t *)
let to_host : type a b. (a, b) t -> (a, b) Nx_c.t =
 fun arr ->
  ...
  let layout = Nx_c.C_contiguous in
  let byte_order = Nx_c.Little_endian in
  let access_mode = Nx_c.Read_only in
  (* TODO: That doesn't work, the memory will be gced when host_data is
     deleted *)
  Nx_c.create_from_external host_data arr.dtype arr.shape layout byte_order
    access_mode

(** Overriding functions *)
let pp fmt arr = Nx_c.pp fmt (to_host arr)

let print arr = Nx_c.print (to_host arr)
let to_string arr = Nx_c.to_string (to_host arr)
