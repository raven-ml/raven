...
let binary_op_impl : type a b.
 ...
 fun context op a b out ->
  let a_dtype = Nx_core.dtype a.descriptor in
  let b_dtype = Nx_core.dtype b.descriptor in
  if a_dtype <> b_dtype then failwith "Dtype mismatch in binary operation";
  let metal_dtype = nx_dtype_to_metal_dtype a_dtype in
  let a_shape = Nx_core.shape a.descriptor in
  let b_shape = Nx_core.shape b.descriptor in
  let a_strides = Nx_core.strides a.descriptor in
  let b_strides = Nx_core.strides b.descriptor in
  let out_shape = Nx_core.shape (descriptor out) in
...
let sum : type a b.
 ...
 fun context ~axes ~keepdims arr out ->
  let ctx = context in
  let input_dtype = Nx_core.dtype arr.descriptor in
  let metal_dtype = nx_dtype_to_metal_dtype input_dtype in
  let input_shape = Nx_core.shape arr.descriptor in
  let input_strides = Nx_core.strides arr.descriptor in
    ...
    let kernel_name, buffers, grid_size, thread_group_size =
      if can_use_tiled_contiguous then
        ...
        let buffer_out_strides =
          create_int64_buffer ctx (Nx_core.compute_c_strides out_shape)
        in
