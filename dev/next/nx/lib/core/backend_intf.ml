...
module type S = sig
  ...
  (** Create a fresh backend context. *)
  ...
  val op_add : context -> ('a, 'b) t -> ('a, 'b) t -> ('a, 'b) t
  ...
  val op_expand : context -> ('a, 'b) t -> int array -> ('a, 'b) t
  (** Broadcast dimensions of size 1 to a new shape. *)

  val op_reshape : context -> ('a, 'b) t -> int array -> ('a, 'b) t
  (** Change the logical shape without moving data. *)

  val op_permute : context -> ('a, 'b) t -> int array -> ('a, 'b) t
  (** Reorder dimensions according to [axes]. *)

  val op_pad : context -> ('a, 'b) t -> (int * int) array -> 'a -> ('a, 'b) t
  (** Pad with [fill_value] using the given configuration. *)

  val op_shrink : context -> ('a, 'b) t -> (int * int) array -> ('a, 'b) t
  (** Slice according to the given start/stop pairs. *)

  val op_flip : context -> ('a, 'b) t -> bool array -> ('a, 'b) t
  (** Flip dimensions where the boolean array is [true]. *)

  val op_cat : context -> ('a, 'b) t list -> int -> ('a, 'b) t
