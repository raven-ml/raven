...
let empty : type a b. (a, b) Dtype.t -> int array -> (a, b) t =
 fun dt shp ->
  ...
  { dtype = dt; buffer = buf; view = vw }

let full : type a b. (a, b) Dtype.t -> int array -> a -> (a, b) t =
 fun dt shp value ->
  let t = empty dt shp in
...
let copy : type a b. (a, b) t -> (a, b) t =
 fun t_src ->
  ...
  let new_t = { dtype = dtype t_src; buffer = new_buffer; view = new_view } in

  if total_elements = 0 then new_t (* Handle zero-element tensor *)
  else if is_contiguous t_src && View.offset src_view = 0 then (
    (* If source is fully C-contiguous, use fast Bigarray.Array1.blit *)
