...
(* Independent type definitions for the IR *)
module Types = struct
  (* Phantom type used in GADTs to carry element type and layout information *)
  type ('a_elt, 'b_layout_phantom) el_layout_phantom =
    'a_elt * 'b_layout_phantom
  ...
  module Ir_Dtype = struct
    ...
    type ('a_elt, 'b_layout_phantom) t =
      ...
      (* Add other necessary types like Float16, Int64, Uint32, etc. as
         needed *)
    ...
    type any = Any_Ir_Dtype : ('a, 'b) t -> any [@@unboxed]

    let any_to_string (Any_Ir_Dtype dt) = to_string dt
  end
  ...
  module Special_index_kind = struct
    ...
    (* This mirrors Backend_intf.special_kind previously used from Nx
       ecosystem *)
  end
end

open Types (* Makes Ir_Dtype, Var, Special_index_kind directly accessible *)

type any_dtype = Ir_Dtype.any
(** Existential wrapper for Ir_Dtype.t, allowing heterogeneous collections. *)

let any_dtype_to_string = Ir_Dtype.any_to_string

type var_metadata = {
  dtype : any_dtype; (* Uses the self-contained Ir_Dtype.any *)
...
type ('a_elt, 'b_layout_phantom) node_t =
  | Buffer : {
      dtype : ('a_elt, 'b_layout_phantom) Ir_Dtype.t;
...
module Lowered = struct
  ...
  (** Defines the low-level, imperative UOp-like instructions. GADT parameters
      ...
      produces a typed scalar. For effectful ops like Store, it's Ir_Dtype.Unit.
  *)
  type ('a_elt, 'b_layout_phantom) instruction_t =
    ...
    | LI_Range : {
        ...
        -> (Ir_Dtype.int_elt, Ir_Dtype._x_layout_phantom) instruction_t
    | LI_Special_Index : {
        name_hint : string;
    ...
    | LI_Store : {
        ...
        -> (Ir_Dtype.unit_elt, Ir_Dtype.unit_layout_phantom) instruction_t
    | LI_Scalar_ALU : {
