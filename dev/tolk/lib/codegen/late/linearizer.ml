...
(** Linearize a Kernel DAG into a Program SSA instruction sequence.

    Algorithm: split multi-range Ends, compute CFG context for range ordering,
    priority-based topological sort, then emit linear Program instructions. *)
...
(** Split multi-range End nodes into nested single-range Ends.
    Ranges are sorted by kind rank (reversed for nesting: outermost first). *)
let split_ends root =
        ...
        let result =
          ...
          | End { value; ranges } ->
              ...
              let ranges =
                List.map go ranges
                |> List.sort (fun a b ->
                       let ka = range_sort_key a and kb = range_sort_key b in
                       compare kb ka)
              in
              List.fold_left
                (fun v r -> K.end_ ~value:v ~ranges:[ r ])
                value ranges
...
(** For each End, find its single range ref. *)
let end_range node =
  match K.view node with
  | End { ranges = [ r ]; _ } -> (
      match K.view r with Range _ -> Some r | _ -> None)
  | _ -> None

let is_in_backward_slice ~target node =
  let visited = K.Ref_tbl.create 32 in
  let rec walk n =
    if n == target then true
    else if K.Ref_tbl.mem visited n then false
    else begin
      K.Ref_tbl.replace visited n ();
      List.exists walk (K.children n)
    end
  in
  walk node
...
let build_cfg_context topo =
  let deps : unit K.Ref_tbl.t K.Ref_tbl.t = K.Ref_tbl.create (List.length topo) in
  let is_end n = match K.view n with End _ -> true | _ -> false in
  let nesting = K.Ref_tbl.create 32 in
  List.iter
    (fun node ->
      let children_deps : unit K.Ref_tbl.t = K.Ref_tbl.create 16 in
      List.iter
        (fun child ->
          match K.Ref_tbl.find_opt deps child with
          | Some s -> K.Ref_tbl.iter (fun k () -> K.Ref_tbl.replace children_deps k ()) s
          | None -> ())
        (K.children node);
      (match K.view node with
      | End _ | Sink _ ->
          K.Ref_tbl.iter
            (fun x () ->
              if is_end x && not (K.Ref_tbl.mem nesting x) then
                let should_nest =
                  match K.view node, end_range node with
                  | Sink _, _ -> true
                  | End _, Some rr -> (
                      match K.Ref_tbl.find_opt deps x with
                      | Some x_deps -> K.Ref_tbl.mem x_deps rr
                      | None -> false)
                  | _ -> false
                in
                if should_nest then K.Ref_tbl.replace nesting x node)
            children_deps
      | _ -> ());
      (match K.view node with
      | Range _ | End _ -> K.Ref_tbl.replace children_deps node ()
      | _ -> ());
      K.Ref_tbl.replace deps node children_deps)
    topo;
  let siblings = K.Ref_tbl.create 32 in
  K.Ref_tbl.iter
    (fun child parent ->
      let current =
        Option.value ~default:[] (K.Ref_tbl.find_opt siblings parent)
      in
      K.Ref_tbl.replace siblings parent (child :: current))
    nesting;
  let edges = K.Ref_tbl.create 16 in
  let dep_count ends node =
    let node_deps : unit K.Ref_tbl.t =
      match K.Ref_tbl.find_opt deps node with
      | Some s -> s
      | None -> K.Ref_tbl.create 0
    in
    List.fold_left
      (fun acc u -> if K.Ref_tbl.mem node_deps u then acc + 1 else acc)
      0 ends
  in
  let add_edge range_node pred_node =
    if is_in_backward_slice ~target:range_node pred_node then ()
    else K.Ref_tbl.replace edges range_node pred_node
  in
  K.Ref_tbl.iter
    (fun parent ends ->
      let order =
        List.sort
          (fun a b -> compare (dep_count ends a) (dep_count ends b))
          ends
      in
...
(* Big-nat: int list with base 1_000_000_000, least-significant digit first. *)
type big_nat = int list

let big_base = 1_000_000_000L

let normalize digits =
  let rec drop = function [] -> [] | 0 :: rest -> drop rest | xs -> xs in
  match List.rev digits |> drop |> List.rev with [] -> [ 0 ] | xs -> xs
    ...
    let rec go carry acc = function
      | [] ->
          if carry = 0L then normalize (List.rev acc)
            ...
            go
              Int64.(div carry big_base)
              (Int64.to_int Int64.(rem carry big_base) :: acc)
              []
      | d :: rest ->
          let prod =
            Int64.add
              (Int64.mul (Int64.of_int d) (Int64.of_int factor))
              carry
          in
          go
            Int64.(div prod big_base)
            (Int64.to_int Int64.(rem prod big_base) :: acc)
            rest
    in
    go 0L [] digits
...
(* Tuplize: recursive structural key for deterministic tie-breaking.
   When two nodes have equal priority, their structural representation
   breaks the tie. *)
...
let view_ordinal_and_arg view =
  ...
  | K.Sink _ -> (15, TA_none)
  | K.After _ -> (16, TA_none)
  | K.Group _ -> (17, TA_none)
  | K.Gep { idx; _ } -> (18, TA_int idx)
  | K.Vectorize _ -> (19, TA_none)
  | K.Index _ -> (20, TA_none)
  | K.Load _ -> (21, TA_none)
  | K.Store _ -> (22, TA_none)
  | K.Wmma { name; _ } -> (23, TA_str name)
  | K.Cast _ -> (24, TA_none)
  | K.Bitcast _ -> (25, TA_none)
...
let dtype_ordinal node =
  ...
  | Some dt -> Hashtbl.hash (Dtype.to_string dt)
  | None -> 0

(** Build a structural comparison function over the topo list.
    Each node gets a cached [tuplize_key] computed bottom-up. *)
let build_compare_tuplize topo =
  let cache : tuplize_key K.Ref_tbl.t = K.Ref_tbl.create (List.length topo) in
  List.iter
    (fun node ->
      let ord, arg = view_ordinal_and_arg (K.view node) in
      let dt = dtype_ordinal node in
      let children_keys =
        List.map
          (fun c ->
          ...
          (K.children node)
      in
      K.Ref_tbl.replace cache node (TK (ord + (dt * 1000), arg, children_keys)))
    topo;
  fun a b ->
    let ka = K.Ref_tbl.find cache a and kb = K.Ref_tbl.find cache b in
    compare ka kb

(** Compute the enclosing ranges for each node using the linearized topo. *)
      ...
      let from_children =
        List.fold_left
          (fun acc child ->
            ...
            | Some rs -> List.fold_left (fun a r -> if List.memq r a then a else r :: a) acc rs
            | None -> acc)
          [] (children_fn node)
      in
      let ranges =
...
(** Extract the static size of a Range node for run_count calculation. *)
...
let compute_priorities topo children_fn =
  ...
  let priority_of node =
    let run_count =
      List.fold_left
        (fun acc r -> big_mul_small acc (range_extent r))
        [ 1 ] (ranges node)
    in
    let op_priority, extra =
      ...
      | Define_local _ -> (-18, None_)
      | Define_reg _ -> (-17, None_)
      | Load _ -> (-1, None_)
      | Store _ -> (1, None_)
      | Range _ -> (5, None_)
      | End _ -> (-5, None_)
      | _ -> (0, None_)
    in
    { run_count; op_priority; extra }
  in
  let priorities = K.Ref_tbl.create (List.length topo) in
  List.iter
    (fun node -> K.Ref_tbl.replace priorities node (priority_of node))
    topo;
  priorities

(** Max-heap topological sort using priority-based nkey ordering. *)
let linearize_order topo children_fn (cfg : cfg_context) =
  ...
  (* Assign nkey: sorted index by priority, with tuplize as tie-breaker. *)
  let sorted =
    List.stable_sort
      (fun a b ->
        let c =
          compare_priority
            (K.Ref_tbl.find priorities a)
            (K.Ref_tbl.find priorities b)
        in
        if c <> 0 then c else compare_tuplize a b)
      topo
  in
  let nkey = K.Ref_tbl.create (List.length topo) in
  List.iteri (fun i node -> K.Ref_tbl.replace nkey node i) sorted;
  (* Compute out-degree. *)
  let out_degree = K.Ref_tbl.create (List.length topo) in
  ...
  List.iter
    (fun u ->
      List.iter
        (fun v ->
          let d =
            match K.Ref_tbl.find_opt out_degree v with Some d -> d | None -> 0
          in
          K.Ref_tbl.replace out_degree v (d + 1))
        (children_with_cfg u))
    topo;
  (* Assign a unique topo index for tie-breaking in the heap. *)
  let topo_id = K.Ref_tbl.create (List.length topo) in
  List.iteri (fun i node -> K.Ref_tbl.replace topo_id node i) topo;
  let get_topo_id node =
    match K.Ref_tbl.find_opt topo_id node with Some i -> i | None -> 0
  in
  (* Find the root (sink). *)
  let sink =
    match List.rev topo with
    | x :: _ -> x
    | [] -> failwith "Linearizer: empty topo"
  in
  (* Max-heap topo sort. Use a Set as a priority queue.
     Key is (nkey, topo_id) to break ties deterministically. *)
  let module Heap = Set.Make (struct
    type t = int * int * K.t
  ...
  let get_nkey node =
    match K.Ref_tbl.find_opt nkey node with Some k -> k | None -> 0
  in
  let heap =
    ref (Heap.singleton (get_nkey sink, get_topo_id sink, sink))
  in
  let result = ref [] in
  while not (Heap.is_empty !heap) do
    ...
    List.iter
      (fun v ->
        ...
        if d' = 0 then
          heap := Heap.add (get_nkey v, get_topo_id v, v) !heap)
      (children_with_cfg u)
  done;
  !result
...
let err_invalid_index = err_unlowered "Invalid_index"
let err_ptrcat = err_unlowered "Ptrcat"
let err_cat = err_unlowered "Cat"
let err_reduce = err_unlowered "Reduce"
let err_unroll = err_unlowered "Unroll"
let err_contract = err_unlowered "Contract"
let err_bufferize = err_unlowered "Bufferize"
...
(** Walk through After/End/Group to find the underlying value dtype. *)
...
(** Walk through After/Cast/Bitcast to find whether the underlying Index
    carries a gate. *)
...
let rec emit_ensure_range ctx node =
  ...
  | Range { size; dtype; axis; kind } ->
      ...
      | Some _ -> ()
      | None ->
          let size_id = emit_lookup ctx size in
          let id =
            P.emit ctx.builder (Range { size = size_id; dtype; axis; kind })
          in
...
let emit_node ctx node =
  match K.view node with
  | Sink _ -> ()
  | Group { srcs = src :: _ } -> emit_set ctx node (emit_map_id ctx src)
  | Group { srcs = [] } -> failwith err_empty_group
  | After { src; deps } ->
      if K.is_ptr src then emit_set ctx node (emit_map_id ctx src)
      else
        let dtype =
          match after_dtype src with
          | Some dt -> dt
          | None -> failwith err_after_no_dtype
        in
        emit_set_emit ctx node
          (After
             { src = emit_map_id ctx src; deps = List.map (emit_map_id ctx) deps; dtype })
  | Param { idx; dtype } ->
      emit_set_emit ctx node (Param { idx; dtype })
  | Param_image { idx; dtype; width; height } ->
      emit_set_emit ctx node (Param_image { idx; dtype; width; height })
  | Define_local { size; dtype } ->
      emit_set_emit ctx node (Define_local { size; dtype })
  | Define_reg { size; dtype } ->
      emit_set_emit ctx node (Define_reg { size; dtype })
  | Define_var { name; lo; hi; dtype } ->
      emit_set_emit ctx node (Define_var { name; lo; hi; dtype })
  | Const { value; dtype } ->
      emit_set_emit ctx node (Const { value; dtype })
  | Index { ptr; idxs; gate; dtype } ->
      emit_set_emit ctx node
        (Index
           {
             ptr = emit_map_id ctx ptr;
             idxs = List.map (emit_map_id ctx) idxs;
             gate = Option.map (emit_map_id ctx) gate;
             dtype;
           })
  | Load { src; alt; dtype } ->
      ...
      | _ -> ());
      emit_set_emit ctx node
        (Load { src = emit_map_id ctx src; alt = Option.map (emit_map_id ctx) alt; dtype })
  | Store { dst; value; _ } ->
      emit_set_emit ctx node (Store { dst = emit_map_id ctx dst; value = emit_map_id ctx value })
  | End { value; ranges = [] } ->
      emit_set ctx node (emit_map_id ctx value)
  | End { value; ranges = [ range ] } ->
      let range_id = emit_ensure_range ctx range in
      ignore (P.emit ctx.builder (End_range { range = range_id }));
  ...
  | Unary { op; src; dtype } ->
      emit_set_emit ctx node (Unary { op; src = emit_map_id ctx src; dtype })
  | Binary { op; lhs; rhs; dtype } ->
      emit_set_emit ctx node
        (Binary { op; lhs = emit_map_id ctx lhs; rhs = emit_map_id ctx rhs; dtype })
  | Ternary { op; a; b; c; dtype } ->
      emit_set_emit ctx node
        (Ternary { op; a = emit_map_id ctx a; b = emit_map_id ctx b; c = emit_map_id ctx c; dtype })
  | Cast { src; dtype } ->
      emit_set_emit ctx node (Cast { src = emit_map_id ctx src; dtype })
  | Bitcast { src; dtype } ->
      emit_set_emit ctx node (Bitcast { src = emit_map_id ctx src; dtype })
  | Vectorize { srcs; dtype } ->
      emit_set_emit ctx node (Vectorize { srcs = List.map (emit_map_id ctx) srcs; dtype })
  | Gep { src; idx; dtype } ->
      emit_set_emit ctx node (Gep { src = emit_map_id ctx src; idx; dtype })
  | Barrier ->
      emit_set_emit ctx node Barrier
  | Special { dim; size; dtype } ->
      emit_set_emit ctx node (Special { dim; size = emit_map_id ctx size; dtype })
  | Wmma
      {
        name;
        a;
  ...
  | Custom { fmt; args } ->
      emit_set_emit ctx node (Custom { fmt; args = List.map (emit_map_id ctx) args })
  | Custom_inline { fmt; args; dtype } ->
      emit_set_emit ctx node
        (Custom_inline { fmt; args = List.map (emit_map_id ctx) args; dtype })
  | Invalid_index _ -> failwith err_invalid_index
  | Ptrcat _ -> failwith err_ptrcat
  | Cat _ -> failwith err_cat
  | Reduce _ -> failwith err_reduce
  | Unroll _ -> failwith err_unroll
  | Contract _ -> failwith err_contract
  | Bufferize _ -> failwith err_bufferize
