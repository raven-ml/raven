...
let test_from_string_no_header () =
  ...
  let names = column_names df in
  Alcotest.(check (list string)) "default names" ["col0"; "col1"; "col2"] names
...
let test_from_string_dtype_spec () =
  ...
  | Some arr -> 
      check_bool "bool values" true (arr = [|true; false; true|])
...
let test_to_string_no_header () =
  let df = create [
    ("name", Col.string_list ["Alice"]);
    ("age", Col.int32_list [25l])
  ] in
