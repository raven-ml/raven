...
- `lib-simd/` - SIMD intrinsics library (`nx-oxcaml.simd`)
...
all dune commands MUST be run from the `dev/nx-oxcaml` directory with `--root .` to get an isolated build that does not conflict with the parent raven project.
...
- ALWAYS run dune commands from the `dev/nx-oxcaml` directory
