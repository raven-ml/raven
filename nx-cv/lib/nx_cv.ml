...
type uint8_t = Nx.uint8_t
type int16_t = Nx.int16_t
type float32_t = Nx.float32_t
...
let flip_axis img axis =
  ...
  if Array.length shape <= axis || shape.(axis) <= 1 then img
    ...
    slice_shape.(axis) <- 1;
    let temp_slice = Nx.empty (Nx.dtype img) slice_shape in
    ...
    for i = 0 to (n / 2) - 1 do
      ...
      stops2.(axis) <- i' + 1;
      try
        let slice1_view = Nx.slice starts1 stops1 img_copy in
        let slice2_view = Nx.slice starts2 stops2 img_copy in
        Nx.blit slice1_view temp_slice;
        Nx.blit slice2_view slice1_view;
        Nx.blit temp_slice slice2_view
      with e ->
        Printf.eprintf
...
let flip_vertical img = if Nx.ndim img < 1 then img else flip_axis img 0
let flip_horizontal img = if Nx.ndim img < 2 then img else flip_axis img 1
...
let crop ~y ~x ~height ~width img =
  ...
  | `Gray (h, w) ->
      ...
      then
        ...
        Nx.copy (Nx.slice starts stops img)
  | `Color (h, w, c) ->
      if
        y < 0 || x < 0 || height <= 0 || width <= 0
        || y + height > h
...
let to_grayscale img =
  ...
  | `Color (h, w, c) ->
      if c <> 3 then
        ...
        let r = Nx.slice [| 0; 0; 0 |] [| h; w; 1 |] img_f in
        let g = Nx.slice [| 0; 0; 1 |] [| h; w; 2 |] img_f in
        let b = Nx.slice [| 0; 0; 2 |] [| h; w; 3 |] img_f in
        let r_scaled = Nx.mul r (Nx.scalar Nx.float32 0.299) in
        let g_scaled = Nx.mul g (Nx.scalar Nx.float32 0.587) in
        let b_scaled = Nx.mul b (Nx.scalar Nx.float32 0.114) in
        let gray_f = Nx.add r_scaled g_scaled in
        let gray_f = Nx.add_inplace gray_f b_scaled in
        let gray_f_2d = Nx.reshape [| h; w |] gray_f in
        Nx.astype Nx.uint8 gray_f_2d
...
let swap_channels img =
  ...
  | `Color (h, w, c) ->
      if c < 3 then img
        ...
        let new_img = Nx.copy img in
        let starts_c0 = [| 0; 0; 0 |] in
        let stops_c0 = [| h; w; 1 |] in
        let starts_c2 = [| 0; 0; 2 |] in
        let stops_c2 = [| h; w; 3 |] in
        let chan0_orig_view = Nx.slice starts_c0 stops_c0 img in
        let chan2_orig_view = Nx.slice starts_c2 stops_c2 img in
        let chan0_new_view = Nx.slice starts_c0 stops_c0 new_img in
        let chan2_new_view = Nx.slice starts_c2 stops_c2 new_img in
        Nx.blit chan2_orig_view chan0_new_view;
        Nx.blit chan0_orig_view chan2_new_view;
        new_img
...
let to_float (img : uint8_t) =
  if Nx.dtype img <> Nx.uint8 then failwith "to_float requires uint8 input"
  else
    let float_img = Nx.astype Nx.float32 img in
    let s = Nx.scalar Nx.float32 (1.0 /. 255.0) in
    let _ = Nx.mul_inplace float_img s in
    float_img

let to_uint8 (img : float32_t) =
  if Nx.dtype img <> Nx.float32 then failwith "to_uint8 requires float32 input"
  else
    let s_zero = Nx.scalar Nx.float32 0.0 in
    let s_one = Nx.scalar Nx.float32 1.0 in
    let clipped_img = Nx.maximum s_zero (Nx.minimum s_one img) in
    let s_255 = Nx.scalar Nx.float32 255.0 in
    let scaled_img = Nx.mul clipped_img s_255 in
    Nx.astype Nx.uint8 scaled_img
...
let resize ?(interpolation = Nearest) ~height:out_h ~width:out_w img =
  ...
  let dtype = Nx.dtype img in
  match get_dims img with
  | `Gray (in_h, in_w) ->
      let out_img = Nx.empty dtype [| out_h; out_w |] in
      let data_out = Nx.data out_img in
      (match interpolation with
      | Nearest ->
          let data_in = Nx.data img in
      ...
      | Linear ->
          ...
          let out_img_f = Nx.empty Nx.float32 [| out_h; out_w |] in
          let data_in_f = Nx.data img_f in
          let data_out_f = Nx.data out_img_f in
          let h_scale = float in_h /. float out_h in
          let w_scale = float in_w /. float out_w in
      ...
      out_img
  | `Color (in_h, in_w, c) ->
      let out_img = Nx.empty dtype [| out_h; out_w; c |] in
      let data_out = Nx.data out_img in
      (match interpolation with
      | Nearest ->
          let data_in = Nx.data img in
      ...
      | Linear ->
          let img_f = to_float img in
          let out_img_f = Nx.empty Nx.float32 [| out_h; out_w; c |] in
          let data_in_f = Nx.data img_f in
          let data_out_f = Nx.data out_img_f in
          let h_scale = float in_h /. float out_h in
          let w_scale = float in_w /. float out_w in
...
let generate_gaussian_kernel size sigma =
  ...
  in
  let kernel = Nx.empty Nx.float32 [| size |] in
  ...
  for i = 0 to size - 1 do
    ...
    Nx.set_item [| i |] value kernel;
    sum := !sum +. value
  done;
  (if !sum > 0.0 then
     let s = Nx.scalar Nx.float32 (1.0 /. !sum) in
     let _ = Nx.mul_inplace kernel s in
     ());
  kernel
...
let convolve1d_f32 axis (kernel : float32_t) (img_f32 : float32_t) : float32_t =
  ...
  in
  let ksize = Nx.dim 0 kernel in
  let radius = ksize / 2 in
  let out_img_f32 = Nx.empty_like img_f32 in
  let data_in = Nx.data img_f32 in
  let data_out = Nx.data out_img_f32 in
  let data_kernel = Nx.data kernel in
...
let gaussian_blur : type a b.
 ...
 fun ~ksize:(kh, kw) ~sigmaX ?sigmaY img ->
  ...
  match Nx.dtype img with
  | Nx.UInt8 -> to_uint8 blurred_final_f32
  | Nx.Float32 -> blurred_final_f32
  | _ -> failwith "Unsupported image type for gaussian_blur"
...
let threshold ~thresh ~maxval ~type_ (img : uint8_t) : uint8_t =
  ...
  | `Gray (h, w) ->
      ...
      let out_img = Nx.empty_like img in
      let data_in = Nx.data img in
      let data_out = Nx.data out_img in
      let row_stride = w in
      for i = 0 to h - 1 do
...
let box_filter : type a b. ksize:int * int -> (a, b) Nx.t -> (a, b) Nx.t =
 fun ~ksize:(kh, kw) img ->
  ...
  let kernelX = Nx.full Nx.float32 [| kw |] kernel_val_x in
  let kernelY = Nx.full Nx.float32 [| kh |] kernel_val_y in
  ...
  match Nx.dtype img with
  | Nx.UInt8 -> to_uint8 filtered_final_f32
  | Nx.Float32 -> filtered_final_f32
  | _ -> failwith "Unsupported image type for gaussian_blur"
...
let get_structuring_element ~shape ~ksize:(kh, kw) =
  if kh <= 0 || kw <= 0 then invalid_arg "Kernel dimensions must be positive";
  let kernel = Nx.zeros Nx.uint8 [| kh; kw |] in
...
let sobel_kernel dx dy ksize =
  ...
  match (dx, dy) with
  | 1, 0 -> Nx.create Nx.int16 [| 3; 3 |] [| -1; 0; 1; -2; 0; 2; -1; 0; 1 |]
  | 0, 1 -> Nx.create Nx.int16 [| 3; 3 |] [| -1; -2; -1; 0; 0; 0; 1; 2; 1 |]
  | _ -> failwith "Sobel requires dx=1, dy=0 or dx=0, dy=1"

let convolve2d_int16_from_uint8 (kernel : int16_t) (img : uint8_t) : int16_t =
  let kh, kw =
    match Nx.shape kernel with [| kh; kw |] -> (kh, kw) | _ -> failwith "K2D"
  in
  ...
  | `Gray (h, w) ->
      let out_img = Nx.empty Nx.int16 [| h; w |] in
      let data_in = Nx.data img in
      let data_out = Nx.data out_img in
      let data_kernel = Nx.data kernel in
      let row_stride = w in
...
let canny ~threshold1 ~threshold2 ?(ksize = 3) (img : uint8_t) : uint8_t =
  ...
  let gx_f = Nx.astype Nx.float32 gx in
  let gy_f = Nx.astype Nx.float32 gy in

  (* Calculate Magnitude and Angle using vectorized ops *)
  let mag = Nx.sqrt (Nx.add (Nx.square gx_f) (Nx.square gy_f)) in
  let h, w =
    match Nx.shape img with [| h; w |] -> (h, w) | _ -> failwith "C2D"
  in
  let angle = Nx.empty Nx.float32 [| h; w |] in
  let data_angle = Nx.data angle in
  let data_gx_f = Nx.data gx_f in
  let data_gy_f = Nx.data gy_f in
  let row_stride = w in
  for i = 0 to h - 1 do
    let i_offset = i * row_stride in
    for j = 0 to w - 1 do
  ...
  let nms_img = Nx.zeros_like mag in
  let data_mag = Nx.data mag in
  let data_angle = Nx.data angle in
  let data_nms = Nx.data nms_img in
  ...
  let edge_map = Nx.zeros Nx.uint8 [| h; w |] in
  let data_edge = Nx.data edge_map in
  let data_nms = Nx.data nms_img in
