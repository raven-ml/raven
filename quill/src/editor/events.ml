...
let handle_remove ~is_backspace mounted_app dom_el =
  ...
  in
  let model = Vdom_blit.get mounted_app in
  let md = Model.md_of_model model in
  ...
  in
  if new_md <> md then (
    let new_document = Model.document_of_md new_md in
...
let handle_enter mounted_app dom_el =
  ...
  | Some editor_div ->
      ...
      let doc = Model.document_of_md (String.trim new_md) in
      let doc = Model.normalize_blanklines doc in
