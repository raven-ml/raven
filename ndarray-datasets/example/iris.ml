...
let nd_of_float_array data =
  Ndarray.create Ndarray.float32 [| Array.length data |] data

let get_class_features features labels target_label feature_idx1 feature_idx2 =
  let n_samples = (Ndarray.shape features).(0) in
  let indices = List.init n_samples (fun i -> i) in

  let filtered_pairs =
    List.filter_map
      (fun i ->
        let label = Ndarray.get_item [| i; 0 |] labels in
        if label = target_label then
          let f1 = Ndarray.get_item [| i; feature_idx1 |] features in
          let f2 = Ndarray.get_item [| i; feature_idx2 |] features in
          Some (f1, f2)
        else None)
      indices
  in
