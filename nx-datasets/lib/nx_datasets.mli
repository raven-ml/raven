(** nx-datasets: Dataset loading utilities for nx.

    This module provides functions to load common machine learning datasets as
    [Nx]s. Data files are downloaded and cached in the platform-specific cache
    directory (see [Dataset_utils.get_cache_dir]). *)

val load_mnist : unit -> (Nx.uint8_t * Nx.uint8_t) * (Nx.uint8_t * Nx.uint8_t)
(** Load the MNIST handwritten digits dataset.

    This function downloads (if necessary) and returns the MNIST dataset as nxs
    for training and test sets. Each image is represented as a uint8 tensor of
    shape [|num_samples; height; width; 1|], and labels as a uint8 tensor of
    shape [|num_samples; 1|].
...
(** Load the CIFAR-10 image classification dataset.
    ...
    nxs. Images are uint8 tensors of shape [|num_samples; height; width; 3|],
    and labels are uint8 tensors of shape [|num_samples; 1|].
...
val load_breast_cancer : unit -> Nx.float64_t * (int, Bigarray.int_elt) Nx.t
