...
(* Network: Linear(4 -> 128) -> ReLU -> Linear(128 -> 128) -> ReLU -> Linear(128
   -> 2), as a plain record of Linear layers with a hand-written [walk] (the
   Nx.Ptree.S contract). *)
...
module Q = struct
  ...
  let walk c { l1; l2; l3 } =
    ...
    let l3 = field c "l3" Linear.walk l3 in
    { l1; l2; l3 }
...
let count_parameters params =
  Nx.Ptree.fold q_tree (fun _ t n -> n + Nx.numel t) params 0
