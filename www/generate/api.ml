(* API reference generation from odoc HTML output.
   ...
   to match the site URL scheme, and produces pages wrapped in the site
   template. *)

let odoc_dir = Filename.concat ".." "_doc/_html"

(* Libraries to include per package.
   Each entry is (library_name, entry_module_name). The library name is
   displayed in the sidebar; the module name is the odoc directory name. *)
let libraries =
  [
    ( "nx",
      [
...
(* Walk [dir] recursively, collecting all [index.html] files.
   Returns [(rel_path, full_path)] where [rel_path] is relative to [dir]. *)
let rec walk_modules dir rel =
  let full = Filename.concat dir rel in
  let index = Filename.concat full "index.html" in
  let self =
    if Sys.file_exists index then [ (rel, index) ] else []
  in
  let children =
    if not (Sys.is_directory full) then []
      ...
      |> List.concat_map (fun e ->
          walk_modules dir (Filename.concat rel e))
  in
  self @ children
...
let package_modules pkg_name =
  ...
  if not (Sys.file_exists pkg_dir && Sys.is_directory pkg_dir) then []
    ...
    | None -> []
    | Some libs ->
        libs |> List.concat_map (fun (_lib_name, mod_name) ->
...
let direct_submodules pkg_name mod_name =
  let mod_dir =
    Filename.concat (Filename.concat odoc_dir pkg_name) mod_name
  in
  if not (Sys.file_exists mod_dir && Sys.is_directory mod_dir) then []
    ...
    |> List.filter (fun e ->
        ...
        Sys.is_directory d
        && Sys.file_exists (Filename.concat d "index.html"))
    |> List.sort String.compare
...
(* Extract the preamble and content from an odoc HTML page.
   Drops <nav>, <head>, scripts, and the local TOC. *)
let extract_content html =
  ...
  let body =
    ...
    | Some i ->
        ...
        | None -> ""
        | Some j ->
            String.trim (String.sub html i (j - i))
  in
  preamble ^ "\n" ^ body

(* Extract the local TOC from odoc HTML.
   Returns the inner <nav> wrapped in a right-sidebar container. *)
...
(* Extract the module name from the <h1> in the preamble.
   "Module <code><span>Nx</span></code>" -> "Nx" *)
  ...
  | Some start ->
      ...
      | Some h1 ->
          ...
          | Some h1_end ->
              ...
              if String.length text > 7
                 && String.sub text 0 7 = "Module " then
                Some (String.sub text 7 (String.length text - 7))
              else if String.length text > 12
                      && String.sub text 0 12 = "Module type " then
...
(* Rewrite a single href from odoc-relative to site-absolute.
   [current_dir]: the module's position relative to the odoc root,
   e.g. "nx/Nx/Infix". *)
let rewrite_href ~current_dir href =
  if String.length href = 0 || href.[0] = '#' then href
    ...
    let anchor, path =
      ...
      | Some i ->
          ( String.sub href i (String.length href - i),
            String.sub href 0 i )
      | None -> ("", href)
    in
...
let nav_items ~current_url pkg_name =
  ...
  | Some libs ->
      ...
      libs |> List.iter (fun (lib_name, mod_name) ->
        Printf.bprintf buf
          {|            <li class="nav-group">%s</li>|} lib_name;
        Buffer.add_char buf '\n';
        let url =
          Printf.sprintf "/docs/%s/api/%s/" pkg_name mod_name
        in
        let active =
          if url = current_url then {| class="active"|} else ""
        in
        Printf.bprintf buf
          {|            <li><a href="%s"%s>%s</a></li>|} url active mod_name;
        Buffer.add_char buf '\n';
...
let process_page ~build_dir ~templates_dir ~tab_nav ~lib ~module_rel full_path =
  ...
  let content =
    Printf.sprintf {|<div class="odoc-api">%s</div>|} content
  in
  let mod_title =
  ...
  in
  let result =
    Site.apply_template ~template ~title ~breadcrumbs ~content
      ~lib:(Some lib) ~is_lib_index:false ~page_title:mod_title ~path
