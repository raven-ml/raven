...
let extract_odoc_content soup =
  ...
  | Some body ->
      ...
      (match header with
      | Some h -> append_child wrapper h
      | None -> ());
      
      (* Add content if it exists *)
      (match content_div with
      | Some c -> append_child wrapper c
      | None -> ());
...
let adapt_css_classes content =
  ...
  List.iter (fun (old_class, new_class) ->
    content $$ ("." ^ old_class) |> iter (fun elem ->
    ...
    | Some href when not (String.contains href ':') ->
        ...
        let new_href = 
          if String.starts_with ~prefix:"../" href then
            ...
            "/docs/" ^ library ^ "/api/" ^ (String.sub href 3 (String.length href - 3))
...
let enhance_code_blocks content =
  ...
  content $$ "pre code" |> iter (fun code ->
...
let add_backing_lines content =
  ...
  content $$ ".api-spec" |> iter (fun spec ->
    add_class "spec-backing" spec
  )
...
let remove_stdlib_prefix content =
  ...
  content |> descendants |> elements |> iter (fun elem ->
    let text_content = texts elem |> String.concat "" in
    if String.length text_content > 0 && Str.string_match stdlib_regex text_content 0 then
...
let process_odoc_html ~source ~library ~destination =
  ...
  let final_html = Printf.sprintf
    "---\nlayout: layout_docs_%s\ntitle: %s\n---\n\n%s"
    library
    module_name
    (Soup.to_string content)
  in
...
let () =
  ...
  | _ ->
      ...
      Printf.eprintf "Example: %s _html/nx/Nx.html nx www/site/docs/nx/api/Nx.html\n" Sys.argv.(0);
      exit 1