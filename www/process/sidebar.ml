...
let rec collect_modules entry =
  ...
  | _ ->
    (* For library groups or other containers, collect modules from children *)
      ...
      | Some url ->
        ...
        let link =
          if String.length url > 3 && String.sub url 0 3 = "nx/" then
            ...
            sprintf "%s%s" base_path url
        in
        let children_html = 
          if entry.children = [] then ""
          else 
            let child_modules = List.concat_map collect_modules entry.children in
            if child_modules = [] then ""
            else sprintf "\n<ul>\n%s</ul>" (generate_html ~base_path child_modules)
        in
        sprintf "<li><a href=\"%s\">%s</a>%s</li>" link entry.name children_html
      | None ->
...
let generate_api_nav_html ~library:_ ~sidebar_json_path ~output_path =
  ...
  let full_html = 
    if html = "" then
      ...
      Printf.sprintf "<ul class=\"nav-links\">\n%s\n</ul>" html
  in
