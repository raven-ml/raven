...
let generate_scatter_plot () =
  Random.init 42; (* For reproducibility *)
  let n = 100 in
  let x_data = rand float32 [|n|] in
  let x = mul_s x_data 10. in
  let y = map_item (fun xi -> xi +. Random.float 2. -. 1.) x in
  ...
  let data_array = 
    Array.init 1000 (fun _ -> 
      let rec normal () =
        ...
        if u1 = 0. then normal ()
        else Float.sqrt (-2. *. Float.log u1) *. Float.cos (2. *. Float.pi *. u2)
      in
      normal () *. 2. +. 5.
    ) in
...
let generate_3d_plot () =
  ...
  let fig = plot3d ~title:"3D Helix" ~xlabel:"x" ~ylabel:"y" ~zlabel:"z" x y z in
