(* Generate saga/lib/tokenizers/unicode_data.{ml,mli} from uucp. Run: dune exec
   saga/gen/gen_unicode_data.exe *)
...
let ranges pred =
  ...
  for cp = 0 to max_cp do
    if Uchar.is_valid cp && pred (Uchar.of_int cp) then (if !s < 0 then s := cp)
    else if !s >= 0 then (
      rs := (!s, cp - 1) :: !rs;
      s := -1)
  done;
...
let block_table default data =
  ...
  let index =
    Array.init nb (fun i ->
        let key =
          List.init block_sz (fun j ->
              let cp = (i * block_sz) + j in
              if cp <= max_cp then data.(cp) else default)
        in
...
let emit_block ppf prefix default names idx blks =
  ...
  Array.iteri
    (fun i b -> Array.iteri (fun j v -> flat.((i * block_sz) + j) <- v) b)
    blks;
...
let () =
  Format.printf "Generating Unicode v%s data to %s.{ml,mli}@."
    Uucp.unicode_version file;
  ...
  let gc_data =
    Array.init (max_cp + 1) (fun cp ->
        if Uchar.is_valid cp then
          Hashtbl.find gc_idx
            (gc_str (Uucp.Gc.general_category (Uchar.of_int cp)))
  ...
  for cp = 0 to max_cp do
    if Uchar.is_valid cp then
      Hashtbl.replace sc_set
        (sc_str (Uucp.Script.script (Uchar.of_int cp)))
        true
  done;
  let sc_names =
    Hashtbl.fold (fun k _ acc -> k :: acc) sc_set []
  ...
  let gcb_default = 16 in
  (* XX — most prevalent value *)
  let gcb_data =
    Array.init (max_cp + 1) (fun cp ->
        if Uchar.is_valid cp then
  ...
  for cp = 0 to max_cp do
    if Uchar.is_valid cp then
      match Uucp.Case.Fold.fold (Uchar.of_int cp) with
      | `Self -> ()
      | `Uchars us -> folds := (cp, List.map Uchar.to_int us) :: !folds
  done;
  let folds = List.rev !folds in

  let pp_header ppf =
    pf ppf
      "(* Do not edit. Generated from uucp %s via \
       saga/gen/gen_unicode_data.ml. *)@\n\
       @\n"
  ...
  let pp_types ppf =
    pf ppf "type general_category =@\n  [ %a ]@\n@\n"
      (pp_type_variants ~per_line:5)
      gc_names;
    pf ppf "type script =@\n  [ %a ]@\n@\n"
      (pp_type_variants ~per_line:8)
      sc_names
  in
