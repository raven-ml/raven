...
(** Function that returns logits (unnormalized log probabilities) for the next
    token given the previous token ID. For unigram models, the input is ignored.
*)
...
type config = {
  ...
  top_p : float option;
      (** If set, use nucleus sampling - sample from smallest set of tokens
          whose cumulative probability exceeds p *)
  seed : int option;  (** Random seed for reproducibility *)
}

val default_config : config
(** Default configuration: max_tokens=100, temperature=1.0, top_k=None,
    top_p=None *)
...
val sample_token :
  ...
  float array ->
  int
(** [sample_token ?temperature ?top_k ?top_p ?seed logits] samples a token index
    from logits.
...
val generate :
  ...
  unit ->
  int array
(** [generate ?max_tokens ?temperature ?top_k ?top_p ?seed ?start ~logits_fn
     ~tokenizer ()] generates a sequence of token IDs.
    ...
    @param seed Random seed
    @param start Optional starting text to encode
    @param logits_fn Function to get logits for next token
    @param tokenizer Function to encode text to token IDs
...
val generate_text :
  ...
  unit ->
  string
(** [generate_text] generates text by encoding, sampling, and decoding. Same
    parameters as [generate] plus:
    @param decoder Function to decode token IDs to text
