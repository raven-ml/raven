...
let apply_top_k probs k =
  ...
  if k >= n then probs
    ...
    if sum > 0.0 then Array.map (fun x -> x /. sum) filtered else filtered
...
let apply_top_p probs p =
  ...
  for i = 0 to n - 1 do
    ...
    if !cumsum > p && !cutoff = n then cutoff := i + 1
  done;
...
let sample_from_probs probs rng =
  ...
  for i = 0 to Array.length probs - 1 do
    ...
    if !cumsum > r && !result = Array.length probs - 1 then result := i
  done;
  !result
...
let sample_token ?temperature ?top_k ?top_p ?seed logits =
  ...
  let probs =
    match top_k with Some k -> apply_top_k probs k | None -> probs
  in

  (* Apply top-p filtering if specified *)
  let probs =
    match top_p with Some p -> apply_top_p probs p | None -> probs
  in

  (* Sample from distribution *)
  sample_from_probs probs rng

let generate ?max_tokens ?temperature ?top_k ?top_p ?seed ?start ~logits_fn
    ~tokenizer () =
  ...
  let tokens =
    match start with Some text -> Array.to_list (tokenizer text) | None -> []
  in
  ...
  let rec generate_loop tokens remaining =
    if remaining <= 0 then List.rev tokens
      ...
      let prev_token =
        match tokens with [] -> 0 (* Start token *) | h :: _ -> h
      in
...
let generate_text ?max_tokens ?temperature ?top_k ?top_p ?seed ?start ~logits_fn
    ~tokenizer ~decoder () =
  let tokens =
