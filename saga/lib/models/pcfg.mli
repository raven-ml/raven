...
(** [inside grammar sentence] computes inside probabilities using CKY.
    ...
    Returns a 3D array where [alpha.(i).(j).(A)] = P(span [i,j) derives from
    nonterminal A), the probability that the substring from position i
    (inclusive) to j (exclusive) can be generated by nonterminal A.

    The array has dimensions [(n+1) x (n+1) x num_nonterminals] where n is
    the sentence length. Only entries where [j > i] are meaningful.
...
(** [outside grammar sentence inside_chart] computes outside probabilities.
    ...
    Returns a 3D array where [beta.(i).(j).(A)] = P(generating everything
    outside span [i,j) | A generates span [i,j)), the probability of the
    context outside the span given that nonterminal A generates the span.

    The [inside_chart] must be precomputed using {!inside} on the same
    sentence.

    Outside probabilities are used in the inside-outside algorithm for
...
(** [viterbi grammar sentence] finds the maximum probability parse tree.
    ...
    Returns a backpointer chart where [chart.(i).(j).(A)] = [(k, B, C)]
    indicates that the best way to parse span [i,j) with nonterminal A is to
    split at position k with left child B and right child C.

    For unary rules (terminals), the backpointer is [(-1, -1, -1)].

    Use the backpointers to reconstruct the parse tree by recursively following
    splits from the root span [0, n) with the start symbol.
