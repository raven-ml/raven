...
let sample_texts =
  [
    ...
    "Machine learning and natural language processing are fascinating fields \
     of study.";
    "This is a longer sentence with multiple clauses, punctuation marks, and \
     various word lengths to test tokenization performance.";
    "1234567890 !@#$%^&*() Testing special characters and numbers in \
     tokenization.";
  ]
      ...
      (fun ~param:size ->
        let batch =
          List.init size (fun i ->
              List.nth sample_texts (i mod List.length sample_texts))
        in
...
module Serialization = struct
  let bench_to_json () =
    let tok = create_wordpiece_tokenizer () in
    Ubench.bench "to_json" (fun () -> Tokenizer.to_json tok |> ignore)
  ...
  let all = [ bench_to_json (); bench_from_file () ]
end
