...
module Unigram : sig
  ...
  (** [logits model _prev_token] returns log probabilities for all tokens.
      The previous token is ignored in unigram models. *)
...
module Bigram : sig
  ...
  (** [train_from_corpus ?smoothing corpus] builds model from a list of token arrays *)
  
  val logits : model -> int -> float array
  (** [logits model prev_token] returns log probabilities for next token 
      given the previous token *)
  
  val log_prob : model -> prev:int -> next:int -> float
  (** [log_prob model ~prev ~next] returns conditional log probability P(next|prev) *)
  
  val sample : model -> prev:int -> ?temperature:float -> ?seed:int -> unit -> int
  (** [sample model ~prev ?temperature ?seed ()] samples next token given previous *)
...
module Trigram : sig
  ...
  (** [logits model ~prev1 ~prev2] returns log probabilities given two previous tokens *)
  
  val log_prob : model -> prev1:int -> prev2:int -> next:int -> float
  (** [log_prob model ~prev1 ~prev2 ~next] returns P(next|prev1,prev2) *)
  
  val sample : model -> prev1:int -> prev2:int -> ?temperature:float -> ?seed:int -> unit -> int
...
(** [logits model ~context] returns log probabilities given context.
    Context length should be n-1 for an n-gram model. *)
