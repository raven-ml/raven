...
module Unigram = struct
  ...
  let train tokens =
    ...
    Array.iter (fun token ->
      counts := IntMap.update token (function
        | None -> Some 1
        | Some c -> Some (c + 1)) !counts;
      total := !total + 1;
    ...
    let log_probs = Array.make vocab_size (log (1.0 /. float_of_int (!total + vocab_size))) in
    IntMap.iter (fun token count ->
      log_probs.(token) <- log (float_of_int (count + 1) /. float_of_int (!total + vocab_size))
    ) !counts;
  ...
  let log_prob model token =
    if token >= 0 && token < model.vocab_size then
      model.log_probs.(token)
    else
      log (1.0 /. float_of_int (model.total + model.vocab_size))
  
  let sample model ?(temperature=1.0) ?(seed=Random.int(1000000)) () =
    let rng = Random.State.make [| seed |] in
    let probs = Array.map (fun log_p ->
      exp (log_p /. temperature)
    ) model.log_probs in
    ...
    for i = 0 to model.vocab_size - 1 do
      ...
      if !cumsum > r && !result = model.vocab_size - 1 then
        result := i
    done;
    !result
...
module Bigram = struct
  type model = {
    counts : int IntPairMap.t IntMap.t;  (* prev_token -> (next_token, count) map *)
  ...
  let train ?(smoothing=1.0) tokens =
    ...
    for i = 0 to Array.length tokens - 2 do
      ...
      (* Update counts *)
      counts := IntMap.update prev (function
        | None -> Some (IntPairMap.singleton (prev, next) 1)
        | Some m -> Some (
            IntPairMap.update (prev, next) (function
              | None -> Some 1
              | Some c -> Some (c + 1)
            ) m
          )
      ...
      context_totals := IntMap.update prev (function
        | None -> Some 1
        | Some c -> Some (c + 1)
      ) !context_totals
    done;
  ...
  let logits model prev_token =
    let logits = Array.make model.vocab_size (log 0.0) in  (* Initialize with -inf *)
    
    (* Get context total with smoothing *)
    let context_total = 
      match IntMap.find_opt prev_token model.context_totals with
      | Some total -> float_of_int total +. (model.smoothing *. float_of_int model.vocab_size)
    ...
    for next_token = 0 to model.vocab_size - 1 do
      let count =
        ...
        | Some next_map ->
            ...
            | Some c -> c
      in
      logits.(next_token) <- log ((float_of_int count +. model.smoothing) /. context_total)
    done;
  ...
  let stats model =
    let unique_bigrams = 
      IntMap.fold (fun _ next_map acc ->
        acc + IntPairMap.cardinal next_map
      ) model.counts 0
    in
    {
      vocab_size = model.vocab_size;
      total_tokens = IntMap.fold (fun _ count acc -> acc + count) model.context_totals 0;
...
module Trigram = struct
  type model = {
    counts : (int * int * int, int) Hashtbl.t;  (* (prev1, prev2, next) -> count *)
  ...
  let log_prob model ~prev1 ~prev2 ~next =
    ...
    let count =
      try Hashtbl.find model.counts (prev1, prev2, next)
      with Not_found -> 0
    in
    
    log ((float_of_int count +. model.smoothing) /. context_total)
  
  let sample model ~prev1 ~prev2 ?(temperature=1.0) ?(seed=Random.int(1000000)) () =
    let rng = Random.State.make [| seed |] in
  ...
  let stats model =
    {
      vocab_size = model.vocab_size;
      total_tokens = Hashtbl.fold (fun _ count acc -> acc + count) model.context_totals 0;
...
let create ~n ?(smoothing=1.0) tokens =
  ...
  for i = 0 to Array.length tokens - n do
    let context = 
      if n = 1 then []
      else Array.to_list (Array.sub tokens i (n - 1))
    in
    ...
    let next_counts = 
      try Hashtbl.find counts context
  ...
  
  {
    n;
    vocab_size = !max_token + 1;
    counts = Hashtbl.create 0;  (* Placeholder - real implementation would convert *)
...
let generate model ?(max_tokens=100) ?(temperature=1.0) ?(seed=Random.int(1000000)) ?(start=[||]) () =
  let rng = Random.State.make [| seed |] in
