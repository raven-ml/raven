...
let stats_testable =
  testable
    (fun fmt s ->
      Format.fprintf fmt "{ vocab_size = %d; total_tokens = %d; unique_ngrams = %d }"
        s.Ngram.vocab_size s.Ngram.total_tokens s.Ngram.unique_ngrams)
    ( = )

(* Test data *)
let simple_tokens = [| 0; 1; 2; 1; 3; 2; 1; 0 |]
let _text_tokens = [| 10; 11; 12; 13; 11; 12; 14; 15; 11; 12 |]  (* "the cat sat on cat sat mat dog cat sat" *)
...
let test_unigram_logits () =
  let model = Ngram.Unigram.train simple_tokens in
  let logits = Ngram.Unigram.logits model 0 in  (* prev token ignored *)
...
let test_bigram_logits () =
  let tokens = [| 0; 1; 0; 1; 0; 2 |] in  (* 0->1 appears twice, 1->0 appears twice *)
...
let test_bigram_sample () =
  let tokens = [| 0; 1; 0; 1; 0; 1 |] in  (* 0 always followed by 1 *)
  ...
  for _ = 1 to 20 do
    let next = Ngram.Bigram.sample model ~prev:0 ~temperature:0.01 ~seed:(Random.int 1000) () in
...
let test_trigram_logits () =
  let tokens = [| 0; 1; 2; 0; 1; 2; 0; 1; 3 |] in  (* 0,1->2 appears twice, 0,1->3 once *)
...
let test_trigram_sample () =
  let tokens = [| 0; 1; 2; 0; 1; 2; 0; 1; 2 |] in  (* 0,1 always followed by 2 *)
  let model = Ngram.Trigram.train ~smoothing:0.01 tokens in
  
  (* Should almost always sample 2 after 0,1 *)
  let mut_count = ref 0 in
  for _ = 1 to 20 do
    let next = Ngram.Trigram.sample model ~prev1:0 ~prev2:1 ~temperature:0.01 ~seed:(Random.int 1000) () in
...
let test_save_load_unigram () =
  ...
  check float_array_testable "loaded model should produce same logits" logits1 logits2;
...
let test_generic_generate () =
  ...
  let generated = Ngram.generate model ~max_tokens:5 ~temperature:1.0 ~seed:42 () in
  ...
  Array.iter (fun token ->
    check bool "generated token should be in range" true (token >= 0 && token < 100)
  ) generated

(* Edge cases *)
let test_empty_corpus () =
  (* Empty corpus should still create a valid model, just with no observations *)
  let model = Ngram.Unigram.train [||] in
  let stats = Ngram.Unigram.stats model in
  check int "empty corpus vocab_size" 1 stats.vocab_size;  (* min vocab size is 1 *)
